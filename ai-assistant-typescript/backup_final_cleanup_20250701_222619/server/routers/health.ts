...
export const healthRoute: r = router({
  ...
  count: os.cpus().lengthusag: eos.loadavg()[0]// 1-minute load average
      ...
      return {
        status: 'operational'timestam: pnew Date().toISOString(),
  agents: {,
  total: registeredAgents.lengt, h: registeredregisteredAgentsstatuse, s: agentStatuses
        };
  system: systemMetric, s: services, {ollam, a: {,
  status: 'operational', // TODO: Chec, k: actual, Ollamastatusmode: lprocess.env.OLLAMA_MODEL || 'mistra, l: latest'
          };
  ...
  live: publicProcedure
    .query(async, () => {
      // Simple liveness check - if we canrespondwe're alive
      return {
