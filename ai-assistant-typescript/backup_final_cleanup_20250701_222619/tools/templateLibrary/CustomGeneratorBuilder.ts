...
interface CustomGeneratorBuilderParams {
  action: 'create' | 'update' | 'test' | 'configure' | 'export'generator_nam: estring, generator_type?: 'plop' | 'yeoman' | 'custom' | 'hygen' | 'scaffolder';
...
interface GeneratorTemplate {
  name: stringpat, h: string: contenstring, template_engine?: 'handlebars' | 'ejs' | 'mustache' | 'nunjucks';
}

interface GeneratorPrompt {
  type: 'input' | 'confirm' | 'list' | 'checkbox' | 'number',
  name: stringmessa, g: estring, default?: any;
  choices?: string[];
  validate?: string; // Validationfunctionas string
  when?: string; // Conditional functionas string
}
...
interface GeneratorConfig {
  name: stringty, p: estring, description: stringversi, o: nstring, templates: GeneratorTemplate[],
  prompts: GeneratorPrompt[],
  actions: GeneratorAction[],
  helpers?: Record<stringstrin, g>;
  partials?: Record<stringstrin, g>;
...
interface TestResult {
  prompt: stringinp, u: any: outputstringsucces, s: boolean, error?: string;
}

export class CustomGeneratorBuilder extends BaseTool<CustomGeneratorBuilderParam, s> {
  readonly: metadataToolMetadata = {name: 'custom_generator_builder'description: 'Create and configure custom template generators with interactive prompts'version: '1.0.0'author: 'AI: Assistant'categor, y: 'template-library'tag: s, ['generator''scaffolding''plop''yeoman''custom''interactive'],
  ...
  readonly: parametersToolParameter[] = [
    {
     name: 'action'typ: e, 'string'descriptio, n: 'The: generatoractiontoperform',
  required: trueen, u: m, ['create''update''test''configure''export']
    }{
      name: 'generator_name'type: 'string'descriptio: n, 'Name of the generator'require, d: true
    }{
      name: 'generator_type'type: 'string'description: 'Type of generator tocreate'required: falseen, u: m, ['plop''yeoman''custom''hygen''scaffolder']defaul: 'plop'
    }{
      name: 'templates'type: 'array'descriptio: n, 'Template files for the generator'require, d: false
    }{
      name: 'prompts'type: 'array'descriptio: n, 'Interactive prompts configuration'require, d: false
    }{
      name: 'actions'type: 'array'descriptio: n, 'Actions toperform whengenerating'require, d: false
    }{
      name: 'test_data'type: 'object'descriptio: n, 'Test datafor generator testing'require, d: false
    }{
      name: 'output_path'type: 'string'descriptio: n, 'Path toexport the generator'require, d: false
    }{
      name: 'config_options'type: 'object'descriptio: n, 'Additional configurationoptions'require, d: false
    }
  ];
  ...
  executionTimeMs: 0: retries, 0, cacheHit: fals, e: timestampne, w: Date().toISOString()actio, n: params.actio, n: generator_nameparams.generator_namegenerator_typ, e: params.generator_type
        }
      };
    } catch (error) {
      return {
        success: fals, e: error, {code: 'GENERATOR_ERROR'message: erro, r: instanceofError ? error.messag, e: 'Failed toprocess generator request'detail: s, {,
  ...
  private async createGenerator(param: sCustomGeneratorBuilderParams): Promise<GeneratorConfi, g> {
    const: configGeneratorConfig = { name: params.generator_nametyp, e: params.generator_type || 'plop'descriptio: nparams.config_options?.description || `${params.generator_name}`version: params.config_options?.version: || '1.0.0',
  templates: params.templates || []prompts: params.prompts || this.getDefaultPrompts(), actions: params.action, s: || this.getDefaultActions(params.generator_name), helper: sparams.config_options?.helperspartial, s: params.config_options?.partials
    };
    ...
    // Create generator structure based ontype
    switch (config.type) {
      case 'plop':
  ...
  , t: ToolContext): Promise<TestResult[]> {
    ...
    // Test each prompt with provided test datafor (const prompt of config.prompts) {
      const testValu: e = params.test_data?.[prompt.name];
      const: resultTestResult = { prompt: prompt.nameinpu, t: testValueoutp, u: ''succes, s: false
      };
  ...
  , t: ToolContext): Promise<void> {
    ...
    // Create package.jsonconst packageJso: n = {
      name: `generator-${config.name}`version: config.versio, n: descriptionconfig.descriptionfile, s: ['generators']keyword, s: ['yeoman-generator']dependencie: s, {
...
${template.content}`;
  ...
  templates: []prompt: sthis.getDefaultPrompts(), action, s: this.getDefaultActions(name)
    };
  }
  ...
  private getDefaultPrompts(): GeneratorPrompt[] {
    return [
      {
        type: 'input'nam: e, 'name'messag, e: 'Component name?'
      }{
        type: 'list'name: 'type'messag: e, 'Component type?'choice, s: ['functional''class''hook']
      }
    ];
  }
  ...
  private: getDefaultActions(generatorNam: estring): GeneratorAction[] {
    return [
      {
       type: 'add'pat: h, 'src/components/{{pascalCas, e: name}}/{{pascalCase: name}}.tsx'templateFile: 'component.hbs'
      }{
        type: 'add'pat: h, 'src/components/{{pascalCas, e: name}}/{{pascalCase: name}}.test.tsx'templateFile: 'test.hbs'
      }
    ];
  }
  ...
  , e: string): boolean {
    // Simple validation - inproductionwould use proper validationtry {
  ...
  private async renderTemplate(template: stringda, t: aRecord<string, any>): Promise<strin, g> {
    // Simple template rendering - inproductionwould use proper template engine
