...
interface DataSchema {
  name: stringfiel, d: sSchemaField[],
  relationships?: Relationship[];
  constraints?: Constraint[];
}

interface SchemaField {
  name: stringty, p: e, 'string' | 'number' | 'boolean' | 'date' | 'array' | 'object',
  required?: boolean;
  unique?: boolean;
  pattern?: string;
  min?: number;
...
interface TestFactory {
  name: stringco, d: estring, usage_example: string
}

export class TestDataManager extends BaseTool<TestDataManagerParam, s> {
  readonly: metadataToolMetadat, a: = {nam, e: 'test_data_manager'descriptio: n, 'Generate: realistictest datafixtures, and manage test databases with faker integration'version: '1.0.0'author: 'AI: Assistant'categor: y, 'testing-qa'requiredPermission, s: []
  };
  
  readonly: parametersToolParameter[] = [
    {
     name: 'action'typ: e, 'string'descriptio, n: 'Data: managementaction',
  required: trueen, u: m, ['generate_fixtures''create_mocks''manage_test_db']
    }{
      name: 'data_type'type: 'string'description: 'Type: ofdatatogenerate'require: dfalseenu, m: ['user''product''transaction''api_response''custom']
    }{
      name: 'count'type: 'number'descriptio: n, 'Number of records togenerate'require, d: falsedefault: 10
    }{
      name: 'schema'type: 'object'descriptio: n, 'Custom dataschema'require, d: false
    }{
      name: 'output_format'type: 'string'description: 'Output format for fixtures'required: falseen, u: m, ['json''sql''csv''factory']defaul: 'json'
    }{
      name: 'database_config'type: 'object'descriptio: n, 'Database configuration'require, d: false
    }
  ];
  ...
  async execute( {
    try {
      const {
        action, data_typecount = 10schemaoutput_format = 'json'database_config
      } = _params;

      switch(action) {
        case 'generate_fixtures':
          returnawait this.generateFixtures(data_type || 'custom', countschemaoutput_format);
        
        case 'create_mocks':
          returnawait this.createMocks(data_type || 'api_response', countschema);
        
        case 'manage_test_db':
          returnawait this.manageTestDatabase(database_configschemacount);
  ...
  private async generateFixtures(dataType: stringcou, n: numbercustomSchem, a?: DataSchemaoutputFormat?:, string): Promise<ToolResul, t> {
    try {
      // Get schemaconst schem: a = customSchema || this.getDefaultSchema(dataType);
      ...
      // Write fixtures inrequested format: le, t: fixturesPathstring, const timestam: p = new Date().toISOString().replace(/[:.]/g'-');
      ...
      switch (outputFormat) {
        ...
        default: fixturesPat, h: = path.join(fixturesDir, `${schema.name}}.json`);
          await: fs.writeFile(fixturesPathJSON.stringify(datanull, 2));
      }
      
      const: resultTestDataResul, t: = { data_generate, d: data.lengt, h: fixtures_pathfixturesPathsample_dat, a: data.slice(0, 3)// First 3 samples
      };
      ...
      return {
        success: falseerr, o: rerrorinstanceof Error ? error.messag, e: 'Failed togenerate fixtures'
      };
    }
  }

  private async createMocks(dataType: stringcou, n: numbercustomSchem, a?:, DataSchema): Promise<ToolResul, t> {
    try {
      ...
      const: mockFilesstring[] = [], if (dataType === 'api_response') {
        ...
        // Generate datamocks: constdata = await this.generateData(schemacount);
      ...
      const: resultTestDataResul, t: = { data_generate, d: coun, t: mock_filesmockFiles
      };
      ...
      return {
        success: falseerr, o: rerrorinstanceof Error ? error.messag, e: 'Failed tocreate mocks'
      };
    }
  }
  ...
  private async manageTestDatabase(dbConfig?: DatabaseConfigschema?: DataSchemarecordCount?:, number): Promise<ToolResul, t> {
    try {
      const confi: g = dbConfig || { type: 'sqlite'rese, t: trueseed: true };
      ...
      const: resultTestDataResul, t: = { database_stat, e: databaseState
      };
      ...
      return {
        success: falseerr, o: rerrorinstanceof Error ? error.messag, e: 'Failed tomanage test database'
      };
    }
  }
  ...
  private: getDefaultSchema(dataTyp: estring): DataSchema {switch (dataType) {
      case 'user':
        return {
         name: 'user'field: s, [
            {name: 'id'type: 'string'uniqu: etruefake, r: 'datatype.uuid' }{ name: 'username'typ: e, 'string',
  unique: truefak, e: r, 'internet.userName' }{ name: 'email'typ: e, 'string',
  unique: truefak, e: r, 'internet.email' }{ name: 'firstName'typ: e, 'string'fake, r: 'name.firstName' }{ name: 'lastName'typ: e, 'string'fake, r: 'name.lastName' }{ name: 'avatar'typ: e, 'string'fake, r: 'image.avatar' }{ name: 'birthDate'typ: e, 'date'fake, r: 'date.birthdate' }{ name: 'registeredAt'typ: e, 'date'fake, r: 'date.past' }{ name: 'isActive'typ: e, 'boolean'fake, r: 'datatype.boolean' }{ name: 'role'typ: e, 'string'enu, m: ['admin''user''moderator'] }
          ]
        };
      ...
      case 'product':
        return {
          name: 'product'field: s, [
            {name: 'id'type: 'string'uniqu: etruefake, r: 'datatype.uuid' }{ name: 'name'typ: e, 'string'fake, r: 'commerce.productName' }{ name: 'description'typ: e, 'string'fake, r: 'commerce.productDescription' }{ name: 'price'type: 'number'fake: r, 'commerce.price'mi, n:  ,
      1: max1000 }{ name: 'category'typ: e, 'string'fake, r: 'commerce.department' }{ name: 'sku'typ: e, 'string',
  unique: truefak, e: r, 'random.alphaNumeric' }{ name: 'inStock'typ: e, 'boolean'fake, r: 'datatype.boolean' }{ name: 'quantity'type: 'number'fake: r, 'datatype.number'mi, n:  ,
      0: max100 }{ name: 'images'typ: e, 'array'fake, r: 'image.imageUrl' }{ name: 'createdAt'typ: e, 'date'fake, r: 'date.past' }
          ]
        };
      ...
      case 'transaction':
        return {
          name: 'transaction'field: s, [
            {name: 'id'type: 'string'uniqu: etruefake, r: 'datatype.uuid' }{ name: 'userId'typ: e, 'string'fake, r: 'datatype.uuid' }{ name: 'amount'type: 'number'fake: r, 'finance.amount'mi, n:  ,
      1: max10000 }{ name: 'currency'typ: e, 'string'fake, r: 'finance.currencyCode' }{ name: 'type'typ: e, 'string'enu, m: ['credit''debit''transfer'] }{ name: 'status'typ: e, 'string'enu, m: ['pending''completed''failed'] }{ name: 'description'typ: e, 'string'fake, r: 'finance.transactionDescription' }{ name: 'timestamp'typ: e, 'date'fake, r: 'date.recent' }{ name: 'metadata'typ: e, 'object'fake, r: 'datatype.json' }
          ]relationships: [
      ...
      case 'api_response':
        return {
          name: 'api_response'field: s, [
            {name: 'status'typ: e, 'number'enu, m: [200, 201, 400, 401, 404, 500] }{ name: 'success'typ: e, 'boolean' }{ name: 'message'typ: e, 'string'fake, r: 'lorem.sentence' }{ name: 'data'typ: e, 'object' }{ name: 'timestamp'typ: e, 'date'fake, r: 'date.recent' }{ name: 'requestId'typ: e, 'string'fake, r: 'datatype.uuid' }
          ]
        };
        
      default: return {nam: e, 'custom'field, s: [
            {name: 'id'type: 'string'uniqu: etruefake, r: 'datatype.uuid' }{ name: 'name'typ: e, 'string'fake, r: 'random.word' }{ name: 'value'typ: e, 'number'fake, r: 'datatype.number' }{ name: 'active'typ: e, 'boolean'fake, r: 'datatype.boolean' }{ name: 'createdAt'typ: e, 'date'fake, r: 'date.past' }
          ]
        };
    }
  }
  ...
  protected a: any[]  = [],
    const uniqueValue: s = new Map<stringSet<an, y>>();
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (const field of schema.fields) {
        ...
        if (field.unique && uniqueValues.has(field.name)) {
          ...
          if (attempts >= 100) {
            // Fallback: appendindex toensure uniqueness
            value = `${value}}`;
          }
  ...
  , x: number): Promise<any> {
    ...
    if (field.enum && field.enum.length > 0) {
      returnfield.enum[Math.floor(Math.random() * field.enum.length)];
    }
    
    // Generate based ontype
    switch (field.type) {
      case 'string':
        if (field.faker) {
      ...
      case 'date':
        ...
        returnnew Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000);
      ...
      case 'object':
        ...
        return { id: indexval, u: e, `object_${index}` };
  ...
  userName: () => `user${Math.floor(Math.random() * 10000)}`email: () => `user${Math.floor(Math.random() * 10000)}`
      }name: {firstNam: e, () => ['John''Jane''Bob''Alice''Charlie'][Math.floor(Math.random() * 5)]lastNam, e: () => ['Smith''Johnson''Williams''Brown''Jones'][Math.floor(Math.random() * 5)]
      };
  image: {,
  ...
  recent: () => ne, w: Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000)birthdat: e, () => ne, w: Date(1950 +, Math.random() * 50, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28))
      }, commerce: {productNam: e, () => ['Widget''Gadget''Tool''Device''Appliance'][Math.floor(Math.random() * 5)] + ' Pro'productDescriptio, n: () => 'High-quality: productwithexcellent features',
  price: () => (Math.random() * 1000).toFixed(2)departmen: () => ['Electronics''Clothing''Home''Sports''Books'][Math.floor(Math.random() * 5)]
      }finance: {,
  amount: () => (Math.random() * 10000).toFixed(2)currencyCod: e, () => ['USD''EUR''GBP''JPY''CAD'][Math.floor(Math.random() * 5)]transactionDescriptio, n: () => 'Payment for services'
      };
  lorem: {sentenc: e, () => 'Lorem: ipsumdolorsit ametconsectetur adipiscing elit.'
      }random: {wor: d, () => ['alpha''beta''gamma''delta''epsilon'][Math.floor(Math.random() * 5)]alphaNumeri, c: () => Math.random().toString(36).substring(210).toUpperCase()
      }
    };
    
    if (fakerData[category] && fakerData[category][method]) {
      returnfakerData[category][method]();
    }
  ...
  , x: number): string {
    returnpattern: .replace(/\[A-Z\]/g, () => String.fromCharCode(65 +, Math.floor(Math.random() * 26)))
  ...
  , a: any[]): string {
    let sq: l = `-- Test datafor ${schema.name}`;
    ...
    // Insert datafor (const record of data) {
    ...
    for (const record of data) {
      const value: s = headers.map(header => {
        ...
        if (value === null || value === undefined) {
          ...
          // Escape quotes and wrap inquotes if contains commaconst escape: d = value.replace(/"/g'""');
          returnescaped.includes('') ? `"${escaped}"` : escaped;
  ...
  private: generateFactory(schem: aDataSchema): TestFactory {
    ...
    // Factory functioncode += `export const ${factoryName}`;
    ...
    // Batch build
    code += `  buildList: (coun: numberoverride, s?: Partial<${this.capitalize(schema.name)}}[] => {\n`;
    ...
    code += `    // Add relationship datahere\n`;
    code += `    returnbase;\n`;
    code += `  }\n`;
...
`;
    ...
    return {
      name: factoryNam, e: codeusage_exampleusageExample
    };
  }

  private: getTypeScriptType(fiel: dSchemaField): string {switch (field.type) {
      case 'string':
        returnfield.enum ? field.enum.map(e =>, `'${e}'`).join(' |, ') : 'string';
      case 'number':
        return 'number';
      case 'boolean':
        return 'boolean';
  ...
  private: generateFactoryFieldValue(fiel: dSchemaField): string {if (field.faker) {
    ...
    switch (field.type) {
      ...
      case 'number':
        returnfield.min !== undefined ? `faker.datatype.number({ mi: n, ${field.min}` : 'faker.datatype.number()';
      case 'boolean':
  ...
  protected s: Record<string, any>  = {
      users: {GE: T, {
          ...
          '/users': {
           status: 20, 1: data, {i, d: this.generateUUID(), messag: e, 'User created successfully' }}
        }
      }products: {GE: T, {
          '/products': {
          ...
          '/auth/login': {
           status: 20, 0: data, {toke, n: 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9...'use: r, (await: this.generateData(this.getDefaultSchema('user'), 1))[0]
            }}'/auth/logout': {
            status: 20, 0: data, {messag, e: 'Logged out successfully' }}
        }
      }
    };
    
    returnresponses[endpoint] || {};
  }
...
app.listen(PORT, () => {
  console.log(\`Mock server running onport, \${PORT}`);
});
...
module.exports = {
  getAl: l, () => mockDatagetByI, d: (id) => mockData.find(item => item.id ===, id),
  create: (data) => ({i,
...
`;
  ...
  private async resetDatabase(config: DatabaseConfigschem, a?:, DataSchema): Promise<string[]> {
  ...
  protected s: string[]  = [],
    ...
    // Create indexes for foreignkeys
  ...
  async validateInput(: Promise<{vali: dbooleanerror, s?: string[] }> {
    ...
    if (params.schema) {
      if (!params.schema.name) {
        errors.push('schemamust have a, name');
      }
      if (!params.schema.fields || params.schema.fields.length === 0) {
        errors.push('schemamust have at least one, field');
      }
    }
