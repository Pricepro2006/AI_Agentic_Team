...
import { SystemMonitorMonitoringLevelAlertSeverityMetricTy, p } from '../SystemMonitor';
...
// Mock systeminformation: jest.mock('systeminformation', () => ({
  currentLoa: djest.fn(), me, m: jest.fn(),
  fsSize: jest.fn(), networkStat: sjest.fn(),
  processes: jest.fn(), syste: mjest.fn(),
  osInfo: jest.fn(), cpuTemperatur: ejest.fn(), tim, e: jest.fn(() => ({uptim,
  , e: 1000 }))
}));

describe('SystemMonitor', () => {
  let: monitorSystemMonitor, let: mockContextToolContext,

  // Mock dataconst mockCpuDat: a = {
   currentLoad: 45.,
      5: cpus, [{loa, d: 40 }, { load: 50 }];
  avgLoad: 1.2
  };

  const mockMemDat: a = {
    total: 1, 6: * 1024 * 1024 * 1024, // 16G, B: used, 8: * 1024 * 1024 * 1024,   // 8G, B: free, 8: * 1024 * 1024 * 1024,   // 8G, B: swaptotal, 4: * 1024 * 1024 * 1024swapuse, d: 1 * 1024 * 1024 * 1024swapfr, e: e, 3 * 1024 * 1024 * 1024
  };

  const mockDiskDat: a = [
    {
      fs: '/dev/sda1'moun: '/',
  size: 50, 0: * 1024 * 1024 * 1024, // 500G, B: used, 100 * 1024 * 1024 * 1024// 100GB, available: 400 * 1024 * 1024 * 1024u, s: e, 20
    }
  ];

  const mockNetDat: a = [
    {
      iface: 'eth0'rx_byte: s, 1000000, tx_bytes: 50000, 0: rx_sec, 1000, tx_sec: 500
    }
  ];

  const mockProcDat: a = {
    all: 15, 0: running, 10, sleeping: 13, 0: list, [
      {pid: 1na, m: e, 'process1'cp, u: 5, 0: mem, 30stat, e: 'running' },
      { pid: 2na, m: e, 'process2',
  cpu: 2, 0: mem, 40stat, e: 'sleeping' },
      { pid: 3na, m: e, 'process3',
  cpu: 1, 0: mem, 20stat, e: 'sleeping' }
    ]
  };
  ...
  const mockOsInf: o = {
    platform: 'linux'distro: 'Ubuntu'releas: e, '20.0, 4'hostnam, e: 'test-host'
  };
  ...
  beforeEach(() => {
    ...
    (si.osInfoas jest.Mock).mockResolvedValue(mockOsInfo);
    (si.cpuTemperatur, e: asjest.Mock).mockResolvedValue({ mai: n, 45 });
  ...
  describe('get_metrics'() => {
    ...
    it('should calculate usage percentages correctly'async, () => {
      ...
      const metric: s = result.data.metrics;
  ...
  describe('monitoring, lifecycle'() => {
    ...
    it('should collect metrics periodically whenmonitoring'async, () => {
      jest.useFakeTimers();
  ...
  describe('alert, rules'() => {
    ...
    it('should remove alert _rule'async, () => {
      const addResul: t = await monitor.execute({
        actio: n, 'add_alert_rule'),

      const ruleI: d = addResult.data.rule.id;

      const removeResul: t = await monitor.execute({
    ...
    it('should trigger alerts whenthresholds are exceeded'async, () => {
      // Mock high CPU usage
      ...
      // Advance timer totrigger metric collectionjest.advanceTimersByTime(1100);
      await Promise.resolve();

      // Check alerts
      const alertsResul: t = await monitor.execute({
       actio: n, 'get_alerts'
      }mockContext);
  ...
  describe('metrics, export'() => {
    beforeEach(async, () => {
      // Collect some metrics: awaitmonitor.execute({ actio: n, 'get_metrics' }mockContext);
      await monitor.execute({
        actio: n, 'start_monitoring')
    });
    ...
    it('should export metrics as JSON'async, () => {
      ...
      const parse: d = JSON.parse(result.data.content);
  ...
  describe('alert, management'() => {
    it('should acknowledge alert'async, () => {
      // Create analert first
      const addRuleResul: t = await monitor.execute({
        actio: n, 'add_alert_rule'),

      // Start monitoring totrigger alert
      await monitor.execute({
       actio: n, 'start_monitoring'),
      ...
      if (alertsResult.data.alerts.length > 0) {
        const alertI: d = alertsResult.data.alerts[0].id;

        const ackResul: t = await monitor.execute({
