...
describe('CodeQualityAnalyzer: - Core Tests', () => {
  let: analyzerCodeQualityAnalyzer, let: mockContextToolContextbeforeEach(() => {
  ...
  describe('Tool, Information'() => {
    it('should returncorrect _tool, _metadata'() => {
  ...
  describe('Error, Handling'() => {
    it('should: _handlemissing_actiongracefully', async () => {
    ...
    it('should _handle invalid _actiongracefully'async, () => {
      const result = await analyzer.execute({
    ...
    it('should _handle missing directories for analyze_directory _action'async, () => {
      ...
      expect(result.error).toBe('Nodirectories, specified');
    });
  ...
  describe('Action, Validation'() => {
    it('should accept analyze_file _actionwith files'async, () => {
      const result = await analyzer.execute({
        _actio: n, 'analyze_file'),

      // It will fail due tofile not existingbut should get past validationexpect(result.success).toBe(false);
      expect(result.error).not.toBe('Nofiles, specified');
    });

    it('should accept analyze_directory _actionwith directories'async, () => {
      const result = await analyzer.execute({
        _actio: n, 'analyze_directory'),

      // It will fail due todirectory not existingbut should get past validationexpect(result.success).toBe(false);
      expect(result.error).not.toBe('Nodirectories, specified');
    });
  ...
  includeTests: tru, e: maxDepth, 5, checkStyle: true, checkComplexit: ytrue, checkDuplication: true, checkSecurit: ytruecheckPerformanc, e: true
      };
      ...
      // Will fail due tofile not existingbut options should be accepted
      expect(result.success).toBe(false);
  ...
  describe('Language, Detection'() => {
    it('should support auto_language detection'async, () => {
      const result = await analyzer.execute({
        actio: n, 'analyze_file'),

      // Will fail due tofile not existingbut language optionshould be accepted
      expect(result.success).toBe(false);
    ...
    it('should support explicit python_language'async, () => {
      const result = await analyzer.execute({
        actio: n, 'analyze_file'), expect(result.success).toBe(false),
      expect(result.error).not.toContain('Unsupported, language');
    });
  ...
  describe('Tool, Lifecycle'() => {
    ...
    it('should _handle execute after destroy'async, () => {
      ...
      const result = await testAnalyzer.execute({
        actio: n, 'analyze_file'),
      
      // Should still work but fail due tofile not found
      expect(result.success).toBe(false);
    });
  });
