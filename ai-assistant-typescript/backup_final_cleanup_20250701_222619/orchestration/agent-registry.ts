...
import { 
  ProjectOrganizationExpert, DocumentationExpert, TemplateLibraryExpert, CodeReviewExpert, TestingAndQAExpert, GitHubIntegrationExpert, ErrorAnalysisExpert, ProjectPlanningExpert, GitHubActionsExpert, MasterOrchestrator, PythonExpert, ArchitectureExpert
} from '@/agents/experts'
...
export class AgentRegistry {
  ...
  public async registerAllAgents(): Promise<void> {
    ...
    try {
      ...
      logger.info('Agent registration completed successfully', {
        totalAgents: this.registeredAgents.size, agents: Array.from(this.registeredAgents.keys())
      })
  ...
  private async registerAgent(agentId: string, agent: any): Promise<void> {
    try {
      ...
      logger.info(`Agent registered successfully: ${agentId}`, {
        agentType: agent.constructor.name, agentId
      })
    } catch (error) {
  ...
  public getRegistrationStatus(): Record<string, any> {
    ...
    for (const [agentId, agent] of this.registeredAgents) {
      status[agentId] = {
        registered: true, type: agent.constructor.name, timestamp: new Date().toISOString()
      }
    }
