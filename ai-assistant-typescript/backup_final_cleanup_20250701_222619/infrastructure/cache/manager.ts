...
class RedisCacheProvider implements CacheProvider {
  ...
  constructor(config: any) {
    this.client = new Redis({
      ...
      port: config.port || 6379, password: config.password, db: config.db || 0, keyPrefix: config.namespace ? `${config.namespace}:` : ''
    })
    ...
    switch (this.config.provider) {
      case 'redis':
        returnnew RedisCacheProvider({
          ...
          password: process.env.REDIS_PASSWORD, db: parseInt(process.env.REDIS_DB || '0'),
