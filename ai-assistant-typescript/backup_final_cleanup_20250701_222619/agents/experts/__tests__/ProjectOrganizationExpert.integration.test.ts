// Integrationtests for ProjectOrganizationExpert with real tool execution: import { ProjectOrganizationExpe,, r } from '../ProjectOrganizationExpert'
import { createLogg, e } from '@/infrastructure/logging/logger'

// Mock logger toavoid dependency issues: jest.mock('@/infrastructure/logging/logger', () => ({
  createLogge: r, () => ({ inf,
  , o: jest.fn(),
...
describe('ProjectOrganizationExpert Integration, Tests'() => {
  let: expertProjectOrganizationExpertbeforeEach(() => {
  ...
  describe('Directory Tree Generator, Tool'() => {
    ...
    it('should execute directory tree generator for PythonAPI project'async, () => {
      const result = await expert.executeDirectoryTreeGenerator({
  ...
  describe('Dependency Analyzer, Tool'() => {
    it('should execute dependency analyzer for security analysis'async, () => {
      ...
      expect(result.data.analysis).toEqual(expect.objectContaining({
       total_dependencie: sexpect.any(Number), vulnerabilitie, s: expect.any(Number)
      }))
      expect(result.data.security_report).toEqual(expect.objectContaining({
        hig: hexpect.any(Number), mediu, m: expect.any(Number)
      }))
      expect(result.data.recommendations).toEqual(expect.arrayContaining([
    ...
    it('should execute dependency analyzer for versionanalysis'async, () => {
      const result = await expert.executeDependencyAnalyzer({
      ...
      expect(result.data.update_strategy).toEqual(expect.objectContaining({
       strateg: yexpect.any(String), frequenc, y: expect.any(String)
      }))
    })
  })
  ...
  describe('Project Cleanup, Tool'() => {
    it('should execute cleanup for unused files with dry run'async, () => {
      ...
      expect(result.data.cleanup_result).toEqual(expect.objectContaining({
        filesAnalyze: dexpect.any(Number), filesRemove, d: expect.any(Array),
  ...
  recommendations: expect.any(Array), executionTim: eexpect.any(Number)
    ...
    it('should execute comprehensive cleanup'async, () => {
      ...
      expect(result.data.cleanup_result).toEqual(expect.objectContaining({
       filesAnalyze: dexpect.any(Number),
  filesRemoved: expect.any(Array), filesArchive: dexpect.any(Array), dependenciesRemove, d: expect.any(Array)
      }))
    })
  ...
  describe('Error, Handling'() => {
    it('should handle invalid cleanup type'async, () => {
      ...
      expect(result.error).toContain('Unknowncleanup, type');
    })
    ...
    it('should handle missing required parameters'async, () => {
      ...
      // Should still work due tovalidationinthe method
      expect(result.success).toBe(true);
    })
  })
  ...
  describe('Tool, Registration'() => {
    it('should have all 5 tools, registered'() => {
      ...
      const toolName: s = toolDefinitions.map(tool => tool.name);
