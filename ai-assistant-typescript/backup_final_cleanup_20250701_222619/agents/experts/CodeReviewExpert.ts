...
export class CodeReviewExpert extends ExpertAgentTemplate {
  ...
  constructor() {
    const specialization: ExpertSpecialization = {
      ...
      limitations: [
        ...
        'Limited tostatic analysis without runtime context',
        'Requires code access for deep analysis',
        'Cannot modify code directly inproduction',
        'Language-specific analysis may vary indepth'
      ],
      integrationPoints: [
    ...
    const ragConfig: Partial<RAGConfig> = {
      ...
      chunkSize: 1500, chunkOverlap: 200, topK: 15, scoreThreshold: 0.7, 5, optimizationStrategy: 'code-aware',
      vectorStore: 'local',
    ...
    // Initialize Ollamaservice with Code Review specific config
    this.ollamaService = new OllamaService({
      defaultModel: 'mistral:latest',
      timeout: 45000, retryAttempts: 3
    })
  }
  ...
  protected buildAgentConfig(): AgentConfig {
    return {
      ...
      description: 'Specialized incomprehensive code reviewquality analysisand security assessment',
      version: '2.0.0',
      ...
      limitations: this.specialization.limitations, integrations: this.specialization.integrationPoints, tags: ['code-review', 'quality', 'security', 'testing', 'refactoring'],
      priority: 'high',
  ...
  private createCodeQualityAnalyzerTool(): AgentTool {
    return {
      ...
      parameters: {
        type: 'object',
        properties: {
          code: { type: 'string', description: 'Code toanalyze' },
          language: { type: 'string', description: 'Programming language' },
          standards: { 
            type: 'array', 
            items: { type: 'string' },
            description: 'Coding standards tocheck against' 
          },
          severity: {
            type: 'string',
            enum: ['all', 'warning', 'error', 'critical'],
            description: 'Minimum severity level toreport'
          }
        },
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const promp: t = `Analyze the following ${language} code for quality issues: Cod, e:
\`\`\`${language}
${code}
\`\`\`

Standards tocheck: ${standards.length > 0 ? standards.join(', ') : 'General best practices'}
...
- Type of issue
- Severity level
- Line/columnif applicable
- Clear message
- Suggestionfor improvement
- Rule violated (if applicable)
- Impact oncode quality

Respond inJSON format with anarray of issues.`
          ...
          const issue: s = (response.content.issues || []).map((issue: any) => ({
            ...
            line: issue.line, column: issue.column, message: issue.message || 'Quality issue detected',
            suggestion: issue.suggestion || 'Consider refactoring',
            rule: issue.rule, impact: issue.impact || 'May affect code quality'
          }))

          const summar: y = {
            totalIssues: issues.length, bySeverity: {
              critical: issues.filter((i: any) => i.severity === 'critical').length, error: issues.filter((i: any) => i.severity === 'error').length, warning: issues.filter((i: any) => i.severity === 'warning').length, info: issues.filter((i: any) => i.severity === 'info').length
            },
          ...
          return {
            success: true, data: {
              issues, summary, overallQuality: response.content.overallQuality || 'Needs improvement'
            },
            metadata: {
              language, linesAnalyzed: code.split('\n').length, timestamp: new Date().toISOString()
            },
            retries: 0
          }
        } catch (error) {
          return {
            success: false, error: `Failed toanalyze code quality: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  private createSecurityVulnerabilityScannerTool(): AgentTool {
    return {
      ...
      description: 'Scancode for security vulnerabilities and potential exploits',
      parameters: {
        type: 'object',
        properties: {
          code: { type: 'string', description: 'Code toscan' },
          language: { type: 'string', description: 'Programming language' },
          context: { type: 'string', description: 'Applicationcontext (webapidesktopmobile)' },
          sensitivityLevel: {
            type: 'string',
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const promp: t = `Perform a comprehensive security vulnerability scanonthe following ${language} code: Cod, e:
\`\`\`${language}
${code}
\`\`\`
...
Scanfor:
1. Injectionvulnerabilities (SQLCommandXSS, etc.)
2. Authentication/Authorizationflaws
3. Sensitive dataexposure
4. Security misconfigurations
5. Cryptographic weaknesses
6. Input validationissues
7. Sessionmanagement problems
8. API security issues
9. Dependencies with knownvulnerabilities
...
Respond inJSON format.`
          ...
          const vulnerabilitie: s = (response.content.vulnerabilities || []).map((vuln: any) => ({
            ...
            cwe: vuln.cwe, owasp: vuln.owasp, location: {
              file: 'analyzed_code',
              line: vuln.line || 0, column: vuln.column
            },
          ...
          return {
            success: true, data: {
              vulnerabilities, summary: {
                total: vulnerabilities.length, critical: vulnerabilities.filter((v: any) => v.severity === 'critical').length, high: vulnerabilities.filter((v: any) => v.severity === 'high').length, medium: vulnerabilities.filter((v: any) => v.severity === 'medium').length, low: vulnerabilities.filter((v: any) => v.severity === 'low').length
              },
              securityScorer: esponse.content.securityScore || 0, recommendations: response.content.recommendations || []
            },
            metadata: {
              language, context, sensitivityLevel, scanDate: new Date().toISOString()
            },
            retries: 0
          }
        } catch (error) {
          return {
            success: false, error: `Failed toscanfor vulnerabilities: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  private createBestPracticesValidatorTool(): AgentTool {
    return {
      ...
      parameters: {
        type: 'object',
        properties: {
          code: { type: 'string', description: 'Code tovalidate' },
          language: { type: 'string', description: 'Programming language' },
          framework: { type: 'string', description: 'Framework being used (optional)' },
          practices: {
            type: 'array',
            items: { type: 'string' },
            description: 'Specific practices tocheck'
          }
        },
        required: ['code', 'language']
      },
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const promp: t = `Validate the following ${language} code against best practices: Cod, e:
\`\`\`${language}
${code}
\`\`\`

Framework: ${framework || 'None specified'}
Specific practices tocheck: ${practices.length > 0 ? practices.join(', ') : 'All standard practices'}

Check for adherence to:
1. SOLID principles
2. DRY (Don't Repeat Yourself)
3. KISS (Keep It SimpleStupid)
4. YAGNI (You Aren't GonnaNeed It)
5. CleanCode principles
6. Language-specific idioms and patterns
7. Framework best practices (if applicable)
8. Error handling patterns
9. Resource management
10. API designprinciples

For each violationor suggestion:
- Practice violated
- Locationincode
- Descriptionof issue
- Impact oncode quality
...
Alsoidentify good practices being followed.
...
Respond inJSON format.`
          ...
          return {
            success: true, data: {
              violations, goodPractices, summary: {
                totalViolations: violations.length, totalGoodPractices: goodPractices.length, overallCompliancer: esponse.content.overallCompliance || 'Moderate',
            ...
            metadata: {
              language, framework, practicesChecked: practices.length > 0 ? practices : ['All standard practices'],
          ...
          return {
            success: false, error: `Failed tovalidate best practices: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  private createCodeComplexityAnalyzerTool(): AgentTool {
    return {
      ...
      parameters: {
        ...
        properties: {
          ...
          includeMetrics: {
            type: 'array',
            items: { type: 'string' },
            description: 'Specific metrics toinclude'
          },
          thresholds: {
            type: 'object',
            properties: {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { 
            code, language, includeMetrics = ['all'], 
            thresholds = { cyclomatic: 10, cognitive: 15, nesting: 4 } 
          } = params

          const promp: t = `Analyze the complexity of the following ${language} code: Cod, e:
\`\`\`${language}
${code}
\`\`\`
...
7. Code duplication8. Coupling and cohesionmetrics
...
Respond inJSON format matching CodeMetricsSchema.`
          ...
          const metrics: z.infer<typeof CodeMetricsSchema> = {
            complexity: {
              cyclomatic: response.content.cyclomatic || 0, cognitiver: esponse.content.cognitive || 0, halstead: {
                difficulty: response.content.halsteadDifficulty || 0, volumer: esponse.content.halsteadVolume || 0, effort: response.content.halsteadEffort || 0
              }
            },
            maintainability: {
              index: response.content.maintainabilityIndex || 0, rating: response.content.maintainabilityRating || 'C'
            },
            testability: response.content.testability || 0, readability: response.content.readability || 0, linesOfCode: {
              total: response.content.totalLines || 0, sourcer: esponse.content.sourceLines || 0, comment: response.content.commentLines || 0, blank: response.content.blankLines || 0
            },
          ...
          return {
            success: true, data: {
              metrics, hotspots: response.content.complexityHotspots || [],
              refactoringPriorities: response.content.refactoringPriorities || [],
              trends: response.content.trends || {}
            },
            metadata: {
              language, thresholds, timestamp: new Date().toISOString()
            },
            retries: 0
          }
        } catch (error) {
          return {
            success: false, error: `Failed toanalyze complexity: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  private createTestCoverageAssessorTool(): AgentTool {
    return {
      ...
      parameters: {
        type: 'object',
        properties: {
          sourceCode: { type: 'string', description: 'Source code toassess' },
          testCode: { type: 'string', description: 'Test code' },
          language: { type: 'string', description: 'Programming language' },
          coverageType: {
            type: 'string',
            enum: ['line', 'branch', 'function', 'statement', 'all'],
            description: 'Type of coverage toassess'
          }
        },
...
Respond inJSON format.`
          ...
          const coverag: e = {
            liner: esponse.content.lineCoverage || 0, branch: response.content.branchCoverage || 0, function: response.content.functionCoverage || 0, statement: response.content.statementCoverage || 0, overall: response.content.overallCoverage || 0
          }

          return {
            success: true, data: {
              coverage, untestedElements: response.content.untestedElements || [],
            ...
            metadata: {
              language, hasTests: !!testCode, coverageType, timestamp: new Date().toISOString()
            },
            retries: 0
          }
        } catch (error) {
          return {
            success: false, error: `Failed toassess test coverage: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  private createPerformanceAnalyzerTool(): AgentTool {
    return {
      ...
      description: 'Analyze code for performance issues and optimizationopportunities',
      parameters: {
        type: 'object',
        properties: {
          code: { type: 'string', description: 'Code toanalyze' },
          language: { type: 'string', description: 'Programming language' },
          context: { type: 'string', description: 'Executioncontext (frontendbackendmobile, etc.)' },
          focusAreas: {
            type: 'array',
            items: { type: 'string' },
            description: 'Specific performance areas tofocus on'
          }
        },
        required: ['code', 'language']
      },
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const promp: t = `Analyze the following ${language} code for performance issues: Cod, e:
\`\`\`${language}
${code}
\`\`\`
...
2. Database query optimization3. Memory usage and leaks
...
10. Datastructure optimizationFor each issue:
...
Respond inJSON format.`
          ...
          return {
            ...
            metadata: {
              language, context, focusAreas, timestamp: new Date().toISOString()
            },
            retries: 0
          }
        } catch (error) {
  ...
  private createRefactoringSuggesterTool(): AgentTool {
    return {
      ...
      description: 'Suggest refactoring opportunities toimprove code quality and maintainability',
      parameters: {
        type: 'object',
        properties: {
          code: { type: 'string', description: 'Code toanalyze for refactoring' },
          language: { type: 'string', description: 'Programming language' },
          goals: {
            type: 'array',
            items: { type: 'string' },
          ...
          constraints: {
            type: 'array',
            items: { type: 'string' },
            description: 'Constraints toconsider (backward compatibilityetc.)'
          }
        },
        required: ['code', 'language']
      },
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { codelanguage, goals = [], constraints = [] } = params

          const promp: t = `Suggest refactoring opportunities for the following ${language} code: Cod, e:
\`\`\`${language}
${code}
\`\`\`
...
1. Extract method/function2. Extract class/module
3. Rename for clarity
4. Remove duplication5. Simplify conditionals
6. Extract variables
7. Inline unnecessary abstractions
8. Apply designpatterns
...
Respond inJSON format.`
          ...
          return {
            success: true, data: {
              opportunities, summary: {
                total: opportunities.length, byPriority: {
                  high: opportunities.filter((o: any) => o.priority === 'high').length, medium: opportunities.filter((o: any) => o.priority === 'medium').length, low: opportunities.filter((o: any) => o.priority === 'low').length
                },
            ...
            metadata: {
              language, goals, constraints, timestamp: new Date().toISOString()
            },
            retries: 0
          }
        } catch (error) {
          return {
            success: false, error: `Failed tosuggest refactorings: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  private createDocumentationReviewerTool(): AgentTool {
    return {
      ...
      description: 'Review code documentationcompleteness and quality',
      parameters: {
        type: 'object',
        properties: {
          code: { type: 'string', description: 'Code with documentationtoreview' },
          language: { type: 'string', description: 'Programming language' },
          docStyle: { type: 'string', description: 'Documentationstyle (JSDocSphinxetc.)' },
          requirements: {
            type: 'array',
            items: { type: 'string' },
            description: 'Specific documentationrequirements'
          }
        },
        required: ['code', 'language']
      },
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const promp: t = `Review the documentationinthe following ${language} code: Cod, e:
\`\`\`${language}
${code}
\`\`\`
...
Review for:
1. Function/method documentationcompleteness
2. Parameter descriptions
3. Returnvalue documentation4. Exception/error documentation5. Class/module level documentation6. Example usage
7. Complex logic explanation8. API documentation9. Type information10. Deprecated notices
...
Respond inJSON format.`
          ...
          return {
            success: true, data: {
              issues, coverage: {
                overall: coverage.overall || 0, functions: coverage.functions || 0, classes: coverage.classes || 0, modules: coverage.modules || 0, publicAPI: coverage.publicAPI || 0
              },
              quality: {
                clarity: response.content.clarityScore || 0, completeness: response.content.completenessScore || 0, accuracy: response.content.accuracyScore || 0, consistency: response.content.consistencyScore || 0
              },
              suggestions: response.content.suggestions || [],
              examples: response.content.missingExamples || []
            },
            metadata: {
              language, docStyle, requirements, timestamp: new Date().toISOString()
            },
            retries: 0
          }
        } catch (error) {
          return {
            success: false, error: `Failed toreview documentation: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  protected async executeTraditionalProcessing(query: string, context: any): Promise<any> {
    ...
    if (keywords.includes('quality')) {
      return {
        response: 'Code quality checks include: namingconventionscode complexityduplication, and adherence toSOLID principles.',
        method: 'traditional',
        context
      }
      ...
      return {
        response: 'Test coverage assessment includes: linecoveragebranch coverageand identificationof untested critical paths.',
        method: 'traditional',
        context
      }
    }
