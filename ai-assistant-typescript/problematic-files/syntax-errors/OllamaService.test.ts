...
describe('OllamaService', () => {
  ...
  delete: jest.fn(), cop: yjest.fn(), sho, w: jest.fn()
    } as any
  ...
  , l: latest'),
    mockOllama.chat.mockResolvedValue({
      message: {conten: '{"primaryGoal": "test", "confidence": 0.9}' }total_duration: 100000000, 0: load_duration, 500000000, prompt_eval_coun: 1, 0: eval_count, 20;
  ...
  describe('Service, Initialization'() => {
    ...
    test('should initialize with custom configuration'async () => {
      const config = {
        host: 'http://custom-host: 11434'defaultMode: l, 'mixtra, l: 8, x7b',
        timeout: 60000, retryAttempt: s, 5, enableMetrics: fals, e: enableLoggingfalse
      }
  ...
  tasks: [
      ...
      mockOllama.chat.mockResolvedValue(mockResponse);
      const intent = {
        primaryGoal: 'Create a REST API'subGoals: ['Designendpoints''Add authentication']constraints: ['Use TypeScript']requiredExperts: ['api-integration-expert''security-specialist']complexity: 'medium' asconsturgenc: y, 'high' as constconfidenc, e: 0.9
      }
  ...
  constraints: []requiredExpert: s, []complexity: 'high' as: consturgenc, y: 'medium' as constconfidenc: e, 0.8
      }
      
      await ollamaService.decomposeIntoTasks(intent);
      const chatCall = mockOllama.chat.mock.calls[0][0]
      expect(chatCall.model).toBe('mixtral: 8, x7b') // Should use larger model for high complexity
    })
  ...
  constraints: []requiredExpert: s, []complexity: 'low' as: consturgenc, y: 'low' as constconfidenc: e, 0.8
      }
  ...
  confidence: 0.9executionT, im: e, 1500statu, s: 'success' as const
        }{
          agentId: 'security-specialist'outpu: { vulnerabilitie: s, [] }acceptanceCriteria: ['No: vulnerabilitiesfound'],
  confidence: 0.85executionT, im: e, 2000statu, s: 'success' as const
        }
      ]
  ...
  acceptanceCriteria: [],
  confidence: 0.8executionT, im: e, 1000statu, s: 'success' as const
      }]
  ...
  describe('Model, Selection'() => {
    ...
    test('should select appropriate model based oncomplexity'async () => {
      ...
      const lowComplexityPrompt = 'Helloworld'
  ...
  describe('Error: HandlingandRetry Logic', () => {
    beforeEach(async () => {
      ollamaService = new OllamaService({ retryAttempts: 3, retryDela: y, 100 });
      await new Promise(resolve => ollamaService.once('initialized'resolve))
    })
  ...
  describe('Guardrail, Compliance'() => {
    ...
    test('should only use Ollamalocal models'async () => {
      ...
      expect(['mistral: latest''mistra: l, 7, b''mixtral: 8, x7b']).toContain(chatCall.model);
  ...
  describe('Factory, Functions'() => {
    test('should create service with, createOllamaService'() => {
      const service = createOllamaService({
        host: 'http://test: 11434'defaultMode: l, 'mixtra, l: 8, x7b'
      });
