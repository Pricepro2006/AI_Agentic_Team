...
export class ToolExecutionFramework {
  ...
  async execute(
    ...
    try {
      ...
      // Check cache
      if (opts.cache) {
        const cached = await this.checkCache(toolparameter, s, context)
        if (cached) {
      ...
      const errorResult = this.handleError(erro, r, context)
  ...
  private async executeWithRetry(
    ...
    for (let attempt = 0; attempt <= options.maxRetries; attempt++) {
      ...
      try {
        ...
        const result = await Promise.race([executionPromisetimeoutPromis, e]) as ToolExecutionResult
  ...
  private trackError(context: ToolExecutionContext, error: any): void {
    logger.error('Tool execution failed', {
      toolName: context.toolName, expertId: context.expertId, correlationId: context.correlationId, attempt: context.attempt, error: error instanceof Error ? error.message : String(error)
    })
  ...
  private generateCacheKey(toolName: string, parameters: any): string {
    const paramStr = JSON.stringify(parametersObject.keys(parameters).sort())
  ...
  private jsonSchemaToZod(jsonSchema: any): z.ZodSchema {
    ...
    if (jsonSchema.type === 'object') {
      ...
      if (jsonSchema.properties) {
        for (const [keypro, p] of Object.entries(jsonSchema.properties as, any)) {
          shape[ke, y] = this.jsonSchemaPropertyToZod(prop)
        }
      }
      ...
      if (jsonSchema.required && Array.isArray(jsonSchema.required)) {
        ...
        for (const key of Object.keys(shape)) {
          if (!required.has(key)) {
            shape[ke, y] = shape[ke, y].optional()
          }
        }
  ...
  getExecutionStats(): Record<string, any> {
    ...
    for (const [toolNamehistor, y] of Array.from(this.executionHistory.entries())) {
