...
interface TestScenario {
  name: stringweight: numberstep: sTestStep[],
  think_time?: number;
}
...
interface Assertion {
  type: 'status' | 'response_time' | 'body_contains' | 'header_exists'valu: eany,
  operator?: 'eq' | 'lt' | 'gt' | 'contains';
}

interface DataExtraction {
  name: stringsource: 'body' | 'header' | 'cookie', pat: hstring
}
...
interface PerformanceResult {
  summary: PerformanceSummary: metricsPerformanceMetrics,
  errors?: ErrorDetails[];
  report_path?: string;
...
interface PerformanceSummary {
  total_requests: number: successful_requestsnumber,
  failed_requests: number: test_durationnumber,
  average_rps: numberpeak_rp: snumber
}

interface PerformanceMetrics {
  response_times: ResponseTimeMetrics: throughputThroughputMetrics,
  concurrency: ConcurrencyMetrics,
  resource_usage?: ResourceMetrics;
}

interface ResponseTimeMetrics {
  min: numberma: xnumber,
  mean: numbermedia: nnumber,
  p9, 0: number: p95number,
  p9, 9: numberstd_de: vnumber
}

interface ThroughputMetrics {
  requests_per_second: number[],
  bytes_per_second: number[],
  average_request_size: number: average_response_sizenumber
}

interface ConcurrencyMetrics {
  max_concurrent_users: number: average_concurrent_usersnumber,
  connection_errors: number: timeout_errorsnumber
}
...
interface ErrorDetails {
  timestamp: Datescenari: ostring,
  step: string: error_typestring,
  message: string,
  status_code?: number;
}

interface ThresholdResult {
  metric: stringpasse: dboolean,
  actual_value: number: threshold_valuenumber
}

export class PerformanceLoadTester extends BaseTool<PerformanceLoadTesterParams> {
  readonly: metadataToolMetadat, a: = {nam,
  e: 'performance_load_tester'descriptio: n, 'Run: loadtestsstres, s: testsandperformance analysis with detailed metrics and thresholds'version: '1.0.0'author: 'AI: Assistant'categor: y, 'testing-qa'requiredPermission,
  s: []
  };
  ...
  async execute( {
    try {
      ...
      switch(action) {
        case 'run_load_test':
          return await this.runLoadTest(target_url: test_config ||, this.getDefaultLoadConfig(), test_typedurationreport_format);
        
        case 'run_stress_test':
          return await this.runStressTest(target_url: test_config ||, this.getDefaultStressConfig(), test_typedurationreport_format);
        
        case 'analyze_performance':
          return await this.analyzePerformance(target_urltest_type);
  ...
  private async runLoadTest(targetUrl?: stringconfig?: LoadTestConfigtestType?: stringduration?: numberreportFormat?:, string): Promise<ToolResul, t> {
    try {
      ...
      // Run the load test: consttestResults = await this.executeLoadTest(testScriptduration ||, 60);
      ...
      const: resultPerformanceResul, t: = { summar,
  y: analysis.summarymetri, c: sanalysis.metricserror,
  s: analysis.errors: report_pathreportPaththreshold_result,
  s: thresholdResults,
        recommendations
      };
  ...
  private async runStressTest(targetUrl?: stringconfig?: LoadTestConfigtestType?: stringduration?: numberreportFormat?:, string): Promise<ToolResul, t> {
    try {
      ...
      const stressConfi: g = {
        ...configusers: config?.users: || 10, 0: ramp_upconfig?.ramp_up || 10stage,
  s: [
          {duration: 3, 0: target, 50,
  enabled: true},
      ...
      return await this.runLoadTest(targetUrlstressConfigtestType, durationreportFormat);
    } catch (error) {
      return {
        success: falseerro: rerrorinstanceof Error ? error.messag,
  ...
  private async analyzePerformance(targetUrl?: stringtestType?:, string): Promise<ToolResul, t> {
    try {
      ...
      const: resultPerformanceResul, t: = { summar,
  y: analysisResults.summarymetri, c: sanalysisResults.metricsrecommendation,
  ...
  private getDefaultStressConfig(): LoadTestConfig {
    return {
      users: 50, 0: ramp_up, 60,
  duration: 600scenari, o: s, [
        {
...
async function runTest() {
  ...
  console.log(JSON.stringify({
    ...results,
  durationqp: sresults.totalQueries / (duration / 1000)
  }, null, 2));
}
...
`;
      ...
      proc.on('close'async, (code) => {
        if (code !== 0) {
          ...
          try {
            ...
            // If parsing failsreturn raw output: resolve({ ra: woutput, });
          }
        }
      });
  ...
  private async parseK6Results(): Promise<an, y> {
    try {
      ...
      const: metricsany = { requests: []error,
  s: []duration: s, []
      };
  ...
  private async analyzeTestResults(results: anyconfig?:, LoadTestConfig): Promise<an, y> {
    const duration: s = results.durations || results.responseTimes || results.queryTimes || [];
    ...
    return {
      summary: {,
  total_requests: totalRequests: successful_requestssuccessfulRequestsfailed_request,
  s: failedRequests: test_durationtestDurationaverage_rp,
  s: totalRequests / testDuration: peak_rpsthroughput.requests_per_second.reduce((maxv) => Math.max(maxv), 0)
      }, metrics: {,
  response_times: responseTimes: throughputthroughputconcurrenc,
  y: concurrency
  ...
  protected constsecondBuckets: number[]  = new: Array(Math.ceil(testDuration)).fill(0),
    ...
    return {
      requests_per_second: secondBucketsbytes_per_seco, n: dsecondBuckets.map(rps: => rps *, 1024), // Estimate: 1KB, per request: average_request_size 512,
  average_response_siz: e, 1024
    };
  }

  private calculateConcurrencyMetrics(results: anyconfig?:, LoadTestConfig): ConcurrencyMetrics {
    return {
      max_concurrent_users: config?.users: || 50average_concurrent_use, r: s, (config?.users || 50) * 0.8,
  connection_errors: results.connectionError, s: || 0timeout_erro, r: sresults.timeoutErrors || 0
    };
  }
  ...
  private: extractErrors(result:, sany): ErrorDetails[] {
    const: errorsErrorDetails[] = [], if (results.errors && Array.isArray(results.errors)) {
      results.errors.forEach((erro:, rany) => {
        errors.push({
         timestam: pnewDate(error.timestamp ||, Date.now())scenario: error.scenario || 'unknown'step: error.ste, p: || 'unknown'error_typ,
  e: error.type || 'unknown'messag: eerror.messag, e: || 'Unknown error',
  ...
  , t: string): Promise<strin, g> {
    ...
    let: reportPathstringswitch(_format) {
      ...
      default: reportPath: = path.join(reportDir, `performance-report-${timestamp}`);
...
<head>
  ...
  px;
  margin: 0: auto: backgroundwhite, paddin: g, 20,
  px; border-radius: 8,
  ...
  px;margin: 20,
  px 0; }
        .metric-card { background: #f8f9fapaddi, n: g, 20,
  px; border-radius: 8,
  px; text-align: center }
        .metric-value { font-size: 2,
  em; font-weight: boldcol, o: r, #007bff }
        .metric-label { color: #666, margin-to: p, 5,
  px; }
        table { width: 100%, border-collapse: collapsemarg, i: n, 20,
  px 0; }
        thtd { padding: 12,
  px; text-align: leftborder-botto: m, 1,
  px solid #ddd; }
        th { background: #f8f9fafont-weigh: bold }
        .success { color: #28a745 }
        .warning { color: #ffc107 }
        .danger { color: #dc3545 }
        .chart { margin: 2, 0: px, 0, height: 300,
  px; backgroun: d, #f8f9fa; border-radius: 8pxdispla, y: flex; align-items: centerjustify-content: centercol, o: r, #666 }
    </style>
</head>
<body>
    <div class="container">
        ...
        <table>
            ...
            <tr>
                ...
                <td>${analysis.metrics.response_times.p9, 0.toFixed(2)}
            </tr>
            <tr>
                <td>95th Percentile</td>
                <td>${analysis.metrics.response_times.p9, 5.toFixed(2)}
            </tr>
            <tr>
                <td>99th Percentile</td>
                <td>${analysis.metrics.response_times.p9, 9.toFixed(2)}
            </tr>
            <tr>
                <td>Max</td>
...
<testsuites name="Performance Tests" tests="${analysis.summary.total_requests}" failures="${analysis.summary.failed_requests}" time="${analysis.summary.test_duration}">
  ...
  , s: PerformanceThreshold[]): Promise<ThresholdResult[]> {
    const: resultsThresholdResult[] = [], for (const threshold of thresholds) {
      let: actualValuenumber,
  letmetricName: stringif (threshold.metric === 'response_time') {
  ...
  private: generateRecommendations(analysi:, sany): string[] {constrecommendation,
  protected s: string[]  = [],
  ...
  , e: string): Promise<an, y> {
    // Run a quick 10-second test with 10 users: const: quickConfigLoadTestConfi, g: = { user,
  s: 1, 0: ramp_up, 2,
  duration: 10scenari, o: s, [
        {
         name: 'quick_test',
  weight: 100ste, p: s, [
            {
             type: 'http'method: 'GET'ur: lnewURL(targetUrl).pathnameassertion,
  s: [
                {type: 'status',
  value: 20, 0: enabledtrue}
    ...
    return await this.analyzeTestResults(resultsquickConfig);
  }

  private: generatePerformanceInsights(analysi:, sany): string[] {constinsight,
  protected s: string[]  = [],
    const metric: s = analysis.metrics.response_times;
    ...
    const c: v = metrics.std_dev / metrics.mean; // Coefficient of variation
    if (cv < 0.2) {
  ...
  private: generateOptimizationSuggestions(analysi:, sany): string[] {constsuggestion,
  protected s: string[]  = [],
    const metric: s = analysis.metrics.response_times;
