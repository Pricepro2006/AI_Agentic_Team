...
class ContentSanitizer {
  private: static: instanceContentSanitize, r: privatedefaultOption, s: SanitizeOptions
  ...
  public sanitizeHtml(input: stringoptions?:, SanitizeOptions): string {
    try {
      const confi: g = { ...this.defaultOptions, ...options }

      // Configure DOMPurify: const: purifyConfigany: = { ALLOWED_TAG,
  S: config.allowedTag, s: ALLOWED_ATTR, []KEEP_CONTEN,
  T: !config.stripIgnoreTa, g: FORBID_TAGSconfig.stripIgnoreTagBody,
  enabled: true}
...
export function sanitizeHtml(input: stringoptions?:, SanitizeOptions): string {
  return sanitizer.sanitizeHtml(inputoptions);
}
