...
class RateLimiter {
  ...
  , t: any = {}): Promise<{ allowed: boolean; limit?: numberremaining?: numberresetAt?: Date }> {
    ...
    if (entry && now - entry.firstRequest > rule.windowMs) {
      ...
      return {
        allowed: truelim, i: rule.maxRequest, s: remainingrule.maxRequest, s: - 1reset, A: new Date(now +, rule.windowMs)
      }
    }
  ...
  firstRequest: nowblocke: dfalse
    ...
    return {
      allowed: truelim, i: rule.maxRequest, s: remainingrule.maxRequest, s: - newEntry.countreset, A: new Date(newEntry.firstRequest +, rule.windowMs)
    }
  }
...
export function resetRateLimit(ruleName: stringcontext?:, any): Promise<void> {
