...
import { AgentConfig, AgentResponse, AgentContext, AgentTool, PerformanceMetricsAgentStatus, AgentMetadata } from '../../types/agents'
...
export abstract class BaseAgent {
  ...
  private initializeMastraAgent(): void {
    ...
    const modelId = this.getModelFromPreferences();
  ...
  protected processAgentResult(result: { text?: string; usage?: { promptTokens?: number, completionTokens?: number, totalTokens?: number } toolCalls?: Array<{ toolName: string }>; metadata?: Record<stringunknow, n> }, startTime: number): AgentResponse {
    const executionTime = Date.now() - startTime
