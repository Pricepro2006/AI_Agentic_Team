...
const RoutingStrategySchema = z.enum(['single_expert', 'multi_expert', 'sequential', 'parallel'])
...
export class MasterOrchestrator extends ExpertAgentTemplate {
  ...
  protected buildAgentConfig(): AgentConfig {
    return {
      ...
      description: 'Central coordinator for multi-agent system, responsible for query routing, task decomposition, and response aggregation',
      version: '2.0.0',
  ...
  private createInterpretRequestAITool(): AgentTool {
    return {
      ...
      description: 'Use AI to interpret user request and extract intent, domain, and requirements',
      parameters: {
        type: 'object',
        properties: {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Analyze this user query and extract:
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Break down this request into specific tasks:
Query: ${query}
...
Respond in JSON format with a 'tasks' array.`
          ...
          const tasks = response.content.tasks || []
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Evaluate this response against quality criteria:
...
Respond in JSON format.`
          ...
          const aiResponse = await this.ollamaService.analyze({
            prompt,
            responseFormat: 'json'
          })
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const query = intent.query || intent.originalQuery || 'No query provided'
          ...
          const { decision, expert, secondaryExperts } = await expertRouter.routeQuery(
            query,
            context,
            {
              maxExperts: 3,
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const expertInstances = []
          for (const agentId of agents) {
            try {
              const { expert } = await expertRouter.routeQuery(
                task,
                agentContext,
                { forceExpert: agentId }
              )
          ...
          const result = await expertRouter.coordinateMultiExpertExecution(
            expertInstances,
            task,
            agentContext,
            strategy as any
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Parse this query using ${parsingStrategy} strategy:
Query: "${query}"
...
Respond in JSON format.`
  ...
  private createGenerateOutcomeSpecificationTool(): AgentTool {
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { taskId = `task_${Date.now()}`, taskDescription, requirements = {} } = params

          const prompt = `Generate detailed outcome specification for this task:
...
Respond in JSON format.`
  ...
  private createGenerateTaskPlanTool(): AgentTool {
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { tasks = [], constraints = {}, resources = [] } = params

          const prompt = `Create execution plan for these tasks:
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Create coordination plan for multi-agent collaboration:
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Validate progress for task:
Task ID: ${taskId}
...
Respond in JSON format.`
  ...
  // Tool 11: Assess delivery confidence
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Assess delivery confidence:
Task Plan: ${JSON.stringify(taskPlan)}
...
Respond in JSON format.`
  ...
  // Tool 12: Traditional routing fallback
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          let selectedAgent = 'architecture-expert' // default
          let confidence = 0.5
  ...
  async execute(query: string, context: AgentContext): Promise<ToolExecutionResult> {
    try {
      ...
      const interpretTool = this.getToolDefinitions().find(t => t.name === 'interpret_request_ai')
      if (!interpretTool) {
        throw new Error('Request interpretation tool not available')
      }

      // Use the tool execution framework
      const interpretResult = await toolExecutionFramework.execute(
        interpretTool,
        { query, context: context.metadata || {} },
        this.config.id,
      ...
      const intent = interpretResult.data as RequestIntent

      // Check if this is a multi-agent task
      const needsMultipleExperts = 
      ...
      if (needsMultipleExperts) {
        ...
        const decomposeTool = this.getToolDefinitions().find(t => t.name === 'decompose_tasks_ai')
        if (decomposeTool) {
          const decomposeResult = await toolExecutionFramework.execute(
            decomposeTool,
            { query, context: context.metadata || {} },
            this.config.id,
            { maxRetries: 2, timeout: 30000, cache: true }
          ...
          if (decomposeResult.success) {
            ...
            const coordinateTool = this.getToolDefinitions().find(t => t.name === 'coordinate_multi_agent')
            if (coordinateTool) {
              const tasks = (decomposeResult.data as TaskDecomposition).tasks
              const uniqueExperts = [...new Set(tasks.flatMap(t => t.requiredExpertise))]
              
              const coordResult = await toolExecutionFramework.execute(
                coordinateTool,
                {
                  task: query,
      ...
      const routeTool = this.getToolDefinitions().find(t => t.name === 'route_with_enhanced_tools')
      if (!routeTool) {
        throw new Error('Routing tool not available')
      }

      const routingResult = await toolExecutionFramework.execute(
        routeTool,
        { intent, availableAgents: [], strategy: 'auto' },
        this.config.id,
        { maxRetries: 2, timeout: 30000, cache: true }
      )
      ...
      const routingDecision = routingResult.data as RoutingDecision
      ...
      const expertResult = await expertRouter.executeWithExpert(
        expert,
        query,
        context
      )
      ...
      if (expertResult.success && routingDecision.supportingAgents.length > 0) {
        const supportingResults = await Promise.allSettled(
          routingDecision.supportingAgents.map(async (agent) => {
            ...
            return expertRouter.executeWithExpert(supportingExpert, query, context)
          })
        )

        // Aggregate results
        const successfulSupporting = supportingResults
