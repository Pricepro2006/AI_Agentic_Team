...
export class ArchitectureExpert extends ExpertAgentTemplate {
  ...
  constructor() {
    const specialization: ExpertSpecialization = {
      ...
      limitations: [
        'Cannot directly provision infrastructure',
        'Requires external tools for deployment',
        'Limited to architectural guidance and design',
        'Cannot modify production systems directly',
        'Requires collaboration for implementation'
      ],
      integrationPoints: [
        'Development teams',
        'DevOps pipeline',
    ...
    // Initialize Ollama service with Architecture specific config
  ...
  protected buildAgentConfig(): AgentConfig {
    return {
      ...
      description: 'Software architecture specialist focusing on system design, patterns, and technology decisions',
      version: '2.0.0',
      ...
      tags: ['architecture', 'design', 'patterns', 'technology'],
      priority: 'high',
  ...
  // Tool 1: Code Analyzer - Analyzes code for structural quality and patterns
  private createCodeAnalyzerTool(): AgentTool {
    return {
      name: 'code_analyzer',
      description: 'Analyze code for structural quality, architectural patterns, and provide recommendations',
      parameters: {
        type: 'object',
        properties: {
          filePath: { type: 'string', description: 'Path to file or directory to analyze' },
          analysisType: { 
            type: 'string', 
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { filePath, analysisType = 'all', depth = 3 } = params

          const prompt = `Analyze the code structure and architecture at ${filePath}:
Analysis Type: ${analysisType}
Depth: ${depth}

Provide:
1. Overall structure analysis (files, lines, languages, modules)
2. Architectural patterns identified
3. Code quality metrics (complexity, maintainability)
4. Issues and recommendations
5. Improvement suggestions

Focus on architectural aspects and high-level design.
Respond in JSON format matching CodeAnalysisSchema.`
  ...
  // Tool 2: Architectural Pattern Advisor
  private createArchitecturalPatternAdvisorTool(): AgentTool {
    return {
      name: 'architectural_pattern_advisor',
      description: 'Recommend architectural patterns based on requirements and context',
      parameters: {
        type: 'object',
        properties: {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Recommend architectural patterns for:
...
Respond in JSON format.`
          ...
          const recommendations = patterns.map((p: any) => ({
            pattern: p.name || 'Unknown Pattern',
          ...
          return {
            ...
            data: {
              recommendations,
              reasoning: response.content.reasoning || 'Based on requirements and constraints analysis'
            },
            metadata: {
          ...
          return {
            success: false,
            error: `Failed to advise on patterns: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  // Tool 3: Dependency Analyzer
  private createDependencyAnalyzerTool(): AgentTool {
    return {
      name: 'dependency_analyzer',
      description: 'Analyze project dependencies for security, compatibility, and optimization',
      parameters: {
        type: 'object',
        properties: {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { projectPath, checkType = 'all', includeTransitive = true } = params

          const prompt = `Analyze dependencies for project at ${projectPath}:
...
Respond in JSON format.`
  ...
  private createPerformanceAnalyzerTool(): AgentTool {
    return {
      ...
      description: 'Analyze architecture for performance bottlenecks and optimization opportunities',
      parameters: {
        type: 'object',
        properties: {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { architecture, metrics = {}, targetScale = 'medium' } = params

          const prompt = `Analyze architecture for performance:
...
6. Load distribution improvements
...
Respond in JSON format.`
  ...
  // Tool 5: Technology Stack Advisor
  private createTechnologyStackAdvisorTool(): AgentTool {
    return {
      name: 'technology_stack_advisor',
      description: 'Recommend optimal technology stack based on project requirements',
      parameters: {
        type: 'object',
        properties: {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          ...
          const prompt = `Recommend technology stack for:
...
Respond in JSON format.`
          ...
          const stack: z.infer<typeof TechnologyStackSchema> = {
            ...
            reasoning: response.content.reasoning || 'Based on project requirements and constraints'
          }
          ...
          return {
            success: false,
            error: `Failed to advise on technology stack: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  // Tool 6: Security Architecture Assessor
  private createSecurityArchitectureAssessorTool(): AgentTool {
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { architecture, securityRequirements = [], threatModel = {} } = params

          const prompt = `Assess security architecture:
Architecture: ${JSON.stringify(architecture)}
...
Respond in JSON format.`
          ...
          return {
            success: false,
            error: `Failed to assess security: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  // Tool 7: Architecture Documentation Generator
  private createArchitectureDocumentationGeneratorTool(): AgentTool {
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { systemName, architecture, format = 'full', audience = 'technical' } = params

          const prompt = `Generate architecture documentation for ${systemName}:
Architecture: ${JSON.stringify(architecture)}
Format: ${format}
Audience: ${audience}
...
Make it clear, comprehensive, and appropriate for the audience.
Respond in JSON format with structured documentation.`
  ...
  // Tool 8: System Design Validator
  private createSystemDesignValidatorTool(): AgentTool {
    return {
      name: 'system_design_validator',
      description: 'Validate system design against best practices and requirements',
      parameters: {
        type: 'object',
        properties: {
          design: { type: 'object', description: 'System design to validate' },
          requirements: { type: 'array', description: 'System requirements' },
          constraints: { type: 'array', description: 'Design constraints' },
          validationFocus: { type: 'array', description: 'Areas to focus validation on' }
        },
        required: ['design']
      },
      execute: async (params: any): Promise<ToolExecutionResult> => {
        try {
          const { design, requirements = [], constraints = [], validationFocus = [] } = params

          const prompt = `Validate system design:
...
Provide validation score and specific issues/recommendations.
Respond in JSON format.`
          ...
          const validation = {
            overallScorer: esponse.content.overallScore || 0,
            requirementsCoverager: esponse.content.requirementsCoverage || {},
          ...
          return {
            success: false,
            error: `Failed to validate design: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  protected async executeTraditionalProcessing(query: string, context: any): Promise<any> {
    ...
    if (keywords.includes('pattern')) {
      return {
        response: 'Common architectural patterns include: MVC, Microservices, Event-Driven, Layered, and Hexagonal architectures.',
        method: 'traditional',
        context
      }
    } else if (keywords.includes('technology') || keywords.includes('stack')) {
      return {
        response: 'Technology selection depends on requirements, team expertise, and constraints. Consider factors like scalability, maintainability, and ecosystem support.',
        method: 'traditional',
        context
      }
    } else if (keywords.includes('security')) {
      return {
        response: 'Security architecture should include: authentication, authorization, encryption, input validation, and security monitoring.',
        method: 'traditional',
        context
      }
    }
