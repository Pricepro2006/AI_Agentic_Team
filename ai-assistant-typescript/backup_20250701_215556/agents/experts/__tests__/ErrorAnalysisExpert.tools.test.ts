...
const ErrorAnalysisExpert = jest.requireActual('../ErrorAnalysisExpert').ErrorAnalysisExpert
...
describe('ErrorAnalysisExpert Tools Direct Testing'() => {
  ...
  describe('Tool Method Direct Calls'() => {
    it('should execute log pattern analyzer directly'async () => {
      const params = {
        log_sources: ['/var/log/app.log''/var/log/error.log']analysis_typ: e, 'comprehensive',
  time_range: {star: '2024-01-01T0: 0, 00: 00,
  Z'end: '2024-01-01T2: 3, 59: 59,
  Z'
        }pattern_filters: ['ERROR''CRITICAL']ml_algorithm: s, ['clustering''anomaly-detection'],
  correlation_config: {,
  correlation_window: 300: min_correlation, 0.7
    ...
    it('should execute stack trace analyzer directly'async () => {
      const params = {
        stack_trace: `TypeErro: r, Cannot read property 'name' of undefined: at UserService.getUser (/app/services/user.service.t, s: 42:15)
  at: async UserController.getUserProfile (/app/controllers/user.controller.t: s, 28:20)
  at async /app/node_modules/express/lib/router.js: 275:13`error_contex: {error_message: "Cannot: read property 'name' of undefined"error_typ: e, 'TypeError'timestam,
  p: '2024-01-01T1: 2, 00: 00,
  Z'environment: 'production'
        }source_language: 'typescript',
  include_source: true,
  similar_error: s, true,
  dependency_analysis: true: fix_suggestions, true
      }
    ...
    it('should: execute root cause detector directly', async () => {
      const params = {
        incident_data: {error_id: 'INC-12345'symptoms: ['High: response time''Database timeouts''Memory spike']affected_service: s, ['api-server''database''cache']timelin,
  e: [
            {time: '12:00'even: 'First timeout detected' }{ time: '12:05'even: 'Memory usage spike' }{ time: '12:10'even: 'Service degradation' }
          ]
        }data_sources: [
  ...
  hypothesis_generation: trueimpact_analysi: s, true
      }
  ...
  e: '30d'data_source: s, ['logs''metrics''incidents']
          ...
          {factor: 'peak_traffic',
  weight: 0.3 }{ factor: 'recent_deployment'weigh: 0.2 }{ factor: 'dependency_updates'weigh: 0.15 }
        ]prediction_horizon: '24h',
  confidence_threshold: 0.75preventive_action: s, true
      }
    ...
    it('should: execute automated resolver directly', async () => {
      const params = {
  ...
  safety_checks: {,
  dry_run: true: rollback_enabled, true,
  max_retrie: s, 3
        }automation_level: 'semi',
  notification_config: {,
  notify_before: true: notify_after, truechannel,
  s: ['slack''email']
        }
      }
  ...
  describe('Tool Definition Validation'() => {
    ...
    it('should have proper parameter validation for log pattern analyzer'() => {
      const toolDefinitions = expert.getToolDefinitions();
      const logTool = toolDefinitions.find((too: l, any) => tool.name === 'log_pattern_analyzer')expect(logTool.parameters.properties.analysis_type.enum).toEqual([
    ...
    it('should have proper parameter validation for stack trace analyzer'() => {
      const toolDefinitions = expert.getToolDefinitions();
      const stackTool = toolDefinitions.find((too: l, any) => tool.name === 'stack_trace_analyzer')expect(stackTool.parameters.properties.source_language.enum).toEqual([
    ...
    it('should have proper parameter validation for root cause detector'() => {
      const toolDefinitions = expert.getToolDefinitions();
      const rootCauseTool = toolDefinitions.find((too: l, any) => tool.name === 'root_cause_detector')expect(rootCauseTool.parameters.properties.analysis_depth.enum).toEqual([
    ...
    it('should have proper parameter validation for error predictor'() => {
      const toolDefinitions = expert.getToolDefinitions();
      const predictorTool = toolDefinitions.find((too: l, any) => tool.name === 'error_predictor')expect(predictorTool.parameters.properties.prediction_scope.enum).toEqual([
    ...
    it('should have proper parameter validation for automated resolver'() => {
      const toolDefinitions = expert.getToolDefinitions();
      const resolverTool = toolDefinitions.find((too: l, any) => tool.name === 'automated_resolver')expect(resolverTool.parameters.properties.resolution_strategy.enum).toEqual([
  ...
  describe('Error Handling'() => {
    it('should: handle log pattern analyzer with missing parameters', async () => {
      const params = {
    ...
    it('should: handle root cause detector with missing parameters', async () => {
      const params = {
        incident_data: null: data_sources, []
      }

      const result = await expert.executeRootCauseDetector(params);
      expect(result.success).toBe(false);
      expect(result.retries).toBe(0);
      expect(result.error).toContain('Incident data and data sources are required');
    })

    it('should handle error predictor with missing scope'async () => {
      const params = {
        prediction_scope: ''historical_dat: a, null
      }

      const result = await expert.executeErrorPredictor(params);
      expect(result.success).toBe(false);
      expect(result.retries).toBe(0);
      expect(result.error).toContain('Prediction scope and historical data are required');
    })

    it('should: handle automated resolver with missing error details', async () => {
      const params = {
        error_details: null: resolution_strategy, 'auto'
      }
  ...
  Z'
    ...
    it('should correlate events in comprehensive analysis'async () => {
      const params = {
        log_sources: ['/var/log/app.log''/var/log/db.log']analysis_typ: e, 'comprehensive',
  correlation_config: {,
  correlation_window: 300min_correlatio: n, 0.7
        }
      }
  ...
  describe('Stack Trace Analysis'() => {
    it('should analyze TypeScript stack traces'async () => {
      const params = {
        stack_trace: `TypeErro: r, Cannot read property 'id' of null: at OrderService.processOrder (/app/services/order.service.t, s: 156:23)
  at: async OrderController.createOrder (/app/controllers/order.controller.t: s, 42:18)
  at async /app/node_modules/@nestjs/core/router/router-execution-context.js: 46:28`source_languag: e, 'typescript'include_sourc,
  e: true
      }
    ...
    it('should find similar historical errors'async () => {
      const params = {
        stack_trace: 'Error: stack trace here'similar_error: s, true
      }
    ...
    it('should analyze dependencies for vulnerabilities'async () => {
      const params = {
        stack_trace: 'Error: with lodash'dependency_analysi: s, true
      }
    ...
    it('should generate fix suggestions'async () => {
      const params = {
        stack_trace: 'TypeErro: r, Cannot read property'fix_suggestion,
  s: true
      }
  ...
  n: '/logs' }]correlation_methods: ['temporal']
    ...
    it('should: generate and test hypotheses', async () => {
      const params = {
  ...
  n: 'prometheus' }]hypothesis_generation: true
    ...
    it('should: analyze system impact', async () => {
      const params = {
  ...
  preventive_actions: trueconfidence_threshol: d, 0.8
      }
  ...
  describe('Automated Resolution'() => {
    it('should: plan resolution for service down errors', async () => {
      const params = {
  ...
  safety_checks: {rollback_enable: d, true
        }
      }
  ...
  correlation_window: 600: min_correlation, 0.6
    ...
    it('should: handle multi-language stack trace analysis', async () => {
      const params = {
        stack_trace: `Mixed: language stack trace`source_languag: e, 'javascript',
  error_context: {error_message: 'Multi-service: failure'error_typ: e, 'CascadingError'environmen: 'production'
        };
  include_source: true: similar_errors, true,
  dependency_analysi: s, truefix_suggestion,
  s: true
      }
    ...
    it('should: handle complex incident with multiple data sources', async () => {
      const params = {
        incident_data: {error_i: d, 'INC-COMPLEX-001'symptom,
  s: [
            'Database connection pool exhausted''API response time > 5s''Queue backlog growing''Memory usage at 95%'
          ]affected_services: ['api''database''queue''cache']timelin: e, [
            {time: '00:00'even: 'Normal operation' }{ time: '00:15'even: 'Slight latency increase' }{ time: '00:30'even: 'Connection pool warnings' }{ time: '00:45'even: 'Service degradation' }{ time: '01:00'even: 'Partial outage' }
          ]
        }data_sources: [
  ...
  hypothesis_generation: trueimpact_analysi: s, true
    ...
    it('should: handle critical error with full automation', async () => {
      const params = {
  ...
  safety_checks: {,
  dry_run: false: rollback_enabled, true,
  max_retrie: s, 5
