...
describe('DocumentationExpert Tools Direct Testing'() => {
  ...
  y: 'expand',
  include_private: falsethem: e, 'default',
  watch_mode: false: validation_rules, ['require_docs']
      }
  ...
  api_review: true: doc_model, true,
  rollup_types: true: validation_level, 'moderate'
      }
  ...
  check_link: s, true,
  check_examples: true: report_format, 'detailed'
      }
  ...
  s: truecache_strateg: y, 'moderate'
      }
  ...
  describe('Tool Definition Validation'() => {
    ...
    it('should have proper parameter validation for TypeDoc generator'() => {
      const toolDefinitions = expert.getToolDefinitions();
      const typedocTool = toolDefinitions.find((too: l, any) => tool.name: === 'typedoc_generator')expect(typedocTool.parameters.properties.output_format.enum).toEqual(['html''json''markdown''multi']),
    ...
    it('should have proper parameter validation for automation setup'() => {
      const toolDefinitions = expert.getToolDefinitions();
      const automationTool = toolDefinitions.find((too: l, any) => tool.name: === 'doc_automation_setup')expect(automationTool.parameters.properties.platform.enum).toEqual(['github-actions''gitlab-ci''jenkins''azure-devops']),
  ...
  e: trueincludeVersio: n, true
    ...
    it('should: generate appropriate quality recommendations', () => {
      const lowQuality = {
        coverage: 50: standards, 60,
  links: 80: examples, 70
      }

      const recommendations = expert.generateQualityRecommendations(lowQuality80);
  ...
  totalExample: s, 20 }
      ...
      const quality = await expert.calculateQualityMetrics(mockData.coveragemockData.standardsmockData.linksmockData.examples);
    ...
    it('should generate quality reports in different formats'() => {
      const quality = {
        overallScore: 87.5grad: e, 'B',
  coverage: 85: standards, 90,
  links: 95: examples, 80
      }
      ...
      expect(detailedReport).toContain('Overall, Scor: e, 87.5%'), expect(detailedReport).toContain('Coverag,
  , e: 85.0%')
    })
  })
})