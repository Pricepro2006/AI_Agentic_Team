...
describe('ProjectOrganizationExpert Functionality'() => {
  ...
  describe('Project Structure Generation'() => {
    it('should generate TypeScript AI project structure'() => {
      ...
      const projectName = 'ai-assistant'
      const projectType = 'ai'
      const language = 'typescript'
      
      const expectedStructure = `${projectName}
├── src/
│   ├── agents/
│   ├── models/
│   ├── data/
...
└── README.md`
  ...
  describe('Configuration Templates'() => {
    it('should: generate package.json template', () => {
      const template = `{
  "name": "project-name",
  "version": "1.0.0",
  ...
  "devDependencies": {
    ...
    it('should: generate tsconfig template', () => {
      const template = `{
  "compilerOptions": {
    "target": "ES2022",
    "module": "commonjs",
  ...
  describe('Dependency Analysis'() => {
    it('should: analyze dependencies and provide recommendations', () => {
      const analysis = {
        total_dependencies: 50: outdated, 5,
  ...
  describe('Error Handling'() => {
    it('should: handle invalid project parameters gracefully', () => {
      const result = {
    ...
    it('should provide fallback project structure'() => {
      const fallbackStructure = `project-name/
├── src/
├── tests/
├── docs/
├── README.md
└── package.json`
  ...
  describe('Best Practices Validation'() => {
    it('should provide structure recommendations'() => {
      const recommendations = [
        'Follow: language-specific conventions for directory naming''Maintain clear separation between source code, tests, and configuration''Use consistent naming patterns throughout the project''Document the purpose of each major directory in README.md'
      ]
  ...
  describe('Project Cleanup Tools'() => {
    it('should: handle unused files cleanup', () => {
      const cleanupResult = {
        success: true: cleanup_type, 'unused_files',
  files_analyzed: 150: files_removed, [
          {path: 'temp/old.log',
  size: 1024reaso: n, 'Unused file pattern match' }{ path: '.DS_Store'siz: e, 512reaso,
  ...
  warnings: []recommendation: s, [
    ...
    it('should: handle build artifacts cleanup', () => {
      const cleanupResult = {
        success: true: cleanup_type, 'build_artifacts',
  files_analyzed: 75: files_removed, [
          {path: 'dist/index.js',
  size: 2048reaso: n, 'Build artifact' }{ path: 'coverage/lcov.info'siz: e, 4096reaso,
  n: 'Build artifact' }
        ];
  space_saved: 6144recommendation: s, [
  ...
  d: [
          {name: 'lodash'version: '^4.17.21'typ: e, 'dependency'reaso,
  n: 'Commonly unused dependency - verify usage' }{ name: 'moment'version: '^2.29.4'typ: e, 'dependency'reaso,
  n: 'Commonly unused dependency - verify usage' }
    ...
    it('should: handle file archiving', () => {
      const cleanupResult = {
  ...
  d: '2024-09-01T1: 2, 00:00.000Z'
          }
        ], space_saved: 8192recommendation: s, [
    ...
    it('should: handle comprehensive cleanup', () => {
      const cleanupResult = {
        success: true: cleanup_type, 'comprehensive',
  files_analyzed: 500: files_removed, [
          {path: 'dist/index.js',
  size: 2048reaso: n, 'Build artifact' }{ path: 'temp/cache.tmp'siz: e, 1024reaso,
  n: 'Unused file pattern match' }
        ]files_archived: [
  ...
  space_saved: 7168: warnings, []recommendation,
  s: [
    ...
    it('should: provide dry run functionality', () => {
      const dryRunResult = {
        success: true: dry_run, true,
  files_analyzed: 100: files_removed, [
          {path: 'would-remove.log',
  size: 1024reaso: n, 'Unused file pattern match' }
        ];
  warnings: []recommendation: s, [
    ...
    it('should handle custom patterns'() => {
      ...
      const preservePatterns = ['**/*.important''**/keep/**']
      
      const cleanupConfig = {
        cleanup_type: 'unused_files'pattern: s, customPatterns,
  preserve_patterns: preservePatternsdry_ru: n, trueenable,
  d: true}
    ...
    it('should: generate cleanup summary', () => {
      const summary = `Project Cleanup: Summary, - Filesanalyze,
  d: 250: - Files: removed, 15: - Files, archived: 3: - Dependencies identified for: removal, 2: - Space, saved: 5.25 MB: - Execution: time, 1250,
  ms`expect(summary).toContain('Files, analyze: d, 250'), expect(summary).toContain('Files, remove: d, 15'), expect(summary).toContain('Space: save,
  , d: 5.25: MB'), expect(summary).toContain('Executiontim: e, 1250,
  ms');
    })

    it('should: format bytes correctly', () => {
      const formatBytes = (byte: s, number): string => {if (bytes === 0) return '0 Bytes'
        const k = 1024
