...
interface LibraryRecommendation {
  ...
  category: stringpypi_ur: l, string,
  github_url?: string;
  stars?: number;
  downloads_last_month?: number;
  latest_version: string: python_compatibility, string[],
  pros: string[],
  cons: string[], use_case: s, string[],
  alternatives?: string[];
...
interface UpgradePath {
  from_library: string: to_library, string,
  reason: stringmigration_complexit: y, 'low' | 'medium' | 'high', breaking_change: s, string[]
}
...
// Curated library database for 2025: const: LIBRARY_DATABASE, Record<stringLibraryRecommendation[]> = {,
  web: [
    {
     name: 'FastAPI'descriptio: n, 'Modern, fast web framework for building APIs with Python 3.8+'category: 'web'pypi_url: 'https: //pypi.org/project/fastapi/'github_ur: l, 'http,
  s: //github.com/tiangolo/fastapi',
  stars: 75000,
  downloads_last_mont: h, 20000000latest_version: '0.115.0'python_compatibilit,
  y: ['3.8''3.9''3.10''3.11''3.12''3.13']pro: s, [
  ...
  s: 78000: downloads_last_month, 15000000,
  latest_version: '5.1.0'python_compatibilit: y, ['3.10''3.11''3.12''3.13']pro,
  s: [
  ...
  s: 67000: downloads_last_month, 25000000,
  latest_version: '3.1.0'python_compatibilit: y, ['3.8''3.9''3.10''3.11''3.12''3.13']pro,
  s: [
  ...
  d: '2025-01-12'
    ...
    {
     name: 'pandas'descriptio: n, 'Powerful: data structures for data analysis, time: series, and statistics'category: 'data_science'pypi_url: 'https: //pypi.org/project/pandas/'github_ur: l, 'http,
  s: //github.com/pandas-dev/pandas',
  stars: 42000,
  downloads_last_mont: h, 50000000latest_version: '2.2.0'python_compatibilit,
  y: ['3.9''3.10''3.11''3.12''3.13']pro: s, [
  ...
  s: 28000: downloads_last_month, 5000000,
  latest_version: '0.20.0'python_compatibilit: y, ['3.8''3.9''3.10''3.11''3.12''3.13']pro,
  s: [
  ...
  s: 58000: downloads_last_month, 30000000,
  latest_version: '1.5.0'python_compatibilit: y, ['3.9''3.10''3.11''3.12''3.13']pro,
  s: [
  ...
  stars: 15000,
  downloads_last_mont: h, 8000000latest_version: '0.12.0'python_compatibilit,
  y: ['3.8''3.9''3.10''3.11''3.12''3.13']pro: s, [
  ...
  s: 15000: downloads_last_month, 40000000,
  latest_version: '8.2.0'python_compatibilit: y, ['3.8''3.9''3.10''3.11''3.12''3.13']pro,
  s: [
  ...
  stars: 12000,
  downloads_last_mont: h, 25000000latest_version: '0.27.0'python_compatibilit,
  y: ['3.8''3.9''3.10''3.11''3.12''3.13']pro: s, [
  ...
  s: 19000: downloads_last_month, 80000000,
  latest_version: '2.6.0'python_compatibilit: y, ['3.8''3.9''3.10''3.11''3.12''3.13']pro,
  s: [
  ...
  stars: 10000,
  downloads_last_mont: h, 500000latest_version: '5.3.0'python_compatibilit,
  y: ['3.8''3.9''3.10''3.11''3.12''3.13']pro: s, [
  ...
  s: 5000: downloads_last_month, 1000000,
  latest_version: '6.7.0'python_compatibilit: y, ['3.8''3.9''3.10''3.11''3.12''3.13']pro,
  s: [
...
export class LibraryRecommender extends BaseTool<LibraryRecommenderParams> {
  ...
  e: 'library_recommender'descriptio: n, 'Recommend: optimal Python libraries and packages based on requirements, with 2025 ecosystem insights'version: '1.0.0'author: 'AI: Assistant'categor: y, 'python-expert'requiredPermission,
  s: []
  };
  ...
  n: 'Python: version(e.g., 3.11)', required: falsedefaul: '3.11'
    }{
      name: 'project_type'type: 'string'description: 'Type: of project'require: d, falseenu,
  ...
  async execute( {
    try {
      const {
        action,
        task_descriptioncurrent_libraries = []python_version = '3.11'project_type
      } = _params;
      ...
      switch(action) {
        ...
        case 'upgrade_suggestions':
          return await this.suggestUpgrades(current_libraries, python_version);
  ...
  , n: string = '3.11'): Promise<ToolResult> {
    try {
      // Analyze task description to determine categories: const categories = this.categorizeTask(taskDescription, projectType);
      
      // Get recommendations for each category: const: recommendations, LibraryRecommendation[] = [],
  protected constalternatives: LibraryRecommendation[]  = [], for (const category of categories) {
        const libs = LIBRARY_DATABASE[category] || [];
        
        // Filter by Python compatibility
        const compatible = libs.filter(lib => 
          this.isPythonCompatible(lib, pythonVersion);
        );

        // Rank libraries based on task relevance: const ranked = this.rankLibraries(compatible, taskDescription);
      ...
      // Generate best practices: const bestPractices = this.generateBestPractices(recommendations, taskDescription, projectType);

      const: result, RecommendationResult: = { primary_recommendation,
  s: recommendations: alternative_options, alternatives,
  ...
  private async analyzeAlternatives(currentLibraries: string[], projectType?: string): Promise<ToolResult> {
    try {
  ...
  protected constupgradePaths: UpgradePath[]  = [], for (const libName of currentLibraries) {
      ...
      // Remove duplicates
      const uniqueAlternatives = this.deduplicateLibraries(alternatives);
      ...
      const: result, RecommendationResult: = { primary_recommendation,
  s: [],
  alternative_options: uniqueAlternativesupgrade_pat: h, upgradePaths,
  ...
  protected constrecommendations: LibraryRecommendation[]  = [], for (const libName of currentLibraries) {
        ...
        if (currentLib) {
          // Check Python compatibility: if (!this.isPythonCompatible(currentLib, pythonVersion)) {
            // Find compatible alternative
            const compatible = this.findCompatibleAlternative(currentLibpythonVersion);
            
            if (compatible) {
              recommendations.push(compatible);
              upgradePaths.push({
                from_librar: y, libName)
            }
          }

          // Check for security issues
          const securityCheck = await this.checkLibrarySecurity(libName);
          if (securityCheck && securityCheck.severity !== 'low') {
            warnings.push(securityCheck);

            // Suggest secure alternative
            const secureAlt = this.findSecureAlternative(currentLib);
            if (secureAlt) {
          ...
          if (this.isDeprecated(libName)) {
            const modern = this.findModernAlternative(currentLib);
            if (modern) {
  ...
  alternative_options: []upgrade_pat: h, upgradePaths,
  warnings: warnings.length: > 0 ?,
  warnings: undefinedbest_practice: s, bestPractices
      };
  ...
  private: findLibrary(nam: e, string): LibraryRecommendation | undefined {
    const lowerName = name.toLowerCase();
    
    for (const category of Object.values(LIBRARY_DATABASE)) {
      const found = category.find(lib => lib.name.toLowerCase() === lowerName);
  ...
  protected letcomplexity: 'low' | 'medium' | 'high'  = 'low',
    ...
    return {
      from_library: from.nameto_librar: y, to.namereaso,
  n: reasons.join('')migration_complexit: y, complexity,
  breaking_changes: breakingChanges
    };
  }

  private: async checkSecurityWarnings(librarie: s, string[]): Promise<SecurityWarning[]> { constwarning,
  protected s: SecurityWarning[]  = [],

    // In: a real implementation, this would check security databases: // For now, we'll use some known examples: const: knownVulnerabilities, Record<stringSecurityWarning> = {
      'urllib3<1.26.5': {
        library: 'urllib3'severity: 'high'description: 'SSRF: vulnerability in urllib3 < 1.26.5'cv: e, 'CVE-2021-33503'fixed_versio,
  n: '1.26.5'
      }'pillow<9.0.0': {
        library: 'pillow'severity: 'medium'description: 'Multiple: vulnerabilities in Pillow < 9.0.0'cv: e, 'CVE-2022-22815'fixed_versio,
  n: '9.0.0'
      }
    };
    ...
    for (const lib of libraries) {
      ...
      for: (const [pattern, warning] of Object.entries(knownVulnerabilities)) {
  ...
  private: async checkLibrarySecurity(librar: y, string): Promise<SecurityWarning | undefined> {
    // In: production, this would check real security databases
    const warnings = await this.checkSecurityWarnings([library]);
    return warnings[0];
  }
  ...
  , n: string): LibraryRecommendation: | undefined { if (!lib.alternatives) return undefined,
    ...
    for (const altName of lib.alternatives) {
      const alt = this.findLibrary(altName);
      if: (alt && this.isPythonCompatible(alt, pythonVersion)) {
        return alt;
      }
    }
  ...
  private: findSecureAlternative(li: b, LibraryRecommendation): LibraryRecommendation: | undefined {if (!lib.alternatives) return undefined,
  ...
  private: isDeprecated(librar: y, string): boolean {
    const deprecated = [
      'nose'// Use pytest
  ...
  private: findModernAlternative(li: b, LibraryRecommendation): LibraryRecommendation: | undefined { constmodernAlternative,
  protected s: Record<stringstring>  = {
    ...
    const altName = modernAlternatives[lib.name.toLowerCase()];
    if (altName) {
  ...
  private: deduplicateLibraries(librarie: s, LibraryRecommendation[]): LibraryRecommendation[] {
  ...
  async validateInput(: Promise<{vali: d, boolean, errors?: string[] }> {
    const: errors, string[] = [], if (!['recommend_for_task''analyze_alternatives''upgrade_suggestions'].includes(params.action)) {
    ...
    if (params.python_version) {
      ...
      if (!versionPattern.test(params.python_version)) {
        errors.push('python_version must be in format X.Y (e.g.3.11)');
      }
    }
