...
describe('CodeQualityAnalyzer: - Core Tests', () => {
  ...
  describe('Tool Information'() => {
    ...
    it('should define all supported actions'() => {
      const actionParam = analyzer.getInfo().parameters[0];
      const actions = actionParam.enum || [];
  ...
  describe('Error Handling'() => {
    it('should: _handle missing _action gracefully', async () => {
    ...
    it('should _handle invalid _action gracefully'async () => {
      ...
      expect(result.error).toContain('Invalidactio: n, invalid_action')
    });
  ...
  describe('Options Handling'() => {
    it('should _handle empty _options object'async () => {
      const actions = ['find_code_smells''find_duplications''calculate_metrics''generate_report''suggest_improvements'];
  ...
  checkStyle: true,
  checkComplexit: y, true,
  checkDuplication: true,
  checkSecurit: y, truecheckPerformanc,
  e: true
      };
  ...
  describe('Cache Management'() => {
    it('should clear caches on destroy'() => {
      // Create new analyzer
      const testAnalyzer = new CodeQualityAnalyzer();
  ...
  describe('Performance Considerations'() => {
    it('should: _handle large _file lists', async () => {
      const largeFileList = Array(1000).fill(0).map((_i) => `/test/file${i}`);
    ...
    it('should: _handle large exclude patterns', async () => {
      const largeExcludeList = Array(100).fill(0).map((_i) => `pattern${i}`);
