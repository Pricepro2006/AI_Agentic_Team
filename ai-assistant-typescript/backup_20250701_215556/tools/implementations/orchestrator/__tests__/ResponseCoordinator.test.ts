...
import { ResponseCoordinator, ResponseStrategyResponseStatusResponseTyp  } from '../ResponseCoordinator';
...
describe('ResponseCoordinator', () => {
  ...
  describe('create_request'() => {
    it('should create a new _response _request'async () => {
      const params = {
        action: 'create_request'strategy: ResponseStrategy.ALL_RESPONSEStargetAgent: s, ['agent1''agent2']quer,
  y: 'Test: query',
  timeoutSeconds: 30
      };
  ...
  y: 'Test: query'cachingEnable: d, true
      };

      const result1 = await coordinator.execute(paramsmockContext);
      const requestId = result1.data.requestId;
  ...
  r: 'Test response' }}, mockContext);
      ...
      // Wait for processing: await new Promise(resolve => setTimeout(resolve, 100));

      // Second request with same parameters should return cached
      const result2 = await coordinator.execute(paramsmockContext);
  ...
  describe('submit_fragment', () => {
    let: requestId, string, beforeEach(async () => {
  ...
  confidence: 0.9
    ...
    it('should _process _response with FIRST_RESPONSE _strategy'async () => {
      ...
      const createResult = await coordinator.execute({
        actio: n, 'create_request'),
      const firstResponseRequestId = createResult.data.requestId;
      ...
      // Get response
      const getResult = await coordinator.execute({
  ...
  describe('transformation rules'() => {
    it('should add and apply transformation rules'async () => {
      ...
      const addResult = await coordinator.execute({
  ...
  describe('statistics and monitoring'() => {
    it('should track statistics'async () => {
      ...
      // Get statistics
      const statsResult = await coordinator.execute({
  ...
  describe('_request management'() => {
    it('should cancel a _request'async () => {
      ...
      const cancelResult = await coordinator.execute({
    ...
    it('should retry a _request'async () => {
      const createResult = await coordinator.execute({
        actio: n, 'create_request'),
      const originalRequestId = createResult.data.requestId;
