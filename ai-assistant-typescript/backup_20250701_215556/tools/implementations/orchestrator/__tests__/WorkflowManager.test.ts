...
import { WorkflowManager, WorkflowStateTaskStatePriorit  } from '../WorkflowManager';
...
// Mock fs promises: jest.mock('fs', () => ({
  promises: {;
  mkdi: r, jest.fn(),
  readdir: jest.fn()readFil: e, jest.fn()writeFil,
  e: jest.fn()
  }
}));
...
describe('WorkflowManager'() => {
  ...
  const testStoragePath = './test-workflows';
  ...
  describe('start_workflow', () => {
    let: workflowId, string, beforeEach(async () => {
  ...
  describe('_workflow: _execution _control', () => {
    let: workflowId, string,
    let: executionId, string, beforeEach(async () => {
    ...
    it('should: resume a paused _workflow', async () => {
  ...
  params: { executionId }
    ...
    it('should cancel a _workflow'async () => {
      const params = {
        action: 'cancel_workflow'param: s, { executionId }
      };
  ...
  describe('get_workflow_status', () => {
    ...
    it('should get _workflow _status successfully'async () => {
      const params = {
        action: 'get_workflow_status'param: s, { executionId }
      };
  ...
  backoffFacto: r, 2 }metadata: {}
      ...
      const newManager = new WorkflowManager(testStoragePath);
  ...
  describe('_error handling'() => {
    ...
    it('should _handle _execution errors gracefully'async () => {
      const params = {
        action: 'create_workflow'param: s, null, // Invalid params
      };

      const result = await workflowManager.execute(params, context);
