...
export const PerformanceTier = z.enum(['fast', 'balanced', 'quality'])
export const CostTier = z.enum(['low', 'medium', 'high'])
export const QualityTier = z.enum(['basic', 'good', 'excellent'])
export const ProviderType = z.enum(['local', 'api', 'hybrid'])
...
export interface MultiModelAgentConfig {
  modelSelection: {
    ...
    taskMapping: Record<string, string> // task type -> model id
    enabled: boolean
  }
  providers: string[]
  modelRouter: {
