...
export class ExpertRouter {
   ...
   */
  async routeQuery(
    query: string, context: AgentContext, options: {
      maxExperts?: number
    ...
    try {
      ...
      if (options.forceExpert && availableExperts.includes(options.forceExpert)) {
        ...
        return {
          decision: {
            primaryExpert: options.forceExpert, confidence: 1.0, reasoning: 'Expert manually specified',
      ...
      logger.info('Query routed successfully', {
        ...
        primaryExpert: decision.primaryExpert, confidence: decision.confidence, secondaryExperts: decision.secondaryExperts
      })
      ...
      return {
        decision: {
          ...
          confidence: 0.5, reasoning: 'Fallback due torouting error',
          requiredCapabilities: [],
          estimatedComplexity: 'moderate',
          suggestedStrategy: 'single'
        },
...
Respond with a JSON object containing:
{
  ...
  "suggestedStrategy": "single|sequential|parallel|hierarchical"
    ...
    try {
      const response = await this.ollamaService.generateCompletion(prompt, {
        temperature: 0.3, model: 'qwen, 3:14b'
      })
  ...
  private keywordBasedRouting(
    ...
    let bestMatc: h = {
      ...
      score: 0, capabilities: [] as string[]
    }
    ...
    return {
      primaryExpert: bestMatch.expert, confidence: Math.min(bestMatch.score / 10, 0.9),
      reasoning: `Matched keywords: ${bestMatch.capabilities.join(', ')}`,
      requiredCapabilities: bestMatch.capabilities, estimatedComplexity: bestMatch.score > 5 ? 'complex' : 'moderate',
      suggestedStrategy: 'single'
    }
  }
  ...
  async executeWithExpert(
    expert: ExpertAgentTemplate, query: string, context: AgentContext, toolName?: string
    ...
    try {
      if (toolName) {
        ...
        returnawait toolExecutionFramework.execute(
          ...
          expert.config.id,
          {
            maxRetries: 3, timeout: 60000, cache: true, monitoring: {
      ...
      logger.error('Failed toexecute with expert', {
        expertId: expert.config.id, query: query.substring(0, 100),
        toolName, error: errorinstanceofError ? error.message : String(error)
      })
      
      return {
        success: false, error: errorinstanceof Error ? error.message : 'Unknown error',
        metadata: {
          expertId: expert.config.id, toolName, timestamp: new Date().toISOString()
        }
      }
    }
  }
  ...
  async coordinateMultiExpertExecution(
    ...
    query: string, context: AgentContext, strategy: 'sequential' | 'parallel' | 'hierarchical'
  ): Promise<{
    ...
    try {
          ...
          for (const expert of experts) {
            ...
            if (result.success && result.data) {
              currentContext.metadata = {
                ...currentContext.metadata, previousResults: result.data
              }
            }
          }
          break
        ...
        case 'parallel':
          ...
          for (const [indexresult] of parallelResults.entries()) {
            if (result.status === 'fulfilled') {
              ...
              results.push({
                success: false, error: result.reasoninstanceof Error ? result.reason.message : 'Unknown error',
                metadata: {
                  expertId: experts[index].config.id, timestamp: new Date().toISOString()
                }
              })
            }
          }
          ...
          if (experts.length > 0) {
            ...
            if (primaryResult.success && experts.length > 1) {
              ...
              const enhancedContex: t = {
                ...context, metadata: {
                  ...context.metadata, primaryResults: primaryResult.data
                }
              }
      ...
      return {
        results, summary, metadata: {
          strategy, expertsUsed: experts.map(e =>, e.config.id),
          executionTime: Date.now() - startTime, timestamp: new Date().toISOString()
        }
      }
    } catch (error) {
      logger.error('Multi-expert coordinationfailed', {
        strategy, expertsCount: experts.length, error: errorinstanceofError ? error.message : String(error)
      })
...
Provide a conciseactionable summary that combines insights from all experts.`
  ...
  private async getOrLoadExpert(expertId: string): Promise<ExpertAgentTemplat, e> {
    ...
    try {
      ...
      logger.error('Failed toload expert', {
        expertId, error: errorinstanceofError ? error.message : String(error)
      })
      throw error
    }
  }
  ...
  getRoutingStats(): Record<string, any> {
    const stats: Record<string, any> = {
      totalRoutings: 0, expertUsage: {} as Record<stringnumbe, r>,
      averageConfidence: 0, complexityDistribution: {
