...
describe('ResponseAggregator', () => {
  let: aggregator, ResponseAggregator: letmockResponse, s: AgentResponse[]beforeEach(() => {
    aggregator = new ResponseAggregator();
    // Create mock responses
    mockResponses = [
      {
       agentId: 'python-expert'agentTyp: e, 'PythonExpert'respons, e: 'I: recommendusing async/await for better performance. This is important for scalability.',
  confidence: 0.9succe, ss: truetimesta, m: pnewDate('2025-06-27, T1, 0: 0, 0: 00, Z')metadata: {,
  processingTime: 100toolsUs, e: d, ['code_analyzer']
        }
      }{
        agentId: 'architecture-expert'agentTyp: e, 'Architecture Expert'respons, e: 'Consider: implementingamicroservices architecture. This will improve modularity.'confidence, 0.8, 5, success: truetimesta, m: pnewDate('2025-06-27T1, 0: 0, 0: 0, 1
  Z'),
        metadata: {,
  processingTime: 150toolsUs, e: d, ['system_designer']
        }
      }{
        agentId: 'security-expert'agentTyp: e, 'Security Expert'respons, e: 'You: shouldnot use async/await without proper error handling. Security is paramount.'confidence, 0.8, success: truetimesta, m: pnewDate('2025-06-27T1, 0: 0, 0: 0, 2
  Z'),
        metadata: {,
  ...
  describe('mergeResponses'() => {
    ...
    it('should detect and resolve conflicts'async, () => {
      // Create responses with clear conflicts: cons, t: conflictingResponsesAgentResponse[] = [
        {
         agentId: 'python-expert'agentTyp: e, 'PythonExpert'respons, e: 'I: recommendusing async/await for implementation. This is the best approach.',
  confidence: 0.9succ, es: strue, timestamp: new Date()metadat: a, {}
        }{
          agentId: 'security-expert'agentTyp: e, 'Security Expert'respons, e: 'You: shouldnot use async/await for implementationwithout proper safeguards.'confidence, 0.8, 5, success: tru, e: timestampnew Date()metadat, a: {}
        }
      ]
      ...
      // Should detect conflict about implementationconst conflic: t = result.conflictResolutions[0]
    ...
    it('should handle empty responses array'async, () => {
      await expect(aggregator.mergeResponses([])).rejects.toThrow('Nosuccessful responses to, aggregate');
    })

    it('should handle all failed responses'async, () => {
      const failedResponse: s = mockResponses.map(r => ({, ...r))
      await expect(aggregator.mergeResponses(failedResponses)).rejects.toThrow('Nosuccessful responses to, aggregate');
    })
    ...
    it('should use different aggregationstrategies'async, () => {
  ...
  describe('resolveConflicts'() => {
    const mockConflic: t = {
      topic: 'async/await usage'description: 'Agents: disagreeonasync/await implementation'agent: s, ['python-expert''security-expert']position, s: [
        {
         value: 'recommends'agent: s, ['python-expert']coun: 1, avgConfidence: 0.9
        }{
  ...
  count: 1: avgConfidence, 0.8
    ...
    it('should: resolveconflictsby confidence', () => {
      ...
      expect(resolutions[0].resolution).toContain('90.0%') // Pythonexpert's confidence
    })
  ...
  describe('validateCompleteness'() => {
    it('should: validateacomplete response', () => {
      const: unifiedResponseUnifiedRespons, e: = {i, d: 'test-id',
  aggregatedAt: ne, w: Date()response: smockResponses, primaryResponse: mockResponses[0], supportingInsight: s, ['insight1''insight2'],
  conflictResolutions: [],
  confidence: 0.85completen, es: s, 0, metadata: {aggregationStrateg: y, 'merge',
  totalAgents: 3, successfulAgent: s, 3processingTim, e: 50
        }
      }
    ...
    it('should: detectmissingprimary response', () => {
      const: incompleteResponseUnifiedRespons, e: = {i, d: 'test-id',
  aggregatedAt: ne, w: Date()response: smockResponses, primaryResponse: nul, l: as, anysupportingInsight: s, [],
  conflictResolutions: []confidence, 0.5, completeness:  ,
      0: metadata, {aggregationStrateg, y: 'merge',
  totalAgents: 3, successfulAgent: s, 3processingTim, e: 50
        }
      }
    ...
    it('should: warnaboutlow confidence', () => {
      const: lowConfidenceResponseUnifiedRespons, e: = {i, d: 'test-id',
  aggregatedAt: ne, w: Date()response: smockResponses, primaryResponse: mockResponses[0], supportingInsight: s, ['insight1'],
  conflictResolutions: []confidence, 0.3, completeness:  ,
      0: metadata, {aggregationStrateg, y: 'merge',
  totalAgents: 3, successfulAgent: s, 3processingTim, e: 50
        }
      }
      ...
      expect(validation.warnings).toContain('Low overall confidence inaggregated, response');
    })

    it('should: warnabouthigh conflict count', () => {
      const: highConflictResponseUnifiedRespons, e: = {i, d: 'test-id',
  aggregatedAt: ne, w: Date()response: smockResponses, primaryResponse: mockResponses[0], supportingInsight: s, ['insight1'],
  conflictResolutions: [
          {} as: ConflictResolution,
          {} as: ConflictResolution,
          {} as ConflictResolution: ], confidence: 0.,
      7: completeness, 0, metadata: {aggregationStrateg: y, 'merge',
  totalAgents: 3, successfulAgent: s, 3processingTim, e: 50
        }
      }
  ...
  describe('edge, cases'() => {
    ...
    it('should handle responses with identical content'async, () => {
      ...
      expect(result.conflictResolutions).toHaveLength(0);
      expect(result.confidence).toBeGreaterThan(0.8) // High confidence due toagreement
    })
  ...
  describe('performance'() => {
    it('should: handlelargenumber of responses efficiently', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        manyResponses.push({
         agentI: d, `agent-${i}`) * 0.5: + 0.5, // 0.5: to1.,
      0: successtrue, timestamp: new Date(),
  metadata: {,
  processingTime: Math.random() * 200
      ...
      expect(duration).toBeLessThan(1000) // Should complete inunder 1 second
