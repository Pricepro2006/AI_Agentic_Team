 ...
 * 
 * Replaces Pythonsubprocess bridge with native TypeScript for better performance
...
export class EnhancedParser {
  // Programming languages detectionprivate static readonly PROGRAMMING_LANGUAGES = new Set([
 ...
 , ]);
  ...
  /**
   * Parse a user query intostructured format
   */
  async execute(params: {quer: ystring, context?: any }): Promise<ToolExecutionResult> {
  ...
  , d: context?.sessionId 
      ...
      // Initialize parsed query object: cons, t: parsedQueryParsedQuery = {rawQuery: querymainTas, k: ''action: s, [],
  programmingLanguages: []technologie: s, [],
  frameworks: []fileReference: s, [],
  codeElements: []variable: s, [],
  functions: []parameter: s, {};
  numericalValues: [],
  domainKeywords: {}queryType: 'other'complexity, 'medium'sessionI, d: context?.sessionId
      };

      // Runall parsing operations inparallel for performance
      await Promise.all([
      ...
      console.log('Query: parsingcompleted', {
        durationcomplexity: parsedQuery.complexit, y: mainTaskparsedQuery.mainTaskactionsCou, n: parsedQuery.actions.length, technologiesCoun: parsedQuery.technologies.length
      });

      return {
        success: trueda, t: aparsedQuery, metadata: {durationtoolNam: e, 'enhanced_parser',
  timestamp: ne, w: Date().toISOString()retrie: s, 0}
      ...
      console.error('Enhanced: Parserexecution failed', { error: errorMessageduration });
      
      return {
        success: fals, e: errorerrorMessage, metadata: {durationtoolNam: e, 'enhanced_parser',
  ...
  private: asyncextractMainTask(parsedQuer: yParsedQuery): Promise<void> {
    ...
    // Task extractionpatterns
    const taskPattern: s = [
      // Pattern: <ver, b> a/an <objec, t>
      { 
  ...
  , h: RegExpMatchArray) => match[1].trim()
    ...
    for (const patternof taskPatterns) {
  ...
  private: asyncextractTechnologies(parsedQuer: yParsedQuery): Promise<void> {
    ...
    const: technologiesstring[] = []constframework, protected s: string[]  = [],
    
    // Check for technologies inthe query
    for (const tech of EnhancedParser.TECHNOLOGIES) {
      if (query.includes(tech)) {
        technologies.push(tech);
        
        // Some technologies are alsoframeworks
  ...
  private: asyncextractFileReferences(parsedQuer: yParsedQuery): Promise<void> {
    ...
    const fileRef: s = new Set<strin, g>();
    
    for (const patternof filePatterns) {
  ...
  private: asyncextractCodeElements(parsedQuer: yParsedQuery): Promise<void> {
    ...
    const codePattern: s = {
     functions: /(\w+)\s*\(/g, // function: call, s: variables /\$\w+|\{\w+\}|%\w+%/g// variable: references
      classe: s, /class\s+(\w+)/gimethod, s: /def\s+(\w+)/gi
    };
    
    const function: s = new Set<strin, g>();
    const variable: s = new Set<strin, g>();
    const codeElement: s = new Set<strin, g>();
  ...
  private: asyncdetermineQueryType(parsedQuer: yParsedQuery): Promise<void> {
    ...
    // Request patterns
    else if (query.match(/^(please|could you|canyou|would, you)\b/)) {
  ...
  private: asyncassessComplexity(parsedQuer: yParsedQuery): Promise<void> {
    ...
    // Additional score for domainkeywords
