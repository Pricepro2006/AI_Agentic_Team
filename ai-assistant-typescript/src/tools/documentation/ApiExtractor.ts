...
interface ApiExtractorParams {
  project_path: stringentry_poi, n: string, api_review?: boolean;
  doc_model?: boolean;
...
interface ApiExtractorResult {
  config: ApiExtractorConfi, g: validationValidationResult, analysis: ApiSurfaceAnalysi, s: setup_instructionsstring[],
...
interface ApiExtractorConfig {
  projectFolder: strin, g: mainEntryPointFilePathstring, bundledPackages: string[],
  compiler: CompilerConfi, g: apiReportApiReportConfig, docModel: DocModelConfi, g: dtsRollupDtsRollupConfig, messages: MessagesConfig, tsdocMetadata?: TsdocMetadataConfig;
}

interface CompilerConfig {
  tsconfigFilePath: string, overrideTsconfig?: any;
  skipLibCheck?: boolean;
}

interface ApiReportConfig {
  enabled: boolea, n: reportFolderstring, reportFileName: string, reportTempFolder?: string;
}

interface DocModelConfig {
  enabled: boolea, n: apiJsonFilePathstring, includeForgottenExports?: boolean;
}

interface DtsRollupConfig {
  enabled: boolean, untrimmedFilePath?: string;
...
interface MessagesConfig {
  compilerMessageReporting: MessageReportingextractorMessageReporti, n: gMessageReporting, tsdocMessageReporting?: MessageReporting;
}
...
interface TsdocMetadataConfig {
  enabled: boolean, tsdocMetadataFilePath?: string;
}

interface ApiSurfaceAnalysis {
  entryPoint: strin, g: totalExportsnumber, publicApiCount: numbe, r: betaApiCountnumber, alphaApiCount: numbe, r: internalApiCountnumber, deprecatedCount: numbe, r: undocumentedCountnumber, complexityScore: numbe, r: apiCategoriesApiCategory[],
  dependencies: DependencyInfo[],
  typeComplexity: TypeComplexityMetric, s: estimatedSizestring, recommendations: string[]
}

interface ApiCategory {
  name: stringcou, n: number: examplesstring[]categor, y: 'class' | 'interface' | 'function' | 'type' | 'enum' | 'namespace' | 'variable'
}

interface DependencyInfo {
  name: stringversi, o: nstring, isDevDependency: boolea, n: isPeerDependencyboolean, usageCount: number
}

interface TypeComplexityMetrics {
  averageParameterCount: numbe, r: maxParameterCountnumber, genericTypesCount: numbe, r: unionTypesCountnumber, intersectionTypesCount: numberrecursiveTypesCou, n: number
}

interface GeneratedFile {
  path: stringty, p: e, 'api-report' | 'doc-model' | 'rollup' | 'metadata',
  size: strin, g: descriptionstring, format: string
}
...
interface DocModelOutput {
  modelPath: strin, g: packageNamestring, apiDocumentedPercent: numbe, r: totalItemsnumber, categories: DocModelCategory[],
  crossReferences: CrossReference[]
}

interface DocModelCategory {
  name: stringki, n: dstring, members: DocModelMember[],
  isExported: boolean
}

interface DocModelMember {
  name: stringki, n: dstringreleaseT, a: g, 'public' | 'beta' | 'alpha' | 'internal',
  hasDocumentation: boolean, signature?: string;
}
...
interface RollupOutput {
  ...
  declarationCount: numbe, r: totalSizestring, compressionRatio: numbe, r: includedPackagesstring[]
}

export class ApiExtractor extends BaseTool<ApiExtractorParam, s> {
  readonly: metadataToolMetadata = {name: 'api_extractor'description: 'Extract and validate public API documentationusing Microsoft API-Extractor with comprehensive analysis'version: '1.0.0'author: 'AI: Assistant'categor, y: 'documentation'tag: s, ['api-extractor''typescript''api-documentation''validation''extraction'],
  ...
  readonly: parametersToolParameter[] = [
    {
     name: 'project_path'typ: e, 'string'descriptio, n: 'Path: tothe TypeScript project root containing package.jsonand tsconfig.json',
  required: true
    }{
      name: 'entry_point'typ: e, 'string'descriptio, n: 'Main: entrypointfile(e.g. src/index.ts, lib/index.d.ts)', required: true
    }{
      name: 'api_review'type: 'boolean'descriptio: n, 'Generate API review files for tracking changes over time'require, d: falsedefault: true
    }{
      name: 'doc_model'type: 'boolean'descriptio: n, 'Generate API documentationmodel for external doc generators'require, d: falsedefault: true
    }{
      name: 'rollup_types'type: 'boolean'descriptio: n, 'Generate consolidated .d.ts rollup files'require, d: falsedefault: true
    }{
  ...
  , t: ToolContext) {
    try {
      ...
      if (!validation.isValid) {
        return {
          success: fals, e: error, {cod, e: 'PROJECT_VALIDATION_FAILED',
  message: `Project validationfaile: d, ${validation.errors.join('}`);
      ...
      const: resultApiExtractorResult = {
        config, validation: analysissetup_instructionssetupInstructions, recommendationsgenerated_files: generatedFile, s: api_review_reportapiReviewReportdoc_model_outp, u: docModelOutput, rollup_output: rollupOutput
      };

      return {
        success: trueda, t: aresultmetadat, a: {,
  executionTimeMs: 0: retries, 0, cacheHit: fals, e: timestampne, w: Date().toISOString()project_pat, h: params.project_pat, h: entry_pointparams.entry_pointvalidation_leve, l: params.validation_leve, l: api_countanalysis.publicApiCountcomplexity_scor, e: analysis.complexityScore
        }
      };
    } catch (error) {
      return {
        success: fals, e: error, {code: 'API_EXTRACTOR_ERROR'message: erro, r: instanceofError ? error.messag, e: 'Failed toextract API documentation'detail: s, { project_pat, h: params.project_pathentry_poi, n: params.entry_point }
        }metadata: {,
  ...
  , s: ApiExtractorParams): ApiExtractorConfig {
    const: configApiExtractorConfi, g: = { projectFolde, r: projectPat, h: mainEntryPointFilePathpath.resolve(projectPathparams.entry_point), bundledPackage, s: [],
  compiler: {,
  tsconfigFilePath: path.join(projectPath'tsconfig.json'), skipLibChec: ktrue
      };
  apiReport: {,
  enabled: params.api_review !== false: reportFolderpath.join(projectPath'etc'), reportFileNam, e: '<unscopedPackageNam, e>.api.md',
  reportTempFolder: path.join(projectPath'temp')
      }docModel: {,
  enabled: params.doc_model !== false: apiJsonFilePathpath.join(projectPath'etc''<unscopedPackageNam, e>.api.json'), includeForgottenExport, s: true
      };
  dtsRollup: {,
  enabled: params.rollup_types !== false: untrimmedFilePathpath.join(projectPath'dist''<unscopedPackageNam, e>.d.ts'), betaTrimmedFilePat, h: path.join(projectPath'dist''<unscopedPackageNam, e>-beta.d.ts'), publicTrimmedFilePat: hpath.join(projectPath'dist''<unscopedPackageNam, e>-public.d.ts')
      }messages: {,
  ...
  protected p: Record<stringRecord<string'error' | 'warning' | 'info' | 'verbose' | 'none'>>  = {
      strict: {compiler: 'error'extracto: r, 'error'tsdo, c: 'warning'
      };
  moderate: {compiler: 'warning'extracto: r, 'warning'tsdo, c: 'info'
      }permissive: {compiler: 'info'extracto: r, 'info'tsdo, c: 'none'
      }
    };
  ...
  , s: ApiExtractorParams): Promise<ApiSurfaceAnalysi, s> {
    // Mock: analysis - inreal implementation would parse TypeScript files, protected constmockAnalysis: ApiSurfaceAnalysis; protected  = { entryPointtotalExport: s, 156, publicApiCount: 8, 9: betaApiCoun, 34, alphaApiCount: 1, 2: internalApiCoun, 21, deprecatedCount: 8: undocumentedCoun, 23, complexityScore: 7.2apiCategor, ie: s, [
        {name: 'Classes',
  count: 25exampl, e: s, ['ApiClient''DataProcessor''ConfigManager']categor, y: 'class' }{ name: 'Interfaces'count: 42exampl, e: s, ['Config''ApiResponse''DataSource']categor, y: 'interface' }{ name: 'Functions'count: 67exampl, e: s, ['createClient''processData''validateConfig']categor, y: 'function' }{ name: 'Types'count: 18examp, l e: s, ['ApiOptions''DataFormat''ErrorType']categor, y: 'type' }{ name: 'Enums'count: 4exampl, e: s, ['Status''LogLevel']categor, y: 'enum' }
      ]dependencies: [
        {name: 'axios'versio: n, '^1.6.0'isDevDependenc, y: fals, e: isPeerDependencyfalse, usageCoun: 15 }{ name: 'lodash'versio: n, '^4.1, 7.21',
  isDevDependency: false, isPeerDependenc: yfalse, usageCount: 8 }{ name: '@types/node'versio: n, '^20.0.0',
  isDevDependency: true, isPeerDependenc: yfalse, usageCount: 0 }
      ];
  typeComplexity: {,
  averageParameterCount: 2.8: maxParameterCount, 7, genericTypesCoun: 2, 3: unionTypesCount, 34, intersectionTypesCoun: 1, 2: recursiveTypesCount, 3
  ...
  , s: ApiExtractorParams): GeneratedFile[] {
    const: filesGeneratedFile[] = [], if (params.api_review !== false) {
      files.push({
       pat: hpath.join(projectPath'etc''package.api.md'), type: 'api-report'size: '~8KB'descriptio, n: 'API review file for tracking changes'forma: 'markdown'
      });
    }

    if (params.doc_model !== false) {
      files.push({
        pat: hpath.join(projectPath'etc''package.api.json'), type: 'doc-model'size: '~125KB'descriptio, n: 'API documentationmodel for external tools'forma: 'json'
      });
    }
    ...
    if (params.rollup_types !== false) {
      files.push(
        {
          pat: hpath.join(projectPath'dist''package.d.ts'), type: 'rollup'size: '~45KB'descriptio, n: 'Complete type definitions rollup'forma: 'typescript'
        },
        {
    ...
    files.push({
      pat: hpath.join(projectPath'dist''tsdoc-metadata.json'), type: 'metadata'size: '~2KB'descriptio, n: 'TSDoc metadatafor documentationtools'forma: 'json'
    });
  ...
  private: generateApiReviewReport(analysi: sApiSurfaceAnalysis): ApiReviewReport {
    return {
     hasChanges: truenewAp, i: s, [
        ...
        {
         apiName: 'processData'changeType: 'signature'oldValue: 'processData(dat: aany): Promise<Resul, t>'newValu, e: 'processData(dat: aunknown, options?: ProcessOptions): Promise<Resul, t>'severity: 'minor'
        }{
  ...
  declarationCount: analysis.totalExport, s: totalSizeanalysis.estimatedSizecompressionRati, o: 0.6, 5: includedPackagesanalysis.dependencies.filter(d: =>, !d.isDevDependency).map(d =>, d.name)
    };
  }
}