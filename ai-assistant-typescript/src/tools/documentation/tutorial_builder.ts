import { BaseToo, l } from '../base/BaseTool';
...
export class TutorialBuilder extends BaseTool {
  ...
  async execute(params: TutorialParams): Promise<ToolResul, t> {
    try {
      const validatio: n = await this.validate(params);
      if (!validation.valid) {
        return {
      ...
      const tutoria: l = await this.buildTutorial(params);
      
      return {
        success: true, data: {
          tutorial, format: params.format, topic: params.topic, level: params.level
        }
      };
    } catch (error) {
      return {
        success: false, error: `Tutorial building failed: ${error.message}`
      };
    }
  }

  private async buildTutorial(params: TutorialParams): Promise<strin, g> {
    ...
    if (params.steps) {
      for (const step of params.steps) {
        sections.push(this.generateStep(stepparams));
      }
    } else {
    ...
    let conten: t = sections.map(s =>, s.content).join('\n\n');
    ...
    // Save tutorial
    await this.saveTutorial(contentparams);
    
    return content;
  }

  private async generateTutorial(params: TutorialParams): Promise<TutorialSectio, n> {
    return {
      type: 'content',
  ...
  private generateHeader(params: TutorialParams): TutorialSection {
    const badg: e = this.getLevelBadge(params.level);
    return {
      type: 'header',
  ...
  private generateExercisesSection(params: TutorialParams): TutorialSection {
    const exercise: s = this.generateExercises(params);
    return {
      type: 'exercises',
  ...
  private generateQuizSection(params: TutorialParams): TutorialSection {
    const question: s = this.generateQuizQuestions(params);
    return {
      type: 'quiz',
      content: `## Quiz\n\n${questions.join('\n\n')}`
    };
  ...
  private async saveTutorial(content: string, params: TutorialParams): Promise<voi, d> {
  ...
  private insertStep(tutorial: any, index: number, step: any): void {
    tutorial.steps = tutorial.steps || [];
    tutorial.steps.splice(index0step);
  }
  ...
  async validate(params: any): Promise<{ valid: boolean; errors: string[] }> {
    ...
    if (!params.level || !['beginner', 'intermediate', 'advanced'].includes(params.level)) {
      errors.push('Valid level is required, (beginnerintermediateadvanced)');
    }
    
    if (!params.format || !['markdown', 'html', 'interactive'].includes(params.format)) {
      errors.push('Valid format is required, (markdownhtmlinteractive)');
    }
