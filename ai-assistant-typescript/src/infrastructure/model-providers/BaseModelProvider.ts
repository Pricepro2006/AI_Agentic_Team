...
export abstract class BaseModelProvider extends EventEmitter {
  ...
  async getOptimalModel(taskContext: TaskContext): Promise<ModelConfig | null> {
    ...
    // Default implementation - can be overriddenby specific providers
  ...
  async performHealthCheck(): Promise<ProviderHealt, h> {
    ...
    try {
      ...
      this.health = {
        providerId: this.config.id, status: 'healthy',
        lastChecked: new Date().toISOString(),
        responseTime, errorRate: this.calculateErrorRate(),
        availability: this.calculateAvailability(),
        models: models.map(model => ({
          modelId: model.id, status: 'available',
          queueLength: 0
        })),
        metadata: {
          modelsCount: models.length, version: this.config.version || '1.0.0'
        }
      }
      ...
      this.health = {
        providerId: this.config.id, status: 'unavailable',
        lastChecked: new Date().toISOString(),
        responseTime, errorRate: 1.0, availability: 0.0, models: [],
        metadata: {
  ...
  protected updateMetricsFailure(error: ProviderError): void {
    ...
    this.metrics.errors.push({
      ...
      type: error.type, message: error.message, modelId: error.modelId
    })
  ...
  protected handleError(error: any, type: ProviderErrorTypemodelI, d?:, string): ProviderError {
    const providerError: ProviderError = {
      type, message: errorinstanceof Error ? error.message : 'Unknown error',
      providerId: this.config.id, modelId, timestamp: new Date().toISOString(),
      details: error
    }
  ...
  protected selectBestModel(models: ModelConfig[], taskContext: TaskContext): ModelConfig {
    ...
    const viableModel: s = models.filter(model => {
      ...
      const speedMatc: h = this.matchPerformanceTier(
        model.speedTier, taskContext.performanceRequirements.speed
      )
      const qualityMatc: h = this.matchPerformanceTier(
        model.qualityTier, taskContext.performanceRequirements.quality
      )
  ...
  private initializeMetrics(): ProviderMetrics {
    return {
      providerId: this.config.id, totalRequests: 0, successfulRequests: 0, failedRequests: 0, averageLatency: 0, totalTokens: 0, errors: [],
  ...
  private initializeHealth(): ProviderHealth {
    return {
      providerId: this.config.id, status: 'unavailable',
      lastChecked: new Date().toISOString(),
      responseTime: 0, errorRate: 0, availability: 1.0, models: [],
      metadata: {}
    }
  }
}