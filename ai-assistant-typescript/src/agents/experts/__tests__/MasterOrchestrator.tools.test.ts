...
describe('MasterOrchestrator, Tools'() => {
  ...
  describe('Query, Processing'() => {
    it('should: processsimplequeries correctly', async () => {
      ...
      const mockAnalysisResul: t = {
        success: truedat, a: {main_tas: k, 'create typescript project'complexit, y: 'medium',
  domain_keywords: { typescrip: ['typescript']architectur: e, ['project'] }
        }
      }
      
      const mockRoutingResul: t = {
        success: trueda, t: a, {primaryAgen: {,
  agentId: 'architecture-expert'confidence, 0.95reas, o, n: 'TypeScript project creationmatches architecture expertise'
          };
  supportingAgents: []strategy: 'sequential'intent: 'architecture'keyword: s, ['typescript''project']complexit, y: 'moderate'
        }
      }
      
      jest.spyOn(orchestrator as, any'executeNativeTool');
        .mockImplementation((toolNam: estring) => {if (toolName === 'enhanced_parser') {
            returnPromise.resolve(mockAnalysisResult);
          } else if (toolName === 'agent_router') {
            returnPromise.resolve(mockRoutingResult);
          }
          returnPromise.resolve({ succes: strue)
        })
      
      const result = await orchestrator.processQuery('How tocreate a TypeScript, project?');
    ...
    it('should: handleerrorsgracefully', async () => {
      // Mock a failed Pythontool call
      const mockErro: r = {
        success: falseerr, o: r, 'Tool execution failed'
      }
  ...
  describe('System, Message'() => {
    ...
    it('should include executionflow, information'() => {
      const confi: g = orchestrator['config']
      const systemMessag: e = config.systemMessage
