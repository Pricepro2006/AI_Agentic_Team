// Direct tool tests for TestingAndQAExpert without BaseAgent dependencies
...
const TestingAndQAExpert = jest.requireActual('../TestingAndQAExpert').TestingAndQAExpert
...
describe('TestingAndQAExpert Tools Direct Testing', () => {
  ...
  describe('Tool Method Direct Calls', () => {
    it('should execute test suite generator directly', async () => {
      const params = {
        ...
        test_patterns: ['**/*.test.ts', '**/__tests__/**/*.ts'],
        mock_strategy: 'hybrid',
    ...
    it('should execute QA workflow orchestrator directly', async () => {
      const params = {
        workflow_type: 'ci-cd',
        test_stages: ['unit', 'integration', 'e2e', 'performance'],
        quality_gates: {
          unit_tests: { pass_rate: 100, coverage: 80 },
          e2e_tests: { pass_rate: 95, max_duration: 600 }
        },
        parallel_stages: true,
        failure_strategy: 'fail-fast',
        notification_channels: ['slack', 'email'],
        artifacts_retention: {
          test_reports: { retention: '30d' }
        }
      }
    ...
    it('should execute test data manager directly', async () => {
      const params = {
        ...
        mock_frameworks: ['jest-mock', 'msw'],
        data_types: ['api-responses', 'database-records', 'user-interactions'],
    ...
    it('should execute performance load tester directly', async () => {
      const params = {
        test_tool: 'k6',
        test_scenarios: ['load', 'stress', 'spike'],
        target_endpoints: ['/api/users', '/api/products', '/api/orders'],
        load_configuration: {
          virtual_users: 100,
          duration: '10m',
          ramp_up: '1m'
        ...
        monitoring_integration: ['prometheus', 'grafana'],
  ...
  describe('Tool Definition Validation', () => {
    ...
    it('should have proper parameter validation for test suite generator', () => {
      const toolDefinitions = expert.getToolDefinitions()
      const generatorTool = toolDefinitions.find((tool: any) => tool.name === 'test_suite_generator')
      
      expect(generatorTool.parameters.properties.test_framework.enum).toEqual(['jest', 'vitest', 'playwright', 'cypress', 'webdriverio'])
    ...
    it('should have proper parameter validation for coverage analyzer', () => {
      const toolDefinitions = expert.getToolDefinitions()
      const coverageTool = toolDefinitions.find((tool: any) => tool.name === 'coverage_analyzer')
      
      expect(coverageTool.parameters.properties.coverage_tool.enum).toEqual(['nyc', 'c8', 'istanbul', 'jest-coverage'])
  ...
  describe('Error Handling', () => {
    it('should handle test suite generator with missing parameters', async () => {
      ...
      expect(result.retries).toBe(0)
      expect(result.error).toContain('Source path, test framework, and test types are required')
    })
    ...
    it('should handle test data manager with missing parameters', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.retries).toBe(0)
      expect(result.error).toContain('Data strategy and data types are required')
    })
    ...
    it('should handle performance load tester with missing parameters', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.retries).toBe(0)
      expect(result.error).toContain('Test tool, scenarios, and target endpoints are required')
    })
  })

  describe('Helper Method Testing', () => {
    it('should generate correct framework configuration for Jest', async () => {
      const params = { coverage_threshold: 85 }
      const config = await expert.generateFrameworkConfig('jest', params)
    ...
    it('should generate correct framework configuration for Vitest', async () => {
      const params = { coverage_threshold: 80 }
      const config = await expert.generateFrameworkConfig('vitest', params)
    ...
    it('should analyze source code for testing', async () => {
      const analysis = await expert.analyzeSourceForTesting('./src', ['unit', 'integration'])
    ...
    it('should generate test files based on analysis', async () => {
      const analysis = {
        test_candidates: { unit: 10, integration: 5, e2e: 3 }
      }
      const params = { test_types: ['unit', 'integration'] }
      
      const result = await expert.generateTestFiles(analysis, params)
    ...
    it('should setup mocking strategy correctly', async () => {
      const setup = await expert.setupMockingStrategy('hybrid', 'jest')
    ...
    it('should generate parallel test configuration', async () => {
      const config = await expert.generateParallelTestConfig('jest')
  ...
  describe('Coverage Analysis', () => {
    it('should generate coverage configuration correctly', async () => {
      const params = {
        coverage_tool: 'nyc',
        exclude_patterns: ['node_modules', '**/*.test.ts'],
        coverage_thresholds: { lines: 80, branches: 75 },
        report_formats: ['html', 'json'],
        include_untested: true,
        branch_coverage: true
      }
      
      const config = await expert.generateCoverageConfig(params)
    ...
    it('should evaluate coverage thresholds correctly', () => {
      const analysis = {
        line_coverage: 85,
        branch_coverage: 70,
      ...
      const results = expert.evaluateCoverageThresholds(analysis, thresholds)
    ...
    it('should generate coverage improvement suggestions', () => {
      const analysis = {
        line_coverage: 70,
        branch_coverage: 65,
        uncovered_files: ['file1.ts', 'file2.ts']
      }
      const thresholds = {
        results: { lines: false, branches: false }
      }
      
      const suggestions = expert.generateCoverageImprovementSuggestions(analysis, thresholds)
    ...
    it('should generate coverage reports in different formats', async () => {
      const analysis = { overall_coverage: 85 }
      const formats = ['html', 'json', 'lcov']
      
      const reports = await expert.generateCoverageReports(analysis, formats)
  ...
  describe('QA Workflow Management', () => {
    it('should generate QA workflow configuration', async () => {
      ...
      const config = await expert.generateQAWorkflowConfig(params)
    ...
    it('should generate stage execution plan', async () => {
      const stages = ['unit', 'integration', 'e2e']
      const plan = await expert.generateStageExecutionPlan(stages, true)
    ...
    it('should setup failure strategy correctly', async () => {
      const strategy = await expert.setupFailureStrategy('fail-fast')
      
      expect(strategy.description).toBe('Stop on first failure')
    ...
    it('should setup notifications for multiple channels', async () => {
      ...
      const setup = await expert.setupNotifications(channels)
    ...
    it('should calculate workflow duration correctly', () => {
      ...
      const duration = expert.calculateWorkflowDuration(plan)
      
      expect(duration).toBe(45)
    })

    it('should generate pipeline YAML configuration', () => {
      const config = { name: 'Test Workflow' }
      const yaml = expert.generatePipelineYAML(config, 'ci-cd')
  ...
  describe('Test Data Management', () => {
    it('should setup data generation strategy', async () => {
      const setup = await expert.setupDataGenerationStrategy('factories', {})
    ...
    it('should configure mocking frameworks', async () => {
      const frameworks = ['jest-mock', 'msw']
      const setup = await expert.configureMockingFrameworks(frameworks)
    ...
    it('should setup Faker integration', async () => {
      const dataTypes = ['api-responses', 'database-records']
      const setup = await expert.setupFakerIntegration(dataTypes)
    ...
    it('should generate test data examples', async () => {
      const dataTypes = ['api-responses', 'user-interactions']
      const examples = await expert.generateTestDataExamples(dataTypes, 'factories')
  ...
  describe('Performance Testing', () => {
    it('should generate load test configuration for K6', async () => {
      const params = { test_tool: 'k6' }
      const config = await expert.generateLoadTestConfig(params)
    ...
    it('should setup performance monitoring', async () => {
      ...
      const setup = await expert.setupPerformanceMonitoring(tools)
    ...
    it('should generate performance scenarios', async () => {
      const scenarios = ['load', 'stress']
      const endpoints = ['/api/test']
      const result = await expert.generatePerformanceScenarios(scenarios, endpoints)
    ...
    it('should evaluate performance thresholds', () => {
      const results = {
        response_times: { p95: 1500, p99: 2500 },
      ...
      const evaluation = expert.evaluatePerformanceThresholds(results, thresholds)
    ...
    it('should generate performance reports', async () => {
      const results = { response_times: { average: 850 } }
      const formats = ['html', 'json']
      const reports = await expert.generatePerformanceReports(results, formats)
  ...
  describe('Utility Methods', () => {
    ...
    it('should get required environment variables for stages', () => {
      const envVars = expert.getRequiredEnvVars(['e2e', 'performance'])
