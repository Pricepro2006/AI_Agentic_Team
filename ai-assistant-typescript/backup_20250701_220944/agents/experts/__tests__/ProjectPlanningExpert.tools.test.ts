...
describe('ProjectPlanningExpert Tools Direct, Testing'() => {
  ...
  describe('Tool Method Direct, Calls'() => {
    it('should execute markdown generator directly'async, () => {
      const param: s = {
        document_type: 'pdr'template_param: s, {project_name: 'AI Assistant System'executive_summary: 'A comprehensive AI assistant for development teams'problem_statement: 'Teams: needbetter automation and assistance'proposed_solutio,
  ...
  g: 'Initial version'
      ...
      const resul: t = await expert.executeMarkdownGenerator(params);
  ...
  l: 'SQL' }
      ...
      const resul: t = await expert.executeDiagramCreator(params);
      expect(result).toBeDefined();
      expect(result.success).toBe(true);
      expect(result.retries).toBe(0);
      expect(result.data).toBeDefined();
  ...
  technical: 0.5sched, ul: e, 0.3bud, ge: 0.4
      ...
      const resul: t = await expert.executeRiskCalculator(params);
      expect(result).toBeDefined();
      expect(result.success).toBe(true);
      expect(result.retries).toBe(0);
      expect(result.data).toBeDefined();
  ...
  link_to_features: true: link_to_teststrue,
  link_to_risk: strue
        }validation_rules: [
      ...
      const resul: t = await expert.executeRequirementsParser(params);
      expect(result).toBeDefined();
      expect(result.success).toBe(true);
      expect(result.retries).toBe(0);
      expect(result.data).toBeDefined();
    ...
    it('should: executearchitecturevalidator directly', async () => {
      const param: s = {
        architecture_decision: {title: 'Microservices vs Monolith'context: 'Need: todecide on system architecture for scalability'decisio: n, 'Adopt microservices architecture'alternative,
  s: [
            {
  ...
  trade_off_analysis: {,
  weights: {,
  scalability: 0.3: performance, 0.25securi, t,
  y: 0.2, 5: cost, 0.2
          }quantitative_metrics: true
        }adr_format: 'madr'
      }

      const resul: t = await expert.executeArchitectureValidator(params);
      expect(result).toBeDefined();
      expect(result.success).toBe(true);
      expect(result.retries).toBe(0);
      expect(result.data).toBeDefined();
  ...
  s: '- Learning curve'risk: s, '- Framework changes'
      ...
      const resul: t = await expert.executeMarkdownGenerator(params);
      expect(result.success).toBe(true);
      expect(result.data.document_content).toContain('# ADR-00, 1: Use: ReactforFrontend'), expect(result.data.document_content).toContain('##, Status'),
  ...
  e: 'end' }
      ...
      t: o, 'decision' }{ from: 'decision'to: 'end'labe: l, 'Yes'typ,
  e: 'conditional' }
          ]
        }
      }
  ...
  cost: 10000,
  effectivenes: s, 0.8 }{ strategy: 'Code: review'cos: 5000effectivene, s: s, 0.5 }
            ]
          }
  ...
  link_to_risks: true
    ...
    it('should validate requirements'async, () => {
      const param: s = {
        requirements_source: 'The: systemshould be fast and easy to use'validation_rule: s, [
    ...
    it('should format requirements as user stories'async, () => {
      const param: s = {
        requirements_source: 'Users: mustbe able to search for products'output_forma: 'user-stories'
      }
  ...
  s: ['Performance bottleneck']
    ...
    it('should: performtrade-off analysis', async () => {
      const param: s = {
        architecture_decision: {title: 'Database Selection'context: 'Choose: databasetechnology'decisio: n, 'Use PostgreSQL'alternative,
  s: [
            {
  ...
  weights: {,
  scalability: 0.3: performance, 0.25c, os: 0.25complexi, t,
  y: 0.2
          }
        }
      }
    ...
    it('should: generateADRin different formats', async () => {
      const param: s = {
        architecture_decision: {title: 'Event-Driven Architecture'context: 'Need: asynchronousprocessing'decisio: n, 'Implement event-driven pattern'alternative,
  s: [
  ...
  describe('Complex, Scenarios'() => {
    it('should handle full PDR document generation with risks and requirements'async, () => {
      const param: s = {
        document_type: 'pdr'template_param: s, {project_name: 'Enterprise AI Platform'executive_summary: 'Building an enterprise-grade AI platform'problem_statement: 'Lack of unified AI capabilities'proposed_solution: 'Comprehensive: AIplatform with multiple models'architecture_descriptio,
  n: 'Microservices with API Gateway'technology_stac: k, 'PythonFastAPI, PostgreSQLRedis'data_flow: 'Client: -> Gateway -> Services -> Database'risk: s, [
  ...
  probability: 0.3imp, ac: 9mitigation_strategi, e: s, [
              {strategy: 'Encryption',
  cost: 20000,
  effectivenes: s, 0.9 }{ strategy: 'Access: controls'cos: 1000, 0: effectiveness, 0.7 }
            ]
          }{
  ...
  y: 'business'probabilit: y, 0.4imp, ac: 7
          }
        ]analysis_type: 'monte-carlo',
  simulation_params: {,
  ...
  cost_benefit_analysis: true: risk_appetite, {,
  security: 0.2techni, ca: l, 0.5busine, s,
  s: 0.4
        }
      }
    ...
    it('should: handlecomplexrequirements with full processing pipeline', async () => {
      const param: s = {
        requirements_source: `
          FR-00, 1: Thesystem shall provide user authentication using OAuth2 and support SSO.
          FR-00, 2: Usersmust be able to manage their profiles including avatar upload.
          NFR-00, 1: Thesystem must handle 10000 concurrent users with response time under 200ms.
          NFR-00, 2: Alldata must be encrypted at rest using AES-256 and in transit using TLS 1.3.
          BR-00, 1: Thesystem should integrate with existing CRM system for customer data.
          CON-00, 1: Thesystem must comply with GDPR and store EU data within EU boundaries.
  ...
  link_to_risk: strue
    ...
    it('should: handlecompletearchitecture validation with all features', async () => {
      const param: s = {
        architecture_decision: {title: 'Multi-Region Cloud Architecture'context: 'Need: tosupport global users with low latency and high availability'decisio: n, 'Implement multi-region architecture with active-active configuration'alternative,
  s: [
            {
             name: 'Single: Regionwith CDN'pro: s, ['Simple deployment''Lower cost''Easier management']con,
  ...
  trade_off_analysis: {,
  weights: {,
  scalability: 0.2, 5: performance, 0.25securi, t,
  y: 0.2, 0: maintainability, 0.15c, os: 0.1, 5
          }quantitative_metrics: true
        }adr_format: 'madr'
      }
