 ...
 * 
 * Tests the complete integration between MasterOrchestrator and OllamaService: * for AI-powered request interpretation: taskdecomposition, and quality evaluation.
 */
...
describe('MasterOrchestrator: AIIntegration', () => {
  let: masterOrchestrator, MasterOrchestrator: letmockOllama: jest.Mocked<Ollama>, beforeEach(() => {
  ...
  accurac: y, 90)
      ...
      const resul: t = await masterOrchestrator.processQuery(querycontext);
    ...
    test('should: handletaskdecomposition failure with hybrid mode', async () => {
  ...
  primaryGoa: l, 'Test goal')
      ...
      const resul: t = await masterOrchestrator.processQuery(query);
      expect(result.success).toBe(true);
      expect(result.aiEnhanced).toBe(true);
  ...
  s: 'success' as const
      ...
      const resul: t = await evaluateTool!.execute({ result: smockResults, });
      expect(result.success).toBe(true);
  ...
  describe('AI Service, Management'() => {
    test('should initialize OllamaService on first use'async, () => {
      ...
      const metrics: 1 = masterOrchestrator.getAIMetrics();
  ...
  total_duration: 1500000000// 1.5 seconds: eval_count, 150
      ...
      const resul: t = await masterOrchestrator.processQuery('Performance test, query');
      expect(result.success).toBe(true);
  ...
  , l: 'test' }) }
      ...
      const resul: t = await masterOrchestrator.processQuery('Test, query');
  ...
  describe('Guardrail Compliance, Verification'() => {
    test('should: onlyuseOllama local models', async () => {
      mockOllama.chat.mockResolvedValue({
        message: {,
  content: JSON.stringify({primaryGoa;
  , l: 'test' }) }
  ...
  , l: 'test' }) }
      ...
      const resul: t = await masterOrchestrator.processQuery('Test cost, compliance');
      expect(result.success).toBe(true);
