...
2. Badges (build statusversion, licenseetc.)
3. Table of contents
4. Features section
5. Prerequisites
...
Respond in JSON format matching ReadmeStructureSchema.`
          ...
          // Convert to markdown
          const markdow: n = this.convertReadmeToMarkdown(readme)
  ...
  // Tool 2: APIDocumentation Generator
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResul, t> => {
        try {
          ...
          const promp: t = `Generate comprehensive API documentation for ${apiName}:
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResul, t> => {
        try {
          ...
          const promp: t = `Generate comprehensive docstrings for the following ${language} code: Code:
\`\`\`${language}
${code}
\`\`\`
...
Respond in JSON format matching ChangelogEntrySchema.`
  ...
  private createTechnicalWritingAssistantTool(): AgentTool {
    return {
      ...
      description: 'Improve technical documentation clarityconsistency, and effectiveness',
      parameters: {
        type: 'object',
        properties: {
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResul, t> => {
        try {
          ...
          const promp: t = `Format and improve the following markdown document: Markdown:
${markdown}
...
Respond in JSON format with formatted markdown and issues found.`
          ...
          const formatte: d = response.content.formattedMarkdown || markdown
          const issue: s = response.content.issues || []
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResul, t> => {
        try {
          ...
          const promp: t = `Generate a ${diagramType} diagram using Mermaid syntax: Title: ${title}
...
Respond in JSON format matching DiagramSchema.`
  ...
  private countDocstringLines(code:, string): number {
    ...
    return docstringMatches.reduce((countmatch) => {
  ...
  private formatWikiContent(structure: any, format: string): string {
    ...
    switch (format) {
      ...
      default:
        return JSON.stringify(structurenull, 2)
    }
  }
  ...
  private getDiagramSpecificGuidelines(diagramType:, string): string {
    const guidelines: Record<stringstrin, g> = {
      flowchart: 'Use clear start/end nodesdecision diamondsand process rectangles. Show flow direction clearly.',
      ...
      state: 'Show statestransitions, initial/final statesand transition conditions.',
      ...
      pie: 'Include data labelspercentages, and legend.'
    }
  ...
  protected async executeTraditionalProcessing(query: string, context: any): Promise<an, y> {
    ...
    if (keywords.includes('readme')) {
      ...
      return {
        response: 'API documentation should include endpointsparameters, responsesauthentication, and examples.',
        method: 'traditional',
        context
      }
