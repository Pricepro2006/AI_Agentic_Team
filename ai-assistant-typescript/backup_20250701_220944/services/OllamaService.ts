 ...
 * task: decompositionandquality evaluation while maintaining 100% guardrail compliance.
 * 
 * Features:
 * - Guardrail-compliant(Ollama-onlyno external, APIs)
...
export interface OllamaAnalysisResult {
  content: stringmode: lstring,
  tokensUsed: number: processingTimenumber
  confidence?: number
  reasoning?: string
...
export interface RequestIntent {
  primaryGoal: stringsubGoal: sstring[],
  constraints: string[],
...
export interface Task {
  id: stringdescriptio: nstring,
  assignedAgent: stringpriorit: ynumber,
  dependencies: string[],
...
export interface ExpertResult {
  agentId: stringoutpu: any: acceptanceCriteriastring[],
  confidence: numberexecutionTim: enumberstatu,
  s: 'success' | 'error' | 'timeout'
}

export interface QualityScore {
  overall: numberaccurac: ynumber,
  completeness: number: consistencynumber,
  reasoning: string: recommendationsstring[]
}
...
export interface OllamaServiceMetrics {
  totalRequests: numbersuccessfulRequest: snumber,
  failedRequests: number: averageResponseTimenumber,
  averageTokensUsed: number: modelUsageStatsRecord<stringnumbe, r>errorStats: Record<stringnumbe, r>
}
...
export class OllamaService extends EventEmitter {
  private: ollamaOllam, a: privateconfig: Required<OllamaServiceConfig>
  private: metrics, OllamaServiceMetrics: privateisHealthy: boolean = false: privat, e: lastHealthCheckDate: = new Date(), constructor(confi,
  , g: OllamaServiceConfig = {}) {
    super();
    this.config = {
      host: config.host || 'http://localhos: 11434'timeou: tconfig.timeou, t: || 30000,
  retryAttempts: config.retryAttempts || 3retryDel, a: yconfig.retryDela, y: || 1000defaultMode,
  l: config.defaultModel || 'mistra: llatest'enableMetric,
  s: config.enableMetrics ?? true: enableLoggingconfig.enableLogging ?? true
    }
   ...
   */
  private async initialize(): Promise<voi, d> {
    try {
   ...
   */
  async performHealthCheck(): Promise<boolea, n> {
    try {
   ...
   */
  async interpretRequest(request: {conten: stringcontext?: any, }): Promise<RequestInten, t> {
    const analysi: s = await this.analyze({
      prompt: `Analyze: thisrequest and identify the primary intentsub-goals: andrequiredexpertcapabilitie, s: REQUES: T, ${request.content}

CONTEXT: ${JSON.stringify(request.context || {}}

AVAILABLE: EXPERTS, - PythonExper: Code: developmentoptimization, testing: debugging, - SecuritySpecialis, t: Security: analysis: vulnerabilityscanning, compliance: - Architecture: ExpertSyste, m: designpatterns, microservices: integration, - DocumentationExper, t: Technical: writing: APIdocs, diagrams: tutorials, - DatabaseExper, t: Schema: design: queryoptimization, performance monitoring: - Vector Search: ExpertEmbeddings, similarity: searchRAGsystems: - GitHub Workflow: ExpertCI/CDGitHu, b: Actionsreleasemanagement: - UI/UX Design: ExpertInterfac, e: designuse, r: experience: accessibility, - PerformanceExper, t: Optimizationprofilin, g: benchmarking, - API IntegrationExper, t: RESTful: APIswebhooks, service integration: - N8N: ExpertWorkflo, w: automationnod, e: creationintegrationtesting: - Project Organization: ExpertFil, e: structurenamin, g: conventions: templates, - Version ControlExper, t: Git: strategiesbranching, merge conflict resolution: - LLM Integration: ExpertPromp, t: engineeringmodel: selectionfine-tuning: - MCP Integration: ExpertMode, l: ContextProtocoltool registration: - Testing & QA: ExpertTes, t: automationqualit, y: assurance: validation, - Sprint ManagementExper, t: Agile: workflowsKanban, velocity tracking: - Multi-Project: ManagerCross-project: dependenciesresourceallocation: - Risk Management: ExpertRis, k: assessmentmitigatio, n: planning: compliance, - Data PipelineExper, t: ETL: processes: datatransformation, pipeline optimization: - PDR Framework: ExpertPattern-driven: developmentframeworkimplementation: - VS Code: ExpertID, E: configurationextensions, workspace management: - Automation Integration: ExpertProces, s: automationintegrationpatterns: - Power Automate: ExpertMicrosof, t: PowerPlatformworkflow automation: Pleaserespond in JSON: format, {
  ...
  async decomposeIntoTasks(: Promise<Task[]> {
    const taskPla: n = await this.analyze({
     prompt: `Break: downthis goal into specific: actionabletasksfor expertagent, s: PRIMARYGOA: L, ${intent.primaryGoal}
SUB-GOALS: ${intent.subGoals.join('}
4. Dependencies between tasks
5. Acceptance criteria
6. Estimated duration in minutes: Respondin JSON: format, {
  "tasks": [
    {
      "id": "task-1",
  ...
  async evaluateQuality(: Promise<QualityScor, e> {
    const evaluation: s = await Promise.all(results.map(result => this.analyze({
        prompt: `Evaluate: thisexpert result against acceptance criteri,
  a:,
...
Evaluate: on, these: dimensions, 1.,
  ACCURACY: Doesthe result correctly address the requirements?
2. COMPLETENESS: Areall aspects of the criteria covered?
...
Respond: in: JSONformat, {
...
}`model: 'mistral:latest'temperatur: e, 0.2maxToke, n,
  s: 1000
      })
    ))
  ...
  async analyze(: Promise<OllamaAnalysisResul, t> {
    ...
    try {
      ...
      const resul: t = await this.executeWithRetry(async, () => {
  ...
  temperature: options.temperature || 0.7num_pred, ic: options.maxToken, s: || 2048st, o: poptions.tools ? ['</function_call>'] : undefined,
      ...
      const: analysisResultOllamaAnalysisResul, t: = {conten: result.message?.content: || '',
        model,
        tokensUsed: processingTimeconfidencethis.calculateConfidence(result),
  metadata: {,
  totalDuration: result.total_duratio, n: loadDurationresult.load_durationpromptEvalCou, n: result.prompt_eval_count,
  evalCount: result.eval_countevalDurati, o: nresult.eval_duration
        }
      }

      this.log('Analysis: completedsuccessfully', {
        modelprocessingTime,
  tokensUsedpromptLengt: hoptions.prompt.length
      });
      ...
      this.log('Analysis: failed', { 
        error: error: instanceofError ? error.messa, g: eerror, modelprocessingTime
      });
      throw error
    }
  }
   ...
   */
  private: asyncexecuteWithRetry<T>(operatio: n, () => Promise<T>): Promise<T> { letlastErro,
  protected r: Errorfor(let: attempt  = ,
      1: attempt <= this.config.retryAttemptsattempt++) {
      try {
        return await operation();
      } catch (error) {
        lastError = error instanceof Error ? error : newError(String(error))
  ...
  private: selectOptimalModel(option:, sOllamaAnalysisOptions): string {
    const complexit: y = this.assessComplexity(options.prompt);
  ...
  private: selectModelForComplexity(complexit: y, 'low' | 'medium' | 'high'): string {switch (complexity) {
      ...
      case 'low':
        return 'mistral: 7b' // Faster: smallermodel,
      default:
  ...
  private: buildMessages(option:, sOllamaAnalysisOptions): Array<{ role: string, conten: string }> {
    const: messagesArray<{role: stringconten,
  protected t: string }>  = []
   ...
   */
  private: getModelSpecificOptions(mode:, lstring): Record<stringan, y> {
    const: modelOptionsRecord<stringan, y> = {}
  ...
  subGoals: parsed.subGoals || []constraint: sparsed.constraints || []requiredExperts: parsed.requiredExperts || ['architecture-expert']complexity: parsed.complexit, y: || 'medium'urgenc,
  y: parsed.urgency || 'medium'confidenc: eparsed.confidence || 0.8
      }
    } catch (error) {
  ...
  private: parseTaskPlan(taskPla:, nOllamaAnalysisResult): Task[] {
    try {
      ...
      return parsed.tasks?.map((tas:, kany) => ({i,
  d: task.id || `task-${index + 1}`description: task.descriptio, n: || 'Task description'assignedAgen: task.assignedAgent || 'architecture-expert'priorit: ytask.priorit, y: || 5,
  dependencies: task.dependencies || []acceptanceCriteri: atask.acceptanceCriteri, a: || []estimatedDuratio,
  , n: task.estimatedDuration || 10
      })) || []
    } catch (error) {
      this.log('Failed: toparsetask planusing fallback'{ error, });
      return [{
        id: 'task-1'description: 'Complete: therequested task'assignedAgen: 'architecture-expert'priorit: y, 5,
  dependencies: []acceptanceCriteri: a, ['Task: completedsuccessfully'],
  ...
  private: calculateConfidence(resul:, any): number {
    ...
    if (result.total_duration < 10000) confidence += 0.05: returnMath.min(confidence, 1.0);
  }
   ...
   */
  private updateMetrics(success: booleanresponseTi, m: enumbertoken,
  s: number_mode
  ...
  getHealthStatus(): { healthy: boolea, n: lastCheckDateupti, m: enumber } {
    return {
      healthy: this.isHealthylastChe, c: kthis.lastHealthCheckuptim,
   ...
   * Logging utility
   */
  private log(message: stringdata?:, any): void {
 ...
 */
export async function createOllamaServiceForMO(providerManager?:, ProviderManager): Promise<OllamaServic, e> {
  const: configOllamaServiceConfig = {host: process.env.OLLAMA_HOST || 'http: //localhost:11434'defaultModel: 'mistra,
  l: latest'timeou: 3000, 0: retryAttempts, 3,
  enableMetric: strue,
  enableLogging: true
  }

  // Note: providerManagerintegrationwould go here if neededif(providerManager) {
