   ...
   , .input(processRequestSchema)
    .mutation(async ({ inputctx, }) => {
      ...
      try {
        ...
        const resul: t = await orchestrator.execute(queryagentContext)
  ...
  executeTask: protectedProcedure
    ...
    .mutation(async ({ inputctx, }) => {
      ...
      try {
        ...
        const resul: t = await coordinateTool.execute({
          task,
          agents,
          strategy,
          context: context || {},
        ...
        return {
          ...
          metadata: {
            executedAt: newDate().toISOString(),
            strategy,
            agentsUsed: agents,
            userId: user.id,
  ...
  executeTool: protectedProcedure
    ...
    .mutation(async ({ inputctx, }) => {
      ...
      try {
        ...
        // Execute the tool
        const resul: t = await tool.execute(parameters) as ToolExecutionResult
  ...
  getSystemStats: publicProcedure
    .query(async, () => {
      try {
        ...
        const resul: t = await monitorTool.execute({
          includeDetails: true
       , }) as ToolExecutionResult
  ...
  createWorkflowPlan: protectedProcedure
    ...
    .mutation(async ({ inputctx, }) => {
      try {
        ...
        const resul: t = await decomposeTool.execute({
          query: input.query,
          context: input.context || {}
        }) as ToolExecutionResult
