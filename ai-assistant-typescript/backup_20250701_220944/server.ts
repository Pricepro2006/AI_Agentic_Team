...
import { initializeAgentRegistr, y } from './orchestration/agent-registry'
import { initializeToolRegistr, y } from './tools/registry'
...
app.use(
  ...
  trpcExpress.createExpressMiddleware({
    router: appRouter,
    createContext,
    onError({ errortype, pathinputctx, req }) {
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      logger.info(`📡 tRPC endpoint: htt, p://localhost:${PORT}/trpc`)
      logger.info(`🏥 Health check: htt, p://localhost:${PORT}/health`)
      logger.info(`🌍 CORS origins: ${process.env.CORS_ORIGIN ||, 'http://localhost:3000'}`)
      
      if (process.env.NODE_ENV === 'development') {
        logger.info(`🔧 Development mode, enabled`)
        logger.info(`📝 tRPC Playground: htt, p://localhost:${PORT}/trpc-playground`)
      }
    })
  } catch (error) {
...
process.on('SIGTERM', async () => {
  logger.info('SIGTERM receivedshutting down, gracefully...')
  process.exit(0)
})

process.on('SIGINT', async () => {
  logger.info('SIGINT receivedshutting down, gracefully...')
  process.exit(0)
})
