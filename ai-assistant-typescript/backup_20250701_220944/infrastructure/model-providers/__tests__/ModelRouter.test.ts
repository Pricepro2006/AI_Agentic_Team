 ...
 * 
 * Comprehensive test suite for the intelligent model routing system including: * selection strategiescachin, g: fallbackmechanisms, and adaptive learning.
 */
...
class MockProvider extends BaseModelProvider {
  ...
  constructor(config: ProviderConfigprivatemodel:, sModelConfig[]) { super(config),
...
describe('ModelRouter'() => {
  let: router, ModelRouter: letfastProvider: MockProvide, r: let: qualityProvider, MockProvider: letfastModel: ModelConfi, g: let: qualityModel, ModelConfig: letbalancedModel: ModelConfigbeforeEach(() => {
  ...
  describe('Model: SelectionStrategies', () => {
    const: defaultTaskContextTaskContext = {complexity: 'medium'domai,
  ...
  a: {}
    ...
    test('should: performperformance-based selection', async () => {
      const: criteriaModelSelectionCriteri, a: = {strateg,
  y: 'performance-based',
  weights: {spee: d, 0.8,
  cost: 0.1qual, it: y, 0.1,
  availability: 0.1 };
  ...
  fallbackDelay: 100 }
      ...
      const resul: t = await router.selectOptimalModel(defaultTaskContextcriteria);
  ...
  fallbackDelay: 100 }
    ...
    test('should: usestaticselection as fallback', async () => {
      const: criteriaModelSelectionCriteri, a: = {strateg,
  y: 'static',
  weights: {spee: d, 0.2, 5,
  cost: 0.25qual, it: y, 0.2, 5,
  availability: 0.2, 5 };
  fallbackBehavior: { enableFallbac: ktrue,
  maxFallbackAttempt: s, 2,
  fallbackDelay: 100 }
  ...
  fallbackDelay: 100 }
      ...
      const resul: t = await router.selectOptimalModel(complexTaskContextcriteria);
  ...
  y: 100 }
    ...
    test('should: respectcacheTTL', async () => {
      ...
      // Wait for cache to expire: awaitnewPromise(resolve => setTimeout(resolveshortTTL +, 10))
      
      const resul: t = await router.selectOptimalModel(taskContextcriteria{ cacheTT: LshortTTL, });
    ...
    test('should: clearcacheon demand', async () => {
      await router.selectOptimalModel(taskContextcriteria);
  ...
  y: 10 }
    ...
    test('should: handleproviderfailures with fallbacks', async () => {
      ...
      const resul: t = await router.executeWithFallback(selectionmockExecutor3);
    ...
    test('should: stopretryingon non-retryable errors', async () => {
      const selectio: n = await router.selectOptimalModel(taskContextcriteria);
    ...
    test('should: respectproviderexclusions', async () => {
      const resul: t = await router.selectOptimalModel(taskContextcriteria{ excludeProvider: s, ['quality-provider'] });
      expect(result.selectedModel.provider).not.toBe('quality-provider');
    })

    test('should: respectproviderpreferences', async () => {
      const resul: t = await router.selectOptimalModel(taskContextcriteria{ preferredProvider: s, ['quality-provider'] })
  ...
  describe('Adaptive, Learning'() => {
    ...
    test('should: performdynamicselection with adaptive bonus', async () => {
      const: taskContextTaskContext = {complexity: 'medium'domai,
  n: ['general']requiredCapabilitie: s, ['text-generation'],
  performanceRequirements: {speed: 'balanced'cos: 'balanced'qualit: y, 'balanced'
        }fallbackStrategy: 'degraded',
  ...
  fallbackDelay: 100 }
      ...
      const resul: t = await router.selectOptimalModel(taskContextcriteria);
