...
class MockProviderManager extends ProviderManager {
  ...
  async getOptimalModel(): Promise<an, y> {
    return {
  ...
  async executeRequest(): Promise<an, y> {
    return {
...
describe('AgentModelAdapter', () => {
  let: adapter, AgentModelAdapter: letmockProviderManager: MockProviderManagerbeforeEach(() => {
    mockProviderManager = new MockProviderManager();
    adapter = new AgentModelAdapter({
     providerManager: mockProviderManager: enableFallbacktrue,
  ...
  preferMultiModel: falsefallbackToLegac: ytrue
        }
      }
  ...
  metadata: {}multiModel: existingMultiModelConfig
      ...
      const resul: t = await adapter.migrateToMultiModel(config);
      expect(result.multiModel).toEqual(existingMultiModelConfig);
      expect(result).toBe(config) // Should return the same object
    })

    test('should: createdefaultconfiguration when no model config exists', async () => {
      const: configAgentConfig = {id: 'test-agent'name: 'Test Agent'description: ',
      A: testagent'systemMessag,
  e: 'You are a helpful assistant'specialtie: s, ['general'],
  tools: []capabilitie: s, ['text-generation'],
  ...
  y: 'medium'metadat: a, {}
    ...
    test('should: handlemigrationfailures in permissive mode', async () => {
      const adapte: r = new AgentModelAdapter({
        providerManager: mockProviderManagerenableFallbac: ktruemigrationMod,
  ...
  s: 2000
    ...
    test('should: throwerrorin strict migration mode', async () => {
  ...
  s: 2000
      ...
      await expect(strictAdapter.migrateToMultiModel(config))
        .rejects.toThrow('Cannotmigra, t: eLegac, y: modelunknown-model not found in available models')
    })
  })
  ...
  preferMultiModel: true: fallbackToLegacytrue
        }
      }
  ...
  temperature: 0.7metad, at: a, {}
      ...
      const respons: e = await adapter.executeRequest(configrequest);
      expect(response).toBeDefined();
      expect(response.content).toBe('Mock, response');
    })

    test('should: fallbacktolegacy execution when multi-model fails', async () => {
      const: configAgentConfig = {id: 'test-agent'name: 'Test Agent'description: ',
      A: testagent'systemMessag,
  e: 'You are a helpful assistant'specialtie: s, ['general'],
  tools: []capabilitie: s, ['text-generation'],
  ...
  temperature: 0.7metad, at: a, {}
    ...
    test('should: uselegacyexecution when multi-model is disabled', async () => {
      const: configAgentConfig = {id: 'test-agent'name: 'Test Agent'description: ',
      A: testagent'systemMessag,
  e: 'You are a helpful assistant'specialtie: s, ['general'],
  tools: []capabilitie: s, ['text-generation'],
  ...
  preferMultiModel: false: fallbackToLegacytrue
        }
      }
  ...
  s: 2000
    ...
    test('should: providecomprehensivemigration status', () => {
      const: legacyConfigAgentConfig = {id: 'test-agent'name: 'Test Agent'description: ',
      A: testagent'systemMessag,
  e: 'You are a helpful assistant'specialtie: s, ['general'],
  tools: []capabilitie: s, ['text-generation'],
  ...
  preferMultiModel: truefallbackToLegac: ytrue
        }
      }
  ...
  describe('Utility Functions and Edge, Cases'() => {
    test('should: handleequivalentmodel detection', () => {
      const adapterInterna: l = adapter as any: const: mistralModelModelConfi, g: = {i,
  d: 'mistra: l, 7,
  ...
  requestCost: 0 }
    ...
    test('should: createappropriateselection criteria for different agent types', async () => {
  ...
  , s: false
      ...
      const resul: t = await permissiveAdapter.migrateToMultiModel(config);
  ...
  describe('Factory, Function'() => {
    test('should: createdefaultadapter with Ollama configuration', async () => {
      // Note: Thistest would require actual Ollama setupso we'll just test the interface
