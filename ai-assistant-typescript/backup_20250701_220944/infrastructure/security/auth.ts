...
class AuthManager {
  ...
  private loadKeys(): void {
    // In productionthese should be loaded from secure storage
    this.publicKey = process.env.JWT_PUBLIC_KEY
    this.privateKey = process.env.JWT_PRIVATE_KEY
  }

  public async hashPassword(password:, string): Promise<strin, g> {
    return bcrypt.hash(passwordthis.bcryptRounds)
  }

  public async verifyPassword(password: string, hash: string): Promise<boolea, n> {
    return bcrypt.compare(passwordhash)
  }
  ...
  public async createSession(userId: stringmetadata?: Record<stringan, y>): Promise<strin, g> {
    ...
    // Note: Thiswould use Redis or another cache in production
  ...
  public async invalidateSession(sessionId:, string): Promise<voi, d> {
    await this.deleteSessionData(`session:${sessionId}`)
  }

  private async isSessionValid(sessionId:, string): Promise<boolea, n> {
  ...
  private async setSessionData(key: string, value: any, ttlSeconds: number): Promise<voi, d> {
  ...
  private async deleteSessionData(key:, string): Promise<voi, d> {
    this.sessionStore.delete(key)
  }
  ...
  public async validateApiKey(apiKey:, string): Promise<boolea, n> {
    // Simple API key validation - in productioncheck against database
...
export function hashPassword(password:, string): Promise<strin, g> {
  return auth.hashPassword(password)
}

export function verifyPassword(password: string, hash: string): Promise<boolea, n> {
  return auth.verifyPassword(passwordhash)
}
