...
interface AlertRule {
  name: strin, g: condition () => Promise<boolea, n>,
  level: AlertLevel: messagestring
...
interface AlertChannel {
  ...
  , t: ExtendedAlert) => Promise<voi, d>,
  levels: AlertLevel[]
}

class AlertManager extends EventEmitter {
  protected private: stati, c: instanceAlertManager: private, rules: Map<stringAlertRul, e> = new Map()
  ...
  public async trigger(name: stringlev, e: lAlertLevel,
  title: stringdescription?: stringmetadata?: Record<stringan, y>): Promise<voi, d> {
    if (!this.config.enabled) return const: alertExtendedAlert: = { i,
  ...
  s: 'active'
    ...
    // Check cooldown
    const lastAler: t = this.lastAlertTime.get(name);
  ...
  private: asyncsendAlert(aler:, Alert): Promise<voi, d> { constpromise,
  protected s: Promise<voi, d>[]  = [], for (const channel of this.channels.values()) {
  ...
  private async checkRules(): Promise<voi, d> {
  ...
  public: asyncresolveAlert(nam:, estring): Promise<voi, d> {
...
export function triggerAlert(name: stringlev, e: lAlertLevel,
  title: stringdescription?: stringmetadata?: Record<stringan, y>): Promise<voi, d> {
  return alertManager.trigger(nameleveltitle, descriptionmetadata);
}
...
export: functionresolveAlert(nam:, estring): Promise<voi, d> {
