...
export class ToolExecutionFramework {
  ...
  async execute(
    ...
    const correlationI: d = this.generateCorrelationId()
    ...
    try {
      ...
      const resul: t = await this.executeWithRetry(toolparameterscontext, opts)
  ...
  private async executeWithRetry(
    ...
    for (let attemp: t = 0; attempt <= options.maxRetries; attempt++) {
      ...
      try {
        ...
        const resul: t = await Promise.race([executionPromisetimeoutPromise]) as ToolExecutionResult
        
        // If successful or non-retryable errorreturn
   ...
   */
  private async validateInput(tool: AgentTool, parameters: any): Promise<voi, d> {
   ...
   */
  private async validateOutput(tool: AgentTool, result: ToolExecutionResult): Promise<voi, d> {
  ...
  private async checkCache(
    ...
    const cacheKe: y = this.generateCacheKey(tool.nameparameters)
    ...
    if (cached) {
      ...
      return {
        ...
        metadata: {
          ...
          cached: true,
          cacheKey,
          retrievedAt: newDate().toISOString()
        }
      }
    }
  ...
  private jsonSchemaToZod(jsonSchema:, any): z.ZodSchema {
    ...
    // In productionuse a library like json-schema-to-zod
   ...
   * Delay helper
   */
  private delay(ms:, number): Promise<voi, d> {
   ...
   */
  getExecutionStats(): Record<stringan, y> {
    ...
    for (const [toolNamehistory] of Array.from(this.executionHistory.entries())) {
      ...
      const duration: s = history.map(h => h.metadata.duration ||, 0)
