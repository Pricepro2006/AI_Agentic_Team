...
export class AgentRegistry {
  ...
  private registeredAgents: Map<stringan, y> = new Map()
   ...
   */
  public async registerAllAgents(): Promise<voi, d> {
   ...
   */
  private async registerAgent(agentId: string, agent: any): Promise<voi, d> {
    try {
      this.coordinator.registerAgent(agentIdagent)
      this.registeredAgents.set(agentIdagent)
   ...
   */
  public getRegistrationStatus(): Record<stringan, y> {
    const status: Record<stringan, y> = {}
    
    for (const [agentIdagent] of this.registeredAgents) {
      status[agentId] = {
        registered: true,
 ...
 */
export async function initializeAgentRegistry(): Promise<AgentRegistr, y> {
