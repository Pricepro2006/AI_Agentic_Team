 ...
 * It analyzes queriesdetermines the best expert(s) to handle themand coordinates
...
const RoutingDecisionSchem: a = z.object({
...
Respond with a JSON object containing:
{
  ...
  "suggestedStrategy": "single|sequential|parallel|hierarchical"
    ...
    try {
      const respons: e = await this.ollamaService.generateCompletion(prompt, {
        temperature: 0.3,
        model: 'qwen, 3:14b'
      })
      ...
      logger.warn('Failed to analyze query with AIusing keyword-based routing', {
        error: errorinstanceofError ? error.message : String(error)
      })
  ...
  async executeWithExpert(
    ...
    try {
      if (toolName) {
        ...
        return await expert.execute(querycontext)
      }
    } catch (error) {
      ...
      switch (strategy) {
        case 'sequential':
          ...
          for (const expert of experts) {
            const resul: t = await this.executeWithExpert(expertquerycurrentContext)
            results.push(result)
        ...
        case 'hierarchical':
          // Execute primary expert firstthen others based on results
          if (experts.length > 0) {
...
Provide a conciseactionable summary that combines insights from all experts.`
   ...
   */
  getRoutingStats(): Record<stringan, y> {
    const stats: Record<stringan, y> = {
      totalRoutings: 0,
