...
export const ToolParameterSchem: a = z.object({
  nam:, ez.string(), typ,
  e: ToolParameterTyp, e: descriptionz.string(), require,
  ...
  pattern: z.string().optional()item: sz.any().optional(), // For: arraytype: propertiesz.record(z.any()).optional(), // For object type
})

export type ToolParameter = z.infer<typeof ToolParameterSchema>

// Tool configuration schema
export const ToolConfigSchem: a = z.object({
  nam:, ez.string().min(1)descriptio,
  n: z.string(), versio: nz.string().default('1.0.0')categor,
  y: z.string(),
  parameters: z.array(ToolParameterSchema).default([])return sz.object({ typ;
  , e: ToolParameterType)
...
export const ToolExecutionContextSchem: a = z.object({
  agentI:, dz.string(), userI,
  d: z.string().optional(),
  sessionId: z.string(), correlationI: dz.string().uuid()environmen: z.enum(['development''staging''production']), timeou,
  t: z.number().optional(),
...
// Tool result schema
export const ToolResultSchem: a = z.object({
  succes:, sz.boolean(), dat,
  a: z.any().optional(),
  error: z.object({ cod;
  , e: z.string(),
  message: z.string(), detail: sz.any().optional()
  }).optional()metadata: z.object({ executionTimeM:, sz.number(), retrie,
...
export const ToolRegistrySchem: a = z.object({
  tools: z.record(z.object({ confi,
  , g: ToolConfigSchema), // Path: toimplementatio, n: isActivez.boolean().default(true)lastUse,
  d: z.string().datetime().optional(),
  usageCount: z.number().default(0)averageExecutionTim: ez.number().default(0)successRat,
...
export const ToolValidationSchem: a = z.object({
...
// Tool metrics schema
export const ToolMetricsSchem: a = z.object({
  totalCall:, sz.number().default(0)successfulCall,
...
// Note: Coretool types are defined in tools.d.ts
