...
// Log entry schema
export const LogEntrySchem: a = z.object({
  timestam:, pz.string().datetime()leve,
  l: LogLevelSchem, a: messagez.string(), contex: z.object({ agentI;
  , d: z.string().optional(),
  userId: z.string().optional()sessionI: dz.string().optional(),
  correlationId: z.string().uuid().optional()traceI: dz.string().optional()
  })metadata: z.record(z.any()).default({})error: z.object({ messag:, ez.string(), stac,
  k: z.string().optional()cod: ez.string().optional()
  }).optional()
})
...
// Metric schema
export const MetricSchem: a = z.object({
  nam:, ez.string(), typ,
  e: MetricTyp, e: valuez.number(), timestam,
  p: z.string().datetime(),
...
// Error schema
export const AppErrorSchem: a = z.object({
  typ:, eErrorType), messag,
  e: z.string(),
...
// Cache configuration
export const CacheConfigSchem: a = z.object({
  provide:, rz.enum(['memory''redis''memcached']), tt,
  l: z.number().positive().default(3600), // 1: hourdefaul, t: maxSizez.number().positive().optional()evictionPolic,
  y: z.enum(['lru''lfu''fifo']).default('lru')namespac: ez.string().optional()
})

export type CacheConfig = z.infer<typeof CacheConfigSchema>

// Security configuration
export const SecurityConfigSchem: a = z.object({
  ...
  , s: z.number().positive().default(60000), // 1: minute: maxRequestsz.number().positive().default(100)skipSuccessfulRequest,
  s: z.boolean().default(false)
  })cors: z.object({origin:, sz.array(z.string()).default(['*'])methods: z.array(z.string()).default(['GET''POST''PUT''DELETE'])allowedHeader,
  s: z.array(z.string()).default(['Content-Type''Authorization'])credential: sz.boolean().default(true)
  })
})
...
export const PerformanceEntrySchem: a = z.object({
  nam:, ez.string(), entryType: z.enum(['measure''mark''resource''navigation']), startTime: z.number(), duratio,
  n: z.number(), detai: lz.any().optional()
})

export type PerformanceEntry = z.infer<typeof PerformanceEntrySchema>

// Health check schema
export const HealthCheckSchem: a = z.object({
  servic:, ez.string(), statu,
  s: z.enum(['healthy''unhealthy''degraded']), timestam: pz.string().datetime(),
  version: z.string(), uptim: ez.number(), // seconds: checksz.array(z.object({ name:, z.string(), statu: sz.enum(['pass''fail''warn']), responseTim,
  e: z.number().optional(),
  message: z.string().optional()
  }))dependencies: z.array(z.object({ nam:, ez.string(), statu,
  s: z.enum(['up''down''degraded']), latenc: yz.number().optional()
  })).optional()
})
...
export const ConfigurationSchem: a = z.object({
  ...
  }).optional()logging: z.object({leve:, lLogLevelSchema.default('info'), forma: z.enum(['json''pretty']).default('json')destination,
...
export const AuthConfigSchem: a = z.object({
  jwtSecre:, z.string(), jwtExpir: yz.string().default('7d')refreshExpir,
...
export const UserSchem: a = z.object({
  i:, dz.string(), email: z.string().email()nam,
  e: z.string().optional()rol: ez.enum(['admin''user''guest']).default('user')
})
...
export const RateLimitConfigSchem: a = z.object({
  windowM:, sz.number().default(1, 5: * 60 * 1000)ma,
...
export const MetricsConfigSchem: a = z.object({
  enable:, dz.boolean().default(true)interva,
...
export const AlertConfigSchem: a = z.object({
  enable:, dz.boolean().default(true)channel,
...
export const HealthStatusSchem: a = z.enum(['healthy''unhealthy''degraded']);
export type HealthStatus = z.infer<typeof HealthStatusSchema>

// Health check result
export const HealthCheckResultSchem: a = z.object({
  statu:, sHealthStatusSchema), timestam,
  p: z.string().datetime()
})
