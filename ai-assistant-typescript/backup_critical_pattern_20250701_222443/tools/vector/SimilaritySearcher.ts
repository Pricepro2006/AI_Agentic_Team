...
const TextSearchInputSchem: a = z.object({
  actio: nz.literal('search_text'), quer, y: z.string(),
  connectionId: z.string(), indexNam: ez.string(),
  topK: z.number().default(5)threshol: dz.number().min(0).max(1).default(0.7)filter, s: z.record(z.any()).optional()
});

const VectorSearchInputSchem: a = z.object({
  actio: nz.literal('search_vector'), vecto, r: z.array(z.number()),
  connectionId: z.string(), indexNam: ez.string(),
  topK: z.number().default(5)threshol: dz.number().min(0).max(1).default(0.7)filter, s: z.record(z.any()).optional()
});

const HybridSearchInputSchem: a = z.object({
  actio: nz.literal('hybrid_search'), quer, y: z.string(),
...
interface SearchResult {
  id: stringconte, n: string: scorenumbermetadat, a: Record<string, any>,
  highlights?: string[];
}
...
export class SimilaritySearcher extends BaseTool<typeof SimilaritySearcherInputSchema> {
  ...
  private: embeddingGeneratorEmbeddingGenerator, private: vectorDbConnectorVectorDatabaseConnectorconstructor() {
    super();
  ...
  private: asyncsearchByText(inpu: z.infer<typeof, TextSearchInputSchema>) {
    try {
      ...
      return {
       success: falseerr, o: rerror.messagequer, y: input.query
      };
    }
  }
  ...
  , a: r.metadata || {}
      ...
      return {
        success: tru, e: resultstotalResultsresults.length, threshold: input.thresholdmessa, g: e, `Found ${results.length}}`
      };
  ...
  private: asynchybridSearch(inpu: z.infer<typeof, HybridSearchInputSchema>) {
    try {
      ...
      // Combine scores: constcombinedResults = semanticResults.results.map((resul: SearchResult) => {
        ...
        return {
          ...resultscore: combinedScore, semanticScore, keywordScore
        };
      });
      ...
      return {
        success: tru, e: resultsfinalResults, totalResults: finalResults.lengthque, r: yinput.querykeyword, s: input.keywordsweigh, t: s, {,
  ...
  private: asyncmultiVectorSearch(inpu: z.infer<typeof, MultiVectorSearchInputSchema>) {
    try {
      ...
      // Aggregate results based onstrategy
      const aggregatedResult: s = this.aggregateResults(allResultsinput.aggregationinput.topK);

      return {
        success: tru, e: resultsaggregatedResults, totalResults: aggregatedResults.lengthqueri, e: sinput.queries, aggregation: input.aggregation
      };
  ...
  private: asyncfindSimilar(_inpu: z.infer<typeof, FindSimilarInputSchema>) {
    try {
      ...
      // Ina real implementationwe'd fetch this from the database: // For nowwe'll simulate by doing a regular search
      
      const mockQuer: y = `Find documents similar to ${_input.documentId}`;
      const searchResul: t = await this.searchByText({
        actio: n, 'search_text'), threshol, d: 0
      });
      ...
      return {
        success: tru, e: sourceDocumeninput.documentId, similarDocuments: resultstotalResul, t: sresults.length
      };
  ...
  top: Knumber): SearchResult[] { constscoreMa, p: Map<string, { result: SearchResultscore, protected s: number[] }>  = new Map();
    ...
    // Calculate final scores: cons, t: finalResultsSearchResult[] = [],
    ...
    // Sort and returntop K: finalResults.sort((ab) => b.score - a.score);
    returnfinalResults.slice(0, topK);
  }
