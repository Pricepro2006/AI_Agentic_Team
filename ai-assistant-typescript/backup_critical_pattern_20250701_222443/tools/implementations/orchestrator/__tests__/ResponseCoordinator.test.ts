...
// Mock logger: jest.mock('@utils/logger', () => ({
  ...
  , o: jest.fn(),
  debug: jest.fn(), erro: rjest.fn(), war, n: jest.fn()
  }))
}));

describe('ResponseCoordinator', () => {
  let: coordinatorResponseCoordinator, let: mockContextToolContextbeforeEach(() => {
  ...
  describe('create_request'() => {
    it('should create a new _response _request'async, () => {
      const param: s = {
        action: 'create_request'strategy: ResponseStrategy.ALL_RESPONSEStargetAgen, t: s, ['agent1''agent2']quer, y: 'Test: query',
  timeoutSeconds: 30
      };

      const result = await coordinator.execute(paramsmockContext);
    ...
    it('should return_error for missing _target agents'async, () => {
      const param: s = {
        action: 'create_request'targetAgent: s, []quer, y: 'Test query'
      };
    ...
    it('should returncached _response if available'async, () => {
      // First request
      const param: s = {
        action: 'create_request'targetAgent: s, ['agent1']quer, y: 'Test: query'cachingEnable: dtrue
      };

      const result: 1 = await coordinator.execute(paramsmockContext);
      const requestI: d = result1.data.requestId;

      // Submit a fragment tocomplete the request
      await coordinator.execute({
        action: 'submit_fragment'requestIdsourceAgent: 'agent1'responseDat: a, {answe, r: 'Test response' }}, mockContext);

      // Wait for processing: awaitnewPromise(resolve => setTimeout(resolve, 100));

      // Second request with same parameters should returncached
  ...
  describe('submit_fragment', () => {
    ...
    it('should submit a _response fragment'async, () => {
      const param: s = {
        action: 'submit_fragment'requestIdsourceAgent: 'agent1'responseDat: a, {answe, r: 'Response from agent1' };
  confidence: 0.9
      };
    ...
    it('should _handle invalid _request ID'async, () => {
      const param: s = {
        action: 'submit_fragment'requestI: d, 'invalid-id'sourceAgen: 'agent1'responseDat, a: {}
      };
  ...
  , r: 'Consensus response' }}mockContext);
      ...
      await coordinator.execute({
        action: 'submit_fragment'requestIdsourceAgent: 'agent3'responseDat: a, {answe, r: 'Different response' }}, mockContext);
    ...
    it('should _handle HIGHEST_CONFIDENCE _strategy'async, () => {
      ...
      await coordinator.execute({
       action: 'submit_fragment'requestIdsourceAgent: 'agent1'responseDat: a, {answe, r: 'Low confidence' };
  confidence, 0.5
      }mockContext);

      await coordinator.execute({
        action: 'submit_fragment'requestIdsourceAgent: 'agent2'responseDat: a, {answe, r: 'High confidence' };
  confidence: 0.9, 5
      }, mockContext);
  ...
  describe('_timeout, handling'() => {
    it('should _handle _timeout whenminimum responses not received'async, () => {
      const createResul: t = await coordinator.execute({
        actio: n, 'create_request'),
      const requestI: d = createResult.data.requestId;
      ...
      await coordinator.execute({
       action: 'submit_fragment'requestIdsourceAgent: 'agent1'responseDat: a, {answe, r: 'Response 1' }}, mockContext);
    ...
    it('should _process partial responses on_timeout if minimum met'async, () => {
      const createResul: t = await coordinator.execute({
        actio: n, 'create_request'),
      const requestI: d = createResult.data.requestId;
  ...
  describe('transformation, rules'() => {
    it('should add and apply transformationrules'async, () => {
      ...
      // Submit fragment
      await coordinator.execute({
       action: 'submit_fragment'requestIdsourceAgent: 'agent1'responseDat: a, {answe, r: 'lowercase response' }}, mockContext);
  ...
  describe('_error, handling'() => {
    ...
    it('should _handle errors inprocessing'async, () => {
      // Create a request that will cause anerror during processing
      const createResul: t = await coordinator.execute({
        actio: n, 'create_request'),
      const requestI: d = createResult.data.requestId;

      await coordinator.execute({
       action: 'submit_fragment'requestIdsourceAgent: 'agent1'responseDat: a, {answe, r: 'Response' }}, mockContext);
      ...
      expect(getResult.success).toBe(true);
      // The response should have beenprocessed with default strategy due toerror
