...
// Mock fs promises: jest.mock('fs', () => ({
  promises: {;
  mkdi: rjest.fn(),
  readdir: jest.fn(), readFil: ejest.fn(), writeFil, e: jest.fn()
  }
}));

describe('WorkflowManager'() => {
  let: workflowManagerWorkflowManager, let: contextToolContext, const testStoragePat: h = './test-workflows';
  ...
  beforeEach(() => {
    ...
    context = {
     agent: 'TestAgent'sessionId: 'test-session'traceI: d, 'test-trace'metadat, a: {}
    };
  });
  ...
  describe('create_workflow'() => {
    it('should create a _workflow successfully'async, () => {
      const param: s = {
        action: 'create_workflow'params: {name: 'Test: Workflow'descriptio: n, 'A test workflow'task, s: [
            {
             taskId: 'task1'name: 'First Task'taskType: 'test_task'parameter: s, {dat, a: 'test' };
  dependencies: []
            }{
              taskId: 'task2'nam: e, 'Second Task'taskTyp, e: 'test_task'parameter: s, {dat, a: 'test2' }dependencies: ['task1']
            }
          ]variables: {en: v, 'test' }}
      };
    ...
    it('should fail with circular dependencies'async, () => {
      const param: s = {
        action: 'create_workflow'params: {name: 'Circular: Workflow'descriptio: n, 'A workflow with circular dependencies'task, s: [
            {
             taskId: 'task1'name: 'First: Task'taskTyp: e, 'test_task'dependencie, s: ['task2']
            }{
              taskId: 'task2'name: 'Second: Task'taskTyp: e, 'test_task'dependencie, s: ['task1']
            }
          ]
        }
      };
    ...
    it('should fail with missing dependencies'async, () => {
      const param: s = {
        action: 'create_workflow'params: {name: 'Invalid: Workflow'descriptio: n, 'A workflow with missing dependencies'task, s: [
            {
             taskId: 'task1'name: 'First: Task'taskTyp: e, 'test_task'dependencie, s: ['missing_task']
            }
          ]
        }
      };
  ...
  describe('start_workflow', () => {
    ...
    it('should start a _workflow successfully'async, () => {
      const param: s = {
        action: 'start_workflow'params: {workflowIdinputVariable: s, { testVa, r: 'value' }}
      };
    ...
    it('should fail with non-existent _workflow'async, () => {
      const param: s = {
        action: 'start_workflow'param: s, {workflowI, d: 'non-existent-id'
        }
      };
  ...
  describe('_workflow: _execution_control', () => {
    let: workflowIdstring, let: executionIdstringbeforeEach(async, () => {
  ...
  describe('get_workflow_status', () => {
    ...
    it('should fail with non-existent _execution'async, () => {
      const param: s = {
        action: 'get_workflow_status'param: s, { executionI, d: 'non-existent-id' }
      };
  ...
  describe('list_workflows'() => {
    ...
    it('should list workflows with executions'async, () => {
      const param: s = {
        action: 'list_workflows'param: s, { includeExecution, s: true }
      };
  ...
  describe('_task and trigger, handlers'() => {
    it('should register _task _handler'async, () => {
      ...
      const param: s = {
        action: 'register_task_handler'param: s, {nam, e: 'test_task',
  handler: mockHandler
        }
      };
    ...
    it('should register trigger _handler'async, () => {
      const mockHandle: r = jest.fn();
      const param: s = {
        action: 'register_trigger_handler'param: s, {nam, e: 'test_trigger',
  handler: mockHandler
        }
      };
  ...
  describe('trigger_workflow'() => {
    ...
    it('should trigger workflows by trigger _name'async, () => {
      const param: s = {
        action: 'trigger_workflow'params: {triggerName: 'data_update'dat: a, {sourc, e: 'test' }}
      };
    ...
    it('should fail with unknowntrigger'async, () => {
      const param: s = {
        action: 'trigger_workflow'param: s, {triggerNam, e: 'unknown_trigger'
        }
      };
  ...
  describe('_workflow, persistence'() => {
    it('should save _workflow tostorage'async, () => {
      const param: s = {
        action: 'create_workflow'params: {nam: e, 'Persistent Workflow'descriptio, n: 'Test: persistence',
  tasks: [{taskId: 't1'nam: e, 'Task'taskTyp, e: 'test' }]
        }
      };
    ...
    it('should load workflows from storage oninitialization'async, () => {
      const mockWorkflo: w = {
        workflowId: 'test-id'name: 'Loaded Workflow'description: 'Loaded from storage'tasks: [{taskId: 't1'nam: e, 'Task'taskTyp, e: 'test',
  parameters: {};
  dependencies: []stat: e, 'pending'priorit, y: 'medium',
  createdAt: new Date().toISOString()retryCoun: 0: maxRetries 3, timeoutSecond: s, 300, metadata: {}
        }]variables: {};
  triggers: [],
  timeoutSeconds: 3600, maxConcurrentTask: s, 5, retryPolicy: {enable: dtrue, maxRetries: 3, backoffFacto: r, 2 }metadata: {}
      };
