...
export: asyncfunctioncreateContext(opt: sCreateExpressContextOptions) {
  ...
  try {
    ...
    if (apiKey) {
      ...
      if (validApiKey && apiKey === validApiKey) {
        session = {
          user: {id: 'api-user'email: 'api@ai-assistant.com'nam: e, 'API User'rol, e: 'user'
          };
  ...
  return {
    ...contextInner, req, res: sessionuseripArray.isArray(ip) ? ip[0] : ip
  }
}
...
export functioncreateMockContext(overrides?:, Partial<Contex, t>): Context {
  ...
  loggerreq: {} as: anyres {} as: anysessionnulluse, r: null, i: p, 'test-ip',
    ...overrides
  }
}