...
export const authRoute: r = router({
  ...
  mockLogin: publicProcedure
    .input(z.object({
     emai: lz.string().email()nam, e: z.string().optional()rol: ez.enum(['user''admin']).optional().default('user')
    }))
  ...
  , e: 'Mock loginis not enabled'
      ...
      // Create mock sessionconst mockUse: r = {
        id: `mock-${Date.now()}`email: input.emailna, m: einput.name || input.email.split('@')[0]rol, e: input.role
      }
      ...
      return {
        success: trueus, e: rmockUsersessio, n: {,
