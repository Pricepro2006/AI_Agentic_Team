...
export abstract class BaseAgent {
  ...
  protected performanceMetrics: PerformanceMetrics = {
    totalExecutions: 0, successfulExecutions: 0, failedExecutions: 0, averageExecutionTime: 0, minExecutionTime: Infinity, maxExecutionTime: 0, averageTokenUsage: 0, totalTokensUsed: 0, successRate: 0, errorCounts: {}
  }
...
if (!this.config || !this.config.id) {
      ...
      console.log(`Agent ${this.config.id} executing query`, {
        agentId: this.config.id, agentName: this.config.name
      })
    } catch (error) {
      ...
      console.error(`Failed to initialize agent ${this.config?.id || 'unknown'}`, {
        error: errorMessage, agentId: this.config?.id
      });
      this.status = 'error'
  ...
  private initializeMastraAgent(): void {
    const mastraTools = this.getToolDefinitions().reduce((acc, tool) => {
      acc[tool.name] = createTool({
        id: tool.name, description: tool.description, inputSchema: z.object(tool.parameters.properties),
    ...
    this.agent = new Agent({
      name: this.config.id, instructions: this.config.systemMessage, model: ollama(modelId),
      tools: mastraTools
    })
  }
...
if (this.config.legacyModel?.model) {
  ...
  public async execute(query: string, context: AgentContext): Promise<AgentResponse> {
    ...
    try {
      ...
      // Log execution start
      console.log(`Agent ${this.config.id} executing query`, {
        agentId: this.config.id, query: query.substring(0, 100),
        correlationId
      })

      // TODO: Implement cache when infrastructure is ready
      // const cacheKey = this.generateCacheKey(query, context);
      // const cachedResponse = await cache.get<AgentResponse>(cacheKey)
      // if (cachedResponse) {
      ...
      // TODO: Cache successful response when infrastructure is ready
      // await cache.set(cacheKey, response, this.getCacheTTL())
      ...
      console.error(`Agent ${this.config.id} execution failed`, {
        agentId: this.config.id, error: errorObj, correlationId
      });
      this.status = 'error'
  ...
  protected processAgentResult(result: { text?: string; usage?: { promptTokens?: number, completionTokens?: number, totalTokens?: number }, toolCalls?: Array<{ toolName: string }>; metadata?: Record<string, unknown> }, startTime: number): AgentResponse {
    ...
    return {
      ...
      agentId: this.config.id, agentName: this.config.name, confidence: this.calculateConfidence(result),
      tokenUsage: {
  ...
  public getMetadata(): AgentMetadata {
    return {
      status: this.status, lastHealthCheck: new Date().toISOString(),
      uptime: process.uptime(),
      currentLoad: this.calculateCurrentLoad(),
      queueLength: 0, // Would need to implement queue
      version: this.config.version, dependencies: this.config.integrations
    }
  }
  ...
  protected async trackPerformance<T>(
    operationName: string, operation: () => Promise<T>
  ): Promise<T> {
  ...
  protected createErrorResponse(error: Error, executionTime: number): AgentResponse {
    return {
      ...
      agentId: this.config.id, agentName: this.config.name, confidence: 0, tokenUsage: {
