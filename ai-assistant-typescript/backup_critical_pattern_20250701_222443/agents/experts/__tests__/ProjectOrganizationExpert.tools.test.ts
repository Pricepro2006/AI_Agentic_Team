...
describe('ProjectOrganizationExpert Tools Direct, Testing'() => {
  ...
  describe('Tool Method Direct, Calls'() => {
    it('should execute directory tree generator directly'async, () => {
      const param: s = {
        project_name: 'test-project'project_type: 'ai'language: 'typescript'complexity, 'medium'feature, s: ['docker''tests']
      }
  ...
  include_dev_dependencies: true
    ...
    it('should execute template engine directly'async, () => {
      const param: s = {
        template_type: 'starter'framework: 'typescript'customization: s, {testin, g: true }
      }
      ...
      expect(result.data.template).toEqual(expect.objectContaining({
        file: sexpect.any(Array), script, s: expect.any(Object)
      }))
    ...
    it('should execute project cleanup directly with dry run'async, () => {
      const param: s = {
        cleanup_type: 'unused_files'dry_run: trueproject_pat, h: process.cwd(), pattern: s, ['**/*.test']preserve_pattern, s: ['**/*.important']
      }
  ...
  describe('Error, Handling'() => {
    ...
    it('should handle template generationwith invalid parameters'async, () => {
      const param: s = {
  ...
  describe('Private Method, Testing'() => {
    ...
    it('should: generatecleanupsummary correctly', () => {
      const mockResul: t = {
        filesAnalyzed: 10, 0: filesRemoved, [{pat, h: 'test.log'siz: e, 1024 }];
  filesArchived: [],
  dependenciesRemoved: []spaceSave: d, 1024, executionTime: 500warnin, g: s, []
      }
  ...
  , d: 1: KB'), expect(summary).toContain('Executiontim: e, 500, ms');
    })
  })
})