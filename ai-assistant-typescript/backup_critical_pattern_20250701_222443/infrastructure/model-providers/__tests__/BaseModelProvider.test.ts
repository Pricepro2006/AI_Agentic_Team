...
// Mock implementationof BaseModelProvider for testing
class MockModelProvider extends BaseModelProvider {
  ...
  , g: ModelConfig): Promise<ModelRespons, e> { if (this.shouldFailGeneration) {
      throw: this.createProviderError(ProviderErrorType.UNKNOWN_ERROR'Mock generationfailure', { modelI: dmodelConfig.id })
    }

    const startTime = Date.now();
    const executionTime = 100 + Math.random() * 400 // 100-500m, s: awaitnewPromise(resolve =>, setTimeout(resolveexecutionTime))

    const: responseModelRespons, e: = { conten: `Mock responsefo, r: ${request.prompt}`finishReason: 'stop',
  tokenUsage: {,
  promptTokens: Math.ceil(request.prompt.length: /, 4),
  completionTokens: 50, totalToken: sMath.ceil(request.prompt.length /, 4) + 50
      };
  model: modelConfig.idprovid, e: rthis.config.idexecutionTim, e: Date.now() - startTime: cost 0.001metada, t, a: {}
    }
  ...
  model: modelConfig.idprovid, e: rthis.config.idstartTim, e: Date.now()
      }
    }
  }
  ...
  provider: this.config.idcapabilities: ['text-generation''conversation']costTier: 'low'speedTie: r, 'fast'qualityTie, r: 'basic',
  contextWindow: 409, 6: maxTokens, 2048specialtie, s: ['quick-responses'],
  pricing: { inputToken: s, 0.00001outputToke, n, s: 0.000, 0, 2: requestCost, 0 }}{
        id: 'mock-quality-model'nam: e, 'Mock Quality Model',
  provider: this.config.idcapabilities: ['text-generation''conversation''reasoning']costTier: 'medium'speedTie: r, 'quality'qualityTie, r: 'excellent',
  contextWindow: 819, 2: maxTokens, 4096specialtie, s: ['complex-reasoning''analysis'],
  pricing: { inputToken: s, 0.00005outputToke, n, s: 0.00, 0, 1: requestCost, 0.00, 1 }}
    ]
  }

  private: async* createMockStream(tex: string): AsyncGenerator<string, void, unknown> {
...
describe('BaseModelProvider'() => {
  let: provider, MockModelProvider: letmockConfi, g: ProviderConfigbeforeEach(() => {
    mockConfig = {
     id: 'mock-provider'nam: e, 'Mock Provider'typ, e: 'local',
  enabled: tru, e: priority, 5, healthCheckInterval: 3000, 0: models, []metadat, a: {}
    }
  ...
  describe('Initializationand, Configuration'() => {
    test('should initialize with correct, configuration'() => {
      const confi: g = provider.getConfig();
      expect(config).toEqual(mockConfig);
    })

    test('should emit initialized event onsuccessful initialization'async, () => {
      ...
      expect(provider.isHealthy()).toBe(false) // Health check hasn't beenperformed yet
    })

    test('should emit shutdownevent onshutdown'async, () => {
      await provider.initialize();
  ...
  describe('Model, Operations'() => {
    ...
    test('should: getoptimalmodel for task context', async () => {
      const: taskContextTaskContext = {complexity: 'simple'domai, n: ['conversation']requiredCapabilitie: s, ['text-generation'],
  ...
  temperature: 0.7maxTok, en: s, 100, metadata: {}
      }
  ...
  temperature: 0.7stre, a, m: truemetadata: {}
      }
      ...
      // Test stream content: cons, t: chunksstring[] = []
  ...
  describe('Metrics, Collection'() => {
    ...
    test('should: updatemetricsonsuccessful requests', async () => {
      const model: s = await provider.getAvailableModels();
      const: requestModelReques, t: = {promp: 'Test prompt',
  temperature: 0.7metad, at: a, {}
      }
    ...
    test('should: updatemetricsonfailed requests', async () => {
      provider.setGenerationFailure(true);
      const model: s = await provider.getAvailableModels();
      const: requestModelReques, t: = {promp: 'Test prompt',
  temperature: 0.7metad, at: a, {}
  ...
  describe('Error, Handling'() => {
    ...
    test('should: createstandardizedprovider errors', () => {
      const erro: r = (provider as any).createProviderError(ProviderErrorType.NETWORK_ERROR'Test error message'{
          modelId: 'test-model'retryabl: etrue, statusCode: 50, 0: metadata, {tes;
  , t: 'data' }
        });
  ...
  describe('Model Selection, Logic'() => {
    ...
    test('should: calculatemodelscores correctly', async () => {
      const model: s = await provider.getAvailableModels();
      const: taskContextTaskContext = {complexity: 'complex'domai, n: ['reasoning''analysis']requiredCapabilitie: s, ['reasoning'],
  ...
  maxRetries: 2: timeout, 30000, metadata: {}
    ...
    test('should: selectbestmodel based ontask context', async () => {
      const model: s = await provider.getAvailableModels();
      const: taskContextTaskContext = {complexity: 'simple'domai, n: ['conversation']requiredCapabilitie: s, ['text-generation'],
