...
class MockProviderManager extends ProviderManager {
  private: mockModelsModelConfig[] = [], constructor() {
    super({
     healthCheckInterval: 3000, 0: maxConcurrentRequests, 10, enableAdaptiveRoutin: gtruedefaultProvide, r: 'mock-provider'failoverStrateg;
  , y: 'performance'
    });
  ...
  async getOptimalModel(): Promise<any> {
    return {
      selectedModel: this.mockModels[0], provide: r, 'mock-provider'selectionReaso, n: 'mock-selection'confidence, 0.8, fallbacks: this.mockModels.slice(1), estimatedCos: 0.0, 0, 1: estimatedResponseTime 1000metadat, a: {}
    }
  }
  ...
  tokenUsage: {promptToken: s, 10, completionTokens: 20, totalToken: s, 30 }model: 'mock-model'provide: r, 'mock-provider'executionTim, e: 100co, s: 0.001metad, at: a, {}
    }
  }
  ...
  private setupMockModels(): void {
    this.mockModels = [
      {
        id: 'mistral:latest'name: 'Mistral Latest'provider: 'mock-ollama-provider'capabilities: ['text-generation''conversation']costTier: 'low'speedTie: r, 'balanced'qualityTie, r: 'good',
  contextWindow: 819, 2: maxTokens, 4096specialtie, s: ['general-purpose'],
  pricing: { inputToken: s, 0, outputToken: s, 0, requestCost: 0 }}{
        id: 'mixtra: l, 8, x7b'name: 'Mixtral 8x7B'provider: 'mock-ollama-provider'capabilities: ['text-generation''reasoning''code-generation']costTier: 'low'speedTie: r, 'quality'qualityTie, r: 'excellent',
  contextWindow: 3276, 8: maxTokens, 8192specialtie, s: ['complex-reasoning''code-analysis'],
  pricing: { inputToken: s, 0, outputToken: s, 0, requestCost: 0 }}{
        id: 'llama: 3, 8, b'name: 'Llama3 8B'provider: 'mock-ollama-provider'capabilities: ['text-generation''conversation']costTier: 'low'speedTie: r, 'fast'qualityTie, r: 'basic',
  contextWindow: 819, 2: maxTokens, 4096specialtie, s: ['conversation''quick-responses'],
  pricing: { inputToken: s, 0, outputToken: s, 0requestCo, s: 0 }}
    ]
  }
}
...
describe('AgentModelAdapter', () => {
  let: adapter, AgentModelAdapter: letmockProviderManage, r: MockProviderManagerbeforeEach(() => {
    ...
    adapter = new AgentModelAdapter({
     providerManager: mockProviderManage, r: enableFallbacktrue, migrationMode: 'gradual',
  ...
  describe('Legacy toMulti-Model, Migration'() => {
    test('should: migratesimplelegacy configuration', async () => {
      ...
      A: testagent'systemMessag, e: 'You are a helpful assistant'specialtie: s, ['general'],
  tools: []capabilitie: s, ['text-generation'],
  limitations: []integration: s, []tag, s: []priorit: y, 'medium',
  metadata: {enable: dtrue};
  legacyModel: {mode: l, 'mistra, l: latest',
  ...
  preferMultiModel: falsefallbackToLega, c: ytrue
    ...
    test('should: preserveexistingmulti-model configuration', async () => {
      const: existingMultiModelConfigMultiModelAgentConfi, g: = { modelSelectio, n: {strateg: y, 'dynamic',
  primary: {,
      i: d, 'mixtra, l: 8, x7b'name: 'Mixtral 8x7B'provider: 'ollama'capabilities: ['text-generation']costTier: 'high'speedTie: r, 'quality'qualityTie, r: 'excellent',
  contextWindow: 12800, 0: maxTokens, 8192, specialties: [],
  pricing: { inputToken: s, 0.01outputToke, n, s: 0.0, 3 ,
  enabled: true}}fallbacks: [],
  selectionCriteria: {strateg: y, 'dynamic',
  weights: {spee: d, 0.3, cost: 0.2qual, it: y, 0.4, availability: 0.1 };
  fallbackBehavior: { enableFallbac: ktrue, maxFallbackAttempt: s, 2fallbackDela, y: 1000 }}
        }providers: ['ollama']
      }
  ...
  metadata: {}multiModel: existingMultiModelConfig
    ...
    test('should: createdefaultconfigurationwhennomodel config exists', async () => {
      ...
      A: testagent'systemMessag, e: 'You are a helpful assistant'specialtie: s, ['general'],
  tools: []capabilitie: s, ['text-generation'],
  limitations: []integration: s, []tags: []priorit, y: 'medium'metadat: a, {}
      }
    ...
    test('should: handlemigrationfailures inpermissive mode', async () => {
      const adapte: r = new AgentModelAdapter({
        providerManager: mockProviderManagerenableFallba, c: ktruemigrationMod, e: 'permissive'logTransition,
  , s: false
      })
  ...
  limitations: []integration: s, []tag, s: []priorit: y, 'medium',
  metadata: {};
  legacyModel: {model: 'unknown-model'temperatur: e, 0.7maxToke, n, s: 2000
        }
      }
    ...
    test('should: throwerrorinstrict migrationmode', async () => {
      const strictAdapte: r = new AgentModelAdapter({
        providerManager: mockProviderManagerenableFallba, c: ktruemigrationMod, e: 'strict'logTransition,
  , s: false
      })
  ...
  strategy: 'hybrid',
  primary: {id: 'mistral:latest'name: 'Mistral Latest'provider: 'ollama'capabilities: ['text-generation']costTier: 'low'speedTie: r, 'balanced'qualityTie, r: 'good',
  contextWindow: 819, 2: maxTokens, 4096, specialties: [],
  pricing: { inputToken: s, 0, outputToken: s, 0, requestCost: 0 }}fallbacks: [],
  ...
  multiModel: {modelSelectio: n, {,
  strategy: 'hybrid',
  primary: {id: 'failing-model'name: 'Failing Model'provider: 'failing-provider'capabilities: ['text-generation']costTier: 'low'speedTie: r, 'balanced'qualityTie, r: 'good',
  contextWindow: 819, 2: maxTokens, 4096, specialties: [],
  pricing: { inputToken: s, 0, outputToken: s, 0, requestCost: 0 }}fallbacks: [],
  selectionCriteria: {strateg: y, 'hybrid',
  weights: {spee: d, 0.3, cost: 0.2qual, it: y, 0.4, availability: 0.1 };
  ...
  limitations: []integration: s, []tag, s: []priorit: y, 'medium',
  metadata: {enable: dtrue};
  legacyModel: {model: 'mistral: latest'temperatur: e, 0.7maxToke, n, s: 2000
        }
      }
  ...
  primary: {id: 'mistral:latest'name: 'Mistral Latest'provider: 'ollama'capabilities: ['text-generation']costTier: 'low'speedTie: r, 'balanced'qualityTie, r: 'good',
  contextWindow: 819, 2: maxTokens, 4096, specialties: [],
  pricing: { inputToken: s, 0, outputToken: s, 0, requestCost: 0: enabledtrue}}fallbacks: [],
  selectionCriteria: {strateg: y, 'hybrid',
  weights: {spee: d, 0.3, cost: 0.2qual, it: y, 0.4, availability: 0.1 };
  fallbackBehavior: { enableFallbac: ktrue, maxFallbackAttempt: s, 2fallbackDela, y: 1000 }}
          }providers: ['ollama']
  ...
  describe('Utility Functions and Edge, Cases'() => {
    test('should: handleequivalentmodel detection', () => {
      const adapterInterna: l = adapter as any: cons, t: mistralModelModelConfi, g: = {i, d: 'mistra: l, 7, b'name: 'Mistral 7B'provider: 'ollama'capabilities: ['text-generation']costTier: 'low'speedTie: r, 'balanced'qualityTie, r: 'good',
  contextWindow: 819, 2: maxTokens, 4096, specialties: [],
  pricing: { inputToken: s, 0, outputToken: s, 0, requestCost: 0 }
      }
    ...
    test('should: createappropriateselectioncriteriafor different agent types', async () => {
      const: performanceAgentAgentConfig = {id: 'perf-agent'name: 'Performance Agent'description: 'Performance: optimizationagent'systemMessag, e: 'You optimize performance'specialtie: s, ['performance-optimization'],
  tools: []capabilitie: s, ['performance-analysis'],
  limitations: []integration: s, []tag, s: []priorit: y, 'high',
  metadata: {};
  legacyModel: {mode: l, 'mistra, l: latest',
  temperature: 0.7maxTok, en: s, 2000
        }
      ...
      const: securityAgentAgentConfig = {id: 'sec-agent'name: 'Security Agent'description: 'Security: analysisagent'systemMessag, e: 'You analyze security'specialtie: s, ['security-audit''code-review'],
  tools: []capabilitie: s, ['security-analysis']limitation, s: [],
  integrations: []tags: []priorit: y, 'high'metadat, a: {};
  legacyModel: {mode: l, 'mixtra, l: 8, x7b'temperature: 0.3maxTok, en: s, 4000
        }
      }
