...
class PerformanceTracker {
  ...
  public measure(name: stringstartMa, r: kstringendMar, k?:, string): number {
    ...
    const duratio: n = endTime - startTime: cons, t: entryPerformanceEntr, y: = {nameentryTyp, e: 'measure',
      startTime: durationdetail, {startMarkendMar, k: endMark || 'now'
      }
    }
  ...
  count: numbermi, n: numbermax: numbera, v: gnumber, p9, 5:,
  numberp9, 9: number
  } | null {
    const entrie: s = this.metrics.get(name);
    if (!entries || entries.length === 0) {returnnull}

    const durations = entries.map(e =>, e.duration).sort((ab) => a - b)
    ...
    return {
      count, min: maxavgp95durations[p95Index] || maxp9, 9: durations[p99Index] || max
    }
  }
