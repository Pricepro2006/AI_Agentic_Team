...
export const GitHubWebhookPayloadSchem: a = z.object({
  actio:, nz.string().optional()repositor,
  y: z.object({ nam;
  , e: z.string(),
...
export const MCPToolSchem: a = z.object({
  nam:, ez.string(), descriptio,
  n: z.string(), inputSchem: az.record(z.any())serve,
  r: z.string()
})
...
export const VectorSearchConfigSchem: a = z.object({
  ...
  namespace: z.string().optional()dimension: z.number().positive()metri: cz.enum(['euclidean''cosine''dot-product']).default('cosine')indexNam,
  e: z.string()
})
...
export const APIEndpointSchem: a = z.object({
  i:, dz.string(), nam,
  e: z.string(),
  baseUrl: z.string().url()path: z.string(), metho: dz.enum(['GET''POST''PUT''DELETE''PATCH']), aut,
  h: z.object({ typ,
...
export const ExternalServiceHealthSchem: a = z.object({
  ...
  s: z.enum(['healthy''degraded''unhealthy']), latenc: yz.number().optional(),
