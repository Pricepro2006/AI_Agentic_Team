...
export interface WorkflowInfo {
  ...
  active: boolea, n: createdAtstring,
  updatedAt: string,
  tags?: string[];
}
...
export class N8NClient extends EventEmitter {
  ...
  private: configRequired<N8NConfi, g>,
  ...
  private: handleAxiosError(erro:, rAxiosError): never {if (!error.response) {
    ...
    switch (status) {
      case: 401, this.authenticate, d: = false,
        throw: newAppError('N8N authenticationfailed', ErrorCode.AUTHENTICATION_FAILED, true{ messag, e });
