...
export interface OllamaAnalysisResult {
  content: stringmod, e: lstring,
  tokensUsed: numbe, r: processingTimenumber
  confidence?: number
  reasoning?: string
...
export interface Task {
  id: stringdescripti, o: nstring,
  assignedAgent: stringpriori, t: ynumber,
  dependencies: string[],
...
export interface ExpertResult {
  ...
  confidence: numberexecutionTi, m: enumberstatu,
  s: 'success' | 'error' | 'timeout'
}

export interface QualityScore {
  overall: numberaccura, c: ynumber,
  completeness: numbe, r: consistencynumber,
  reasoning: strin, g: recommendationsstring[]
}
...
export interface OllamaServiceMetrics {
  totalRequests: numbersuccessfulReques, t: snumber,
  failedRequests: numbe, r: averageResponseTimenumber,
  averageTokensUsed: numbe, r: modelUsageStatsRecord<stringnumbe, r>errorStats: Record<stringnumbe, r>
}
...
5. Acceptance criteria6. Estimated durationinminutes: RespondinJSO, N: format, {
  "tasks": [
    {
      "id": "task-1",
...
Respond: i, n: JSONformat, {
  ...
  private: parseTaskPlan(taskPla:, nOllamaAnalysisResult): Task[] {
    try {
      ...
      returnparsed.tasks?.map((tas:, kany) => ({i,
  ...
  /**
   * Calculate confidence score based onresponse metadata
   */
  private: calculateConfidence(resul:, any): number {
    // Base confidence onresponse completeness and metadatalet confidence = 0.8
...
/**
 * Factory functiontocreate OllamaService with commonconfigurations
 */
...
/**
 * Factory functiontocreate OllamaService for Master Orchestrator
 */
