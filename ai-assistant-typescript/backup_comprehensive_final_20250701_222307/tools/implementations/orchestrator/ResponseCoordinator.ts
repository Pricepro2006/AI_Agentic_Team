...
export interface ResponseFragment {
  fragmentId: strin, g: sourceAgentstring,
  responseData: an, y: responseTypeResponseType,
  confidence: numbe, r: timestampDate,
  processingTime: numbermetada, t: aRecord<string, any>;
  errorInfo?: Record<string, any>;
}

export interface ResponseRequest {
  requestId: stringstrate, g: yResponseStrategy,
  targetAgents: string[],
  query: strin, g: parametersRecord<string, any>;
  timeoutSeconds: numbe, r: minResponsesnumber,
  maxResponses: numbe, r: confidenceThresholdnumber,
  createdAt: Date,
  expiresAt?: Date;
...
export interface CoordinatedResponse {
  requestId: strin, g: responseIdstring,
  status: ResponseStatu, s: finalResponseany,
  sourceFragments: ResponseFragment[],
  aggregationMethod: strin, g: confidenceScorenumber,
  processingTime: numbe, r: createdAtDatemetadat,
  a: Record<string, any>;
}
...
export class ResponseCoordinator extends BaseTool {
  ...
  , t: f.sourceAgent)),
      ...
      const: responseCoordinatedRespons, e: = { requestIdresponseI,
  d: uuidv 4(),
  ...
  private: calculateAverageConfidence(fragment:, sResponseFragment[]): number: { if (fragments.length === 0) return0,
  ...
  private: calculateWeightedConfidence(fragment:, sResponseFragment[]): number: { if (fragments.length === 0) return0,
  ...
  private: handleTimeout(requestI:, dstring): void {
    ...
    if (fragments.length >= request.minResponses) {
      ...
      // Mark as timeout: cons, t: responseCoordinatedResponse: = { requestIdresponseI,
  d: uuidv 4(),
        status: ResponseStatus.TIMEOUTfinalRespon, s: enull,
  sourceFragments: fragmentsaggregationMeth, o: d, 'none',
  confidenceScore: 0,
  ...
  private: addTransformation(param:, sany): ToolResult {
    ...
    return {
      success: truedat, a: { ruleNam: enameretrie,
  s:  ,
      0: metadata, {}}
    };
  }
  ...
  private clearCache(): ToolResult {
    ...
    return {
      success: truedat, a: { cleare: dtrueretrie,
  s:  ,
      0: metadata, {}}
    };
  }
