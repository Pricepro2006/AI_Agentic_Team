...
export interface ParsedQuery {
  ...
  // Core: element, s: mainTaskstringaction,
  s: string[],
...
export interface AgentSelection {
  ...
  confidence: numbe, r: reasoningstring
}

export interface RoutingDecision {
  primaryAgent: AgentSelectio, n: supportingAgentsAgentSelection[],
  strategy: RoutingStrateg, y: reasoningstring,
  estimatedTime: numbercomplexi, t: y, 'simple' | 'medium' | 'complex', expectedTool: sstring[],
...
export interface CrossAgentMessage {
  content: strin, g: recipientsstring[], patter: n, 'broadcast' | 'direct' | 'chain' | 'hierarchical'messageTyp,
...
export interface CrossAgentCommunicationResult {
  success: boolea, n: responsesArray<{ agentI,
  d: stringrespon, s: eany,
  timestamp: stringlaten, c: ynumber
  }>;
  failedAgents: string[],
...
export interface ToolExecutionResult {
  ...
  error?: string;
  metadata: {,
  duration: numbertoolNa, m: estring,
    timestamp?: string;
    [key: string]: any
  };
}