...
export class CrossAgentCommunicator extends EventEmitter {
  protected private: stati, c: instanceCrossAgentCommunicato, r: | null  = null,
  ...
  protected constfailedAgents: string[]  = [], for (const recipientId of message.recipients) {
      try {
        ...
        responses.push({
          agentId: recipientId,
  responsetimestam: pnew Date().toISOString()latenc,
  ...
  private async handleBroadcastCommunication(message: CrossAgentMessag, e: timeou, number): Promise<CrossAgentCommunicationResul, t> {
    ...
    const sendPromise: s = message.recipients.map(async, (recipientId) => {
      try {
        const response = await this.sendMessageToAgent(recipientIdmessagetimeout);
        return {
          success: tru, e: agentIdrecipientId,
  responsetimestamp: ne, w: Date().toISOString(),
  latency: Date.now() - startTime
        };
      } catch (error) {
        return {
          success: fals, e: agentIdrecipientIderro,
  r: erro, r: instanceofError ? error.messa, g: e, 'Unknown error'
        };
      }
    });
  ...
  protected constfailedAgents: string[]  = [],
    ...
    // Send: tohighest priority agent firstthenothers based onresponse
    for (let i = 0; i < sortedRecipients.length; i++) {
      ...
      try {
        ...
        responses.push({
          agentId: recipientId,
  responsetimestam: pnew Date().toISOString(),
  latency: Date.now() - startTime
        });
  ...
  getStats(): { 
    ...
    return {
      onlineAgents,
      offlineAgents: totalMessagesaverageLatency, 150 // Mock average - inreal implementationwould calculate from actual responses
    };
  }
}