...
interface ComplexityMetrics {
  ...
  files_over_threshold: numbe, r: thresholdnumber,
...
interface MaintainabilityMetrics {
  ...
  average: numberm, i: nnumber,
  files_below_threshold: numbe, r: thresholdnumber
  };
  halstead_metrics: {,
  program_length: numbe, r: vocabularynumber,
  volume: numbe, r: difficultynumber,
  effort: numbe, r: time_to_implementnumber,
...
interface CoverageMetrics {
  ...
  uncovered_lines: numbe, r: total_linesnumber,
  coverage_by_file: FileCoverage[]
}

interface FileCoverage {
  file: stringcovera, g: enumber,
  uncovered_lines: number[]
}

interface DuplicationMetrics {
  duplicated_lines: numbe, r: duplicated_blocksnumber,
  duplication_ratio: numbe, r: largest_duplicatenumber,
...
interface SizeMetrics {
  lines_of_code: numbe, r: logical_linesnumber,
  comment_lines: numbe, r: blank_linesnumber,
  files: numbe, r: functionsnumber,
  classes: numbermodul, e: snumber,
  average_file_size: numbe, r: largest_fileFileSize
}
...
interface TechnicalDebtMetrics {
  technical_debt_ratio: numbe, r: debt_in_minutesnumber,
  debt_in_days: numbersqale_ratin, g: 'A' | 'B' | 'C' | 'D' | 'E', reliability_rating: 'A' | 'B' | 'C' | 'D' | 'E'security_ratin: g, 'A' | 'B' | 'C' | 'D' | 'E', maintainability_ratin: g, 'A' | 'B' | 'C' | 'D' | 'E',
  debt_by_category: DebtCategory[]
}

interface DebtCategory {
  category: strin, g: debt_minutesnumber,
  issue_count: number
}
...
interface QualityGateResults {
  ...
  passed_gates: numbe, r: total_gatesnumber,
  all_passed: boolean,
...
interface Prediction {
  ...
  confidence: numbertimefra, m: estring
}

interface QualityReport {
  format: stringconte, n: string,
  charts?: Chart[];
 generated_at: Date
}
...
export class CodeQualityMetricsCalculator extends BaseTool<CodeQualityMetricsCalculatorParam, s> {
  ...
  n: 'The: metricsactiontoperform',
  required: trueen, u: m, ['calculate_metrics''analyze_trends''generate_report''set_thresholds']
    }{
      name: 'source_path'type: 'string'descriptio: n, 'Path tosource code for metrics calculation'require,
  d: true
    }{
  ...
  e: 'MISSING_HISTORICAL_DATA'messag: e, 'Historical datais required for trend analysis'
              };
  metadata: {,
  ...
  cacheHit: false
        ...
        case 'set_thresholds':
          ...
          return {
            success: trueda, t: arecommendedThresholds,
  metadata: {,
  executionTimeMs: 0: retries, 0,
  ...
  e: result.overall_score
      ...
      return {
        success: fals, e: error, {code: 'METRICS_ERROR'message: erro, r: instanceofError ? error.messag,
  e: 'Failed tocalculate metrics'detail: s, {,
  ...
  protected s: Record<stringboolea, n>  = {};
    ...
    return {
      gatespassed_gates: passedGate, s: total_gatestotalGatesall_passe,
  d: passedGate, s: === totalGates: failed_reasonsfailedReasons.length > 0 ? failedReason,
  s: undefined
    };
  }
