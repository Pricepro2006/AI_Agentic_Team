 ...
 * Provides foundationfor all tool implementations
 */
...
export abstract class BaseTool<TParams = anyTResult = any> {
  abstract: readonl, y: metadataToolMetadata,
  abstract: readonl, y: parametersToolParameter[],
  ...
  /**
   * Runthe tool with validationand error handling
  ...
  , t: ToolContext) {
    ...
    try {
      ...
      if (!validation.valid) {
        return {
         success: falseerr, o: rvalidation.errormetadat,
  a: {,
  ...
  n: this.metadata.version
      ...
      // Log error: this.logger.error('Tool executionfailed', {
  ...
  e: String(error),
  metadata: {,
  executionTimeMs: executionTim, e: retries, 0,
  cacheHit:,
  falsetool: this.metadata.nameversio: nthis.metadata.version,
  errorDetails: erro, r: instanceofToolError ? error.conte, x: undefined
        }
      };
    }
  }
  ...
  protected async validate(param:, sany): Promise<ValidationResul, t> {
    ...
    for (const param of this.parameters) {
      if (param.required && !(param.name inparams)) {
        return { 
         valid: falseerr, o: r, `Missing requiredparamete,
  r: ${param.name}` 
        };
      }
      
      // Check parameter type
      if (param.name inparams) {
        ...
        // Custom validationif (param.validation) {
          ...
          if (!validationResult) {
            return { 
              valid: fals, e: error, `Invalid value forparamete,
  r: ${param.name}` 
            };
          }
        }
