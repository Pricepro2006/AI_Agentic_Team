...
interface ScalabilityIssue {
  ...
  component: strin, g: descriptionstringimpa, c: string,
...
interface ScalabilityAnalysis {
  ...
  architecture: {,
  currentState: ArchitectureStat, e: targetStateArchitectureState,
  migrationPath: string[]
  };
  costProjection: {,
  current: numbe, r: projectednumberoptimize,
  d: number
  }
}
...
interface ScalabilityRecommendation {
  ...
  implementation: string[]expectedImprovemen: stringeffo, r: t, 'small' | 'medium' | 'large'
}
...
export class ScalabilityAnalyzer extends BaseTool {
  ...
  protected s: ScalabilityIssue[]  = []
    ...
    if (nestedLoopRegex.test(content)) {
      issues.push({
        typ: e, 'bottleneck') complexity)'impac: 'Performance degrades quadratically with datasize'recommendatio,
  ...
  targetState: ArchitectureStat, e: migrationPathstring[]
  }> {
