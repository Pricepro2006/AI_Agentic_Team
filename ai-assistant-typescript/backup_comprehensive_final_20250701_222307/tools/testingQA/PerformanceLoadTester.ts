...
interface PerformanceResult {
  summary: PerformanceSummar, y: metricsPerformanceMetrics,
  errors?: ErrorDetails[];
  report_path?: string;
...
interface PerformanceSummary {
  total_requests: numbe, r: successful_requestsnumber,
  failed_requests: numbe, r: test_durationnumber,
  average_rps: numberpeak_r, p: snumber
}

interface PerformanceMetrics {
  response_times: ResponseTimeMetric, s: throughputThroughputMetrics,
  concurrency: ConcurrencyMetrics,
  resource_usage?: ResourceMetrics;
}

interface ResponseTimeMetrics {
  min: numberm, a: xnumber,
  mean: numbermedi, a: nnumber,
  p9, 0: numbe, r: p95number,
  p9, 9: numberstd_d, e: vnumber
}
...
interface ConcurrencyMetrics {
  max_concurrent_users: numbe, r: average_concurrent_usersnumber,
  connection_errors: numbe, r: timeout_errorsnumber
}
...
interface ErrorDetails {
  timestamp: Datescenar, i: ostring,
  step: strin, g: error_typestring,
  message: string,
  status_code?: number;
}

interface ThresholdResult {
  metric: stringpass, e: dboolean,
...
export class PerformanceLoadTester extends BaseTool<PerformanceLoadTesterParam, s> {
  ...
  required: trueen, u: m, ['run_load_test''run_stress_test''analyze_performance']
      ...
      name: 'target_url'type: 'string'descriptio: n, 'URL totest'require,
  d: false
    }{
  ...
  d: false
      ...
      name: 'duration'type: 'number'descriptio: n, 'Test durationinseconds'require,
  d:,
  falsedefault: 60
    }{
  ...
  s: [...insights, ...optimizations]
      ...
      return {
        success: falseerr, o: rerrorinstanceof Error ? error.messag,
  e: 'Failed toanalyze performance'
      };
    }
  }
...
`;
  ...
  private async analyzeTestResults(results: anyconfi, g?:, LoadTestConfig): Promise<any> {
    ...
    return {
      summary: {,
  total_requests: totalRequest, s: successful_requestssuccessfulRequestsfailed_request,
  s: failedRequest, s: test_durationtestDurationaverage_rp,
  s: totalRequests / testDuration: peak_rpsthroughput.requests_per_second.reduce((maxv) => Math.max(maxv), 0)
      }, metrics: {,
  response_times: responseTime, s: throughputthroughputconcurrenc,
  y: concurrency
...
<hea, d>
  ...
  px;
  margin: 0: aut, o: backgroundwhite, paddin: g, 20,
  px; border-radius: 8,
...
<testsuites name="Performance Tests" tests="${analysis.summary.total_requests}" failures="${analysis.summary.failed_requests}" time="${analysis.summary.test_duration}">
  ...
  , e: string): Promise<any> {
    // Runa quick 10-second test with 10 users: cons, t: quickConfigLoadTestConfi, g: = { user,
  s: 1, 0: ramp_up, 2,
