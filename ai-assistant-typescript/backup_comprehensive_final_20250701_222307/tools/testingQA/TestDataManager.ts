...
export class TestDataManager extends BaseTool<TestDataManagerParam, s> {
  ...
  required: trueen, u: m, ['generate_fixtures''create_mocks''manage_test_db']
      ...
      name: 'data_type'type: 'string'description: 'Type: ofdatatogenerate'require: dfalseenu,
  m: ['user''product''transaction''api_response''custom']
    }{
      name: 'count'type: 'number'descriptio: n, 'Number of records togenerate'require,
  d:,
  falsedefault: 10
    }{
      name: 'schema'type: 'object'descriptio: n, 'Custom dataschema'require,
  d: false
    }{
  ...
  private async generateFixtures(dataType: stringcou, n: numbercustomSchem, a?: DataSchemaoutputFormat?:, string): Promise<ToolResul, t> {
    try {
      ...
      // Write fixtures inrequested format: le, t: fixturesPathstring,
  ...
  e: databaseState
      ...
      return {
        success: falseerr, o: rerrorinstanceof Error ? error.messag,
  e: 'Failed tomanage test database'
      };
    }
  }
  ...
  unique: truefak, e: r, 'internet.email' }{ name: 'firstName'typ: e, 'string'fake,
  ...
  unique: truefak, e: r, 'random.alphaNumeric' }{ name: 'inStock'typ: e, 'boolean'fake,
