...
class RateLimiter {
  ...
  , t: any = {}): Promise<{ allowed: boolean; limit?: numberremaining?: numberresetAt?: Date }> {
    ...
    if (entry?.blocked) {
      ...
      return {
        allowed: fals, e: limitrule.maxRequests,
  remaining: 0,
        resetAt
      }
    }
  ...
  remaining: 0: resetAtnew Date((entry?.firstRequest ||, now) + rule.windowMs)
    ...
    // Allow request and increment counter: cons, t: newEntryRateLimitEntr, y: = entry || { coun: 0,
  firstRequest: nowblock, e: dfalse
    }
