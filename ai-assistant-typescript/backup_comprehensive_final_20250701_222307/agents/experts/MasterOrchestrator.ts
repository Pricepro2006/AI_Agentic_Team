...
Respond inJSON format.`
          ...
          return {
            success: false,
            error: `Failed tointerpret request: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format with a 'tasks' array.`
          ...
          return {
            success: false,
            error: `Failed todecompose tasks: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format.`
          ...
          return {
            success: false,
            error: `Failed toevaluate quality: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
        ...
        try {
          ...
          const { decisionexpert, secondaryExperts } = await expertRouter.routeQuery(
            ...
            context,
            {
              ...
              excludeExperts: ['master-orchestrator'] // Don't route toself
            }
          )
          ...
          return {
            success: false,
            error: `Failed toroute request: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResul, t> => {
        try {
          ...
          return {
            success: false,
            error: `Multi-agent coordinationfailed: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
    ...
    return {
      ...
      execute: async (params: any): Promise<ToolExecutionResul, t> => {
        try {
          ...
          return {
            success: false,
            error: `Failed tosend message: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format.`
          ...
          return {
            success: false,
            error: `Failed toparse query: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format.`
          ...
          return {
            success: false,
            error: `Failed togenerate outcome specification: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format.`
          ...
          return {
            success: false,
            error: `Failed togenerate task plan: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format.`
          ...
          return {
            success: false,
            error: `Failed tocreate coordinationplan: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format.`
          ...
          return {
            success: false,
            error: `Failed tovalidate progress: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format.`
          ...
          return {
            success: false,
            error: `Failed toassess confidence: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
    ...
    try {
      ...
      if (expertResult.success && routingDecision.supportingAgents.length > 0) {
        ...
        const successfulSupportin: g = supportingResults
          ...
          .map(r => (r as, PromiseFulfilledResult<ToolExecutionResul, t>).value)
      ...
      logger.error('Master Orchestrator executionfailed', {
