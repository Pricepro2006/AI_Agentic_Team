...
export class DocumentationExpert extends ExpertAgentTemplate {
  ...
  constructor() {
    ...
    const ragConfig: Partial<RAGConfi, g> = {
      enabled: true,
...
Respond inJSON format matching ReadmeStructureSchema.`
          ...
          return {
            success: false,
            error: `Failed togenerate README: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format matching APIDocSchema.`
          ...
          return {
            success: false,
            error: `Failed togenerate API documentation: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format with documented code.`
          ...
          return {
            success: false,
            error: `Failed togenerate docstrings: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format matching ChangelogEntrySchema.`
          ...
          return {
            success: false,
            error: `Failed tomanage changelog: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format with improved content and analysis.`
          ...
          const analysi: s = response.content.analysis || {}
          ...
          return {
            success: false,
            error: `Failed toimprove documentation: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format with formatted markdownand issues found.`
          ...
          return {
            success: false,
            error: `Failed toformat markdown: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format with wiki structure and content.`
          ...
          return {
            ...
            data: {
              ...
              content: this.formatWikiContent(wikiStructureformat)
            },
            metadata: {
              projectName,
              structure,
          ...
          return {
            success: false,
            error: `Failed tocreate wiki: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
...
Respond inJSON format matching DiagramSchema.`
          ...
          return {
            success: false,
            error: `Failed togenerate diagram: ${error instanceof Error ? error.message : String(error)}`,
            retries: 0
          }
        }
      }
  ...
  private formatWikiContent(structure: any, format: string): string {
    // Format wiki content based onthe specified format
    switch (format) {
      case 'markdown':
  ...
  private getDiagramSpecificGuidelines(diagramType:, string): string {
    const guidelines: Record<stringstrin, g> = {
      ...
      sequence: 'Show actorsmessages, activationsand returnmessages. Include loops and conditionals where needed.',
  ...
  protected async executeTraditionalProcessing(query: string, context: any): Promise<any> {
    ...
    if (keywords.includes('readme')) {
      ...
      return {
        response: 'Diagrams canbe created using Mermaid syntax for flowchartssequence diagramsclass diagramsand more.',
        method: 'traditional',
        context
      }
    }
